...
二分法，也称为二分搜索法，使用二分搜索来寻找实值函数的根。它通过不断缩小平方根所在的区间，直到收敛到指定容差范围内的值。

例如，如果容差是`0.01`，二分法会不断将区间减半，直到上下限之间的差小于或等于`0.01`。

在这个实验中，你将实现一个使用二分法来寻找数字平方根的函数。

**目标：** 实现以下用户需求并通过所有测试以完成实验。

**用户需求：**

1. 你应该定义一个名为`square_root_bisection`的函数，它有三个参数：
   - 要寻找平方根的数字。
   - 容差是结果的可接受误差范围。你应该设置一个默认容差值。
   - 要执行的最大迭代次数。你应该设置一个默认迭代次数。

2. `square_root_bisection`函数应该：
   - 如果传递给函数的数字是负数，则抛出带有消息`Square root of negative number is not defined in real numbers`的`ValueError`。
   - 对于数字`0`和`1`，打印消息：`The square root of [number] is [number]`并返回数字本身作为平方根。
   - 对于任何其他正数，打印近似平方根的消息：`The square root of [square_target] is approximately [root]`并返回计算的根值。
   - 如果没有值满足容差条件，打印失败消息：`Failed to converge within the [maximum] iterations`并返回`None`。
...
你的`square_root_bisection`函数应该有三个参数。
...
你的`square_root_bisection`函数在传递给函数的数字为负数时应该抛出带有消息`Square root of negative number is not defined in real numbers`的`ValueError`。
...
`square_root_bisection(0)`应该返回`0`。
...
`square_root_bisection(0.001, 1e-7, 50)`应该返回一个介于`0.03162267660168379`和`0.031622876601683794`之间的数字。

```js
({ test: () => runPython(`assert 0.03162267660168379 <= square_root_bisection(0.001, 1e-7, 50) <= 0.031622876601683794`) })
```

`square_root_bisection(0.001, 1e-7, 50)`应该打印`The square root of 0.001 is approximately X`，其中`X`是一个介于`0.03162267660168379`和`0.031622876601683794`之间的数字。
...
`square_root_bisection(0.25, 1e-7, 50)`应该返回一个介于`0.4999999`和`0.5000001`之间的数字。

```js
({ test: () => runPython(`assert 0.4999999 <= square_root_bisection(0.25, 1e-7, 50) <= 0.5000001`) })
```

`square_root_bisection(0.25, 1e-7, 50)`应该打印`The square root of 0.25 is approximately X`，其中`X`是一个介于`0.4999999`和`0.5000001`之间的数字。
...
`square_root_bisection(81, 1e-3, 50)`应该返回一个介于`8.999`和`9.001`之间的数字。

```js
({ test: () => runPython(`assert 8.999 <= square_root_bisection(81, 1e-3, 50) <= 9.001`) })
```

`square_root_bisection(81, 1e-3, 50)`应该打印`The square root of 81 is approximately X`，其中`X`是一个介于`8.999`和`9.001`之间的数字。
...
`square_root_bisection(225, 1e-3, 100)`应该返回一个介于`14.999`和`15.001`之间的数字。

```js
({ test: () => runPython(`assert 14.999 <= square_root_bisection(225, 1e-3, 100) <= 15.001`) })
```

`square_root_bisection(225, 1e-3, 100)`应该打印`The square root of 225 is approximately X`，其中`X`是一个介于`14.999`和`15.001`之间的数字。
...
`square_root_bisection(225, 1e-5, 100)`应该返回一个介于`14.99999`和`15.00001`之间的数字。

```js
({ test: () => runPython(`assert 14.99999 <= square_root_bisection(225, 1e-5, 100) <= 15.00001`) })
```

`square_root_bisection(225, 1e-5, 100)`应该打印`The square root of 225 is approximately X`，其中`X`是一个介于`14.99999`和`15.00001`之间的数字。
...
`square_root_bisection(225, 1e-7, 100)`应该返回一个介于`14.9999999`和`15.0000001`之间的数字。

```js
({ test: () => runPython(`assert 14.9999999 <= square_root_bisection(225, 1e-7, 100) <= 15.0000001`) })
```

`square_root_bisection(225, 1e-7, 100)`应该打印`The square root of 225 is approximately X`，其中`X`是一个介于`14.9999999`和`15.0000001`之间的数字。
...
`square_root_bisection(225, 1e-7, 10)`应该返回`None`。

```js
({ test: () => runPython(`assert square_root_bisection(225, 1e-7, 10) is None`) })
```

`square_root_bisection(225, 1e-7, 10)`应该打印`Failed to converge within 10 iterations`。
