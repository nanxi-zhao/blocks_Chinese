...
对于这个实验，所有的HTML和CSS都已经提供给你。你将使用JavaScript来完成书签管理器应用，使其能够将书签集合存储在本地存储中并从中读取数据。

**目标：** 实现以下用户需求并通过所有测试以完成实验。

**用户故事：**

1. 你应该有一个`getBookmarks`函数，它返回存储在本地存储中的`bookmarks`数组。如果`bookmarks`键尚未设置，或者它不包含有效的书签数组，`getBookmarks`函数应该返回一个空数组。
2. 存储在本地存储中的`bookmarks`键应该是一个书签对象数组。每个书签对象应该有三个键：`name`、`category`和`url`。
3. 你应该有一个名为`displayOrCloseForm`的函数，它切换`#main-section`和`#form-section`的`hidden`类。
4. 当你点击`#add-bookmark-button`时，你应该将`.category-name`的内部文本更新为`#category-dropdown`中选定选项的值，并调用`displayOrCloseForm`来显示表单部分并隐藏主部分。
5. 当你点击`#close-form-button`时，你应该运行你的函数来隐藏表单部分并显示主部分。
6. 当你点击`#add-bookmark-button-form`时，你应该通过在数组末尾添加一个书签对象来更新存储在本地存储中的`bookmarks`键。该对象应该将`name`设置为`#name`输入的值，`category`设置为类别下拉菜单中选定选项的值，`url`设置为`#url`输入的值。
7. 一旦`bookmarks`键被更新，你应该将`#name`和`#url`的值重置为空字符串，然后运行你的函数来隐藏表单部分并显示主部分。
8. 你应该有一个名为`displayOrHideCategory`的函数，它切换`#main-section`和`#bookmark-list-section`的`hidden`类。
9. 当你点击`#view-category-button`时，你应该将`.category-name`的内部文本更新为`#category-dropdown`中选定选项的值，根据下面的用户故事修改`#category-list`的内部HTML，并调用`displayOrHideCategory`函数。
10. 如果本地存储中的书签都没有该类别，你应该将`#category-list`的内部HTML设置为一个文本为`No Bookmarks Found`的`p`元素。
11. 如果本地存储中有一个或多个书签具有选定的类别，为每个书签向`#category-list`的innerHTML添加一个单选按钮。给每个单选按钮设置`id`和`value`为书签名称，并设置一个对所有按钮都相同的`name`。
12. 每个单选按钮都应该有一个对应的标签，包含一个锚元素，其中包含书签名称和设置为书签URL的`href`属性。
13. 当你点击`#close-list-button`时，你应该运行你的函数来隐藏`#bookmark-list-section`并显示主部分。
14. 当你点击`#delete-bookmark-button`时，你应该从本地存储中删除与选定单选按钮和适当类别对应的书签，并更新显示的书签列表。
...
你的`getBookmarks`函数应该返回一个数组。

```js
assert.isArray(getBookmarks());
```

你的`getBookmarks`函数应该返回`localStorage`中的`bookmarks`键。
...
当`localStorage`中未设置`bookmarks`键或为空数组时，`getBookmarks`函数应该返回一个空数组。
...
当`localStorage`中的`bookmarks`键不包含有效的书签对象数组时，`getBookmarks`函数应该返回一个空数组。
...
你的`displayOrCloseForm`函数应该切换`#main-section`和`#form-section`的`hidden`类。
...
当你点击`#add-bookmark-button`时，你应该将`.category-name`的内部文本更新为`#category-dropdown`中选定选项的值。
...
当你点击`#add-bookmark-button-form`时，你应该通过在数组末尾添加一个对象来更新存储在本地存储中的`bookmarks`键。添加的对象应该将`name`设置为`#name`输入的值，`category`设置为类别下拉菜单中选定选项的值，`url`设置为`#url`输入的值。
...
当你点击`#add-bookmark-button-form`时，你应该将`#name`和`#url`的值重置为空字符串。
...
你的`displayOrHideCategory`函数应该切换`#main-section`和`#bookmark-list-section`的`hidden`类。
...
当你点击`#view-category-button`时，你应该将`.category-name`的内部文本更新为`#category-dropdown`中选定选项的值。
...
当你点击`#view-category-button`时，如果本地存储中的书签都没有选定的类别，你应该向`#category-list`的内部HTML添加一个文本为`No Bookmarks Found`的`p`元素。
...
当你点击`#view-category-button`时，你应该通过添加一个单选按钮来修改`#category-list`元素的内部HTML。单选按钮应该为选定类别中的每个书签设置`id`和`value`属性为书签名称。此外，每个单选按钮应该具有相同的`name`属性。
...
添加到`#category-list`内部HTML的每个单选按钮都应该有一个对应的标签，包含一个锚元素，其中包含书签名称和设置为书签URL的`href`属性。
...
每个`label`元素都应该包含一个锚元素，其中包含书签名称作为文本，以及设置为书签URL的`href`属性。
...
当你点击`#close-list-button`时，你应该隐藏`#bookmark-list-section`并显示主部分。
...
当你点击`#close-list-button`然后打开任何类别时，`#category-list`应该只包含选定类别的相关数据，而不重复条目。
...
当你点击`#delete-bookmark-button`时，你应该从本地存储中删除与选定单选按钮和适当类别对应的书签，并更新显示的书签列表。
