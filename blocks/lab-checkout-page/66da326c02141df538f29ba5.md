...
在 `h1` 元素之后，你应该至少包含两个 `section` 元素。
...
在表单中，应包含一个输入元素，其 `id` 和 `name` 为 `card-name`。
...
在表单中，应包含一个输入元素，其 `id` 和 `name` 为 `card-number`。
...
所有非 `submit` `type` 的 `input` 元素，都应该有一个与之关联的 `label` 元素。
