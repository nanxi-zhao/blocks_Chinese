...
1. 你的 `CurrencyConverter` 组件应该渲染一个 `input` 元素来接受要转换的金额。
2. 你的 `input` 元素应该接受数字。
3. 你的 `CurrencyConverter` 组件应该渲染两个 `select` 元素来选择要转换的**来源**货币和**目标**货币。
4. 你的 `select` 元素应该至少包含 `USD`、`EUR`、`GBP` 和 `JPY` 的选项。你可以使用任何汇率，只要货币之间没有一对一的映射关系。以下是一些好的和坏的映射示例：
...
5. 你的 `CurrencyConverter` 组件应该记忆化计算**来源**货币的转换金额，这样更改**目标** `select` 选项不会重新计算转换金额。
6. 你的 `CurrencyConverter` 组件应该渲染一个元素，以 `XX.XX CCC` 的格式显示转换后的金额，其中 `XX.XX` 是转换后的金额，`CCC` 是货币代码。
7. 转换后的金额应该四舍五入到小数点后两位。
...
`select` 元素应该至少有 `USD`、`EUR`、`GBP` 和 `JPY` 的选项。
...
更改第一个 `select` 元素的值应该导致转换金额被重新计算。
...
更改第二个 `select` 元素的值不应该导致转换金额被重新计算。
...
更改第一个 `select` 元素的值应该导致页面上的文本发生变化。
...
更改第二个 `select` 元素的值应该导致页面上的文本发生变化。
...
转换后的金额应该以 `XX.XX CCC` 的格式显示，其中 `XX.XX` 是四舍五入到小数点后两位的转换金额，`CCC` 是货币代码。
