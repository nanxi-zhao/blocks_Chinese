...
2. 在 `#drum-machine` 元素内部，你应该有另一个 `div` 元素，其 `id` 为 `pad-bank`。

3. 在 `#drum-machine` 元素内部，你应该有一个 `p` 元素，其 `id` 为 `display`。

4. 在你的 `#pad-bank` 元素内部，你应该有九个可点击的鼓垫元素，每个元素都有一个 `drum-pad` 类，一个唯一的 `id` 来描述鼓垫将触发的音频片段，以及与键盘上以下键对应的内部文本：`Q`、`W`、`E`、`A`、`S`、`D`、`Z`、`X`、`C`。鼓垫必须按此顺序排列。

5. 每个 `.drum-pad` 应该有一个 `audio` 元素，该元素有一个 `clip` 类，一个指向音频片段的 `src` 属性，以及一个与其父 `.drum-pad` 元素的内部文本对应的 `id`（例如 `id="Q"`、`id="W"`、`id="E"` 等）。

6. 当你点击 `.drum-pad` 元素时，应触发其子音频元素中包含的音频片段。

7. 当你按下与每个 `.drum-pad` 关联的触发键时，应触发其子音频元素中包含的音频片段（例如，按下 `Q` 键应触发包含字符串 `Q` 的鼓垫，按下 `W` 键应触发包含字符串 `W` 的鼓垫，等等）。

8. 当触发 `.drum-pad` 时，你应该在 `#display` 元素的内部文本中显示一个描述相关音频片段的字符串（每个字符串必须是唯一的）。

你可以为鼓机使用的音频样本可以在 `https://cdn.MasterPuti.org/curriculum/drum/<fileName>` 找到，其中 `<fileName>` 如下：
...
在 `#drum-machine` 元素内部，你应该有另一个 `div` 元素，其 `id` 为 `pad-bank`。

```js
assert.isNotNull(document.querySelector('#drum-machine div#pad-bank'));
```

在 `#drum-machine` 元素内部，你应该有一个 `p` 元素，其 `id` 为 `display`。

```js
assert.isNotNull(document.querySelector('#drum-machine p#display'));
```

在你的 `#pad-bank` 元素内部，你应该有九个可点击的鼓垫元素，每个元素都有一个 `drum-pad` 类。

```js
assert.lengthOf(document.querySelectorAll('#pad-bank .drum-pad'), 9);
```

每个 `.drum-pad` 应该按顺序具有以下字母之一作为 `innerText`：`Q`、`W`、`E`、`A`、`S`、`D`、`Z`、`X`、`C`。
...
每个 `.drum-pad` 应该有一个 `audio` 元素，该元素有一个 `clip` 类，一个指向音频片段的 `src` 属性，以及一个与其父 `.drum-pad` 元素的内部文本对应的 `id`（例如 `id="Q"`、`id="W"`、`id="E"` 等）。
...
当你点击 `.drum-pad` 元素时，应触发其子音频元素中包含的音频片段。
...
当你按下键盘上的 Q、W、E、A、S、D、Z、X、C 键之一时，相应的 `audio` 元素应该播放相应的声音。
...
当触发 `.drum-pad` 时，你应该在 `#display` 元素的内部文本中显示一个描述相关音频片段的字符串（每个字符串必须是唯一的）。
...
const bankOne = [
  {
    ...
    keyTrigger: 'Q',
    id: 'Heater-1',
    url: 'https://cdn.MasterPuti.org/curriculum/drum/Heater-1.mp3'
  },
  {
    keyCode: 87,
    keyTrigger: 'W',
    id: 'Heater-2',
    url: 'https://cdn.MasterPuti.org/curriculum/drum/Heater-2.mp3'
  },
  {
    keyCode: 69,
    keyTrigger: 'E',
    id: 'Heater-3',
    url: 'https://cdn.MasterPuti.org/curriculum/drum/Heater-3.mp3'
  },
  {
    keyCode: 65,
    keyTrigger: 'A',
    id: 'Heater-4',
    url: 'https://cdn.MasterPuti.org/curriculum/drum/Heater-4_1.mp3'
  },
  {
    keyCode: 83,
    keyTrigger: 'S',
    id: 'Clap',
    url: 'https://cdn.MasterPuti.org/curriculum/drum/Heater-6.mp3'
  },
  {
    keyCode: 68,
    keyTrigger: 'D',
    id: 'Open-HH',
    url: 'https://cdn.MasterPuti.org/curriculum/drum/Dsc_Oh.mp3'
  },
  {
    keyCode: 90,
    keyTrigger: 'Z',
    id: "Kick-n'-Hat",
    url: 'https://cdn.MasterPuti.org/curriculum/drum/Kick_n_Hat.mp3'
  },
  {
    keyCode: 88,
    keyTrigger: 'X',
    id: 'Kick',
    url: 'https://cdn.MasterPuti.org/curriculum/drum/RP4_KICK_1.mp3'
  },
  {
    keyCode: 67,
    keyTrigger: 'C',
    id: 'Closed-HH',
    url: 'https://cdn.MasterPuti.org/curriculum/drum/Cev_H2.mp3'
  }
];
...
const bankTwo = [
  {
    ...
    keyTrigger: 'Q',
    id: 'Chord-1',
    url: 'https://cdn.MasterPuti.org/curriculum/drum/Chord_1.mp3'
  },
  {
    keyCode: 87,
    keyTrigger: 'W',
    id: 'Chord-2',
    url: 'https://cdn.MasterPuti.org/curriculum/drum/Chord_2.mp3'
  },
  {
    keyCode: 69,
    keyTrigger: 'E',
    id: 'Chord-3',
    url: 'https://cdn.MasterPuti.org/curriculum/drum/Chord_3.mp3'
  },
  {
    keyCode: 65,
    keyTrigger: 'A',
    id: 'Shaker',
    url: 'https://cdn.MasterPuti.org/curriculum/drum/Give_us_a_light.mp3'
  },
  {
    keyCode: 83,
    keyTrigger: 'S',
    id: 'Open-HH',
    url: 'https://cdn.MasterPuti.org/curriculum/drum/Dry_Ohh.mp3'
  },
  {
    keyCode: 68,
    keyTrigger: 'D',
    id: 'Closed-HH',
    url: 'https://cdn.MasterPuti.org/curriculum/drum/Bld_H1.mp3'
  },
  {
    keyCode: 90,
    keyTrigger: 'Z',
    id: 'Punchy-Kick',
    url: 'https://cdn.MasterPuti.org/curriculum/drum/punchy_kick_1.mp3'
  },
  {
    keyCode: 88,
    keyTrigger: 'X',
    id: 'Side-Stick',
    url: 'https://cdn.MasterPuti.org/curriculum/drum/side_stick_1.mp3'
  },
  {
    keyCode: 67,
    keyTrigger: 'C',
    id: 'Snare',
    url: 'https://cdn.MasterPuti.org/curriculum/drum/Brk_Snr.mp3'
  }
];
