...
在本实验中，你将利用语义化 HTML 元素创建网页结构。你将添加内容和图像，使其看起来像一个真正的活动中心。
...
1. 每个链接都应由一个带有 `href` 属性的 `a` 元素表示，而元素链接到页面的相应部分，分别是 `#upcoming-events` 和 `#past-events`。
...
1. 在 `#past-events` 部分中，应该有：
   ...
   - 两个 `article` 元素。每个文章元素都应代表一个已结束的活动，而且应有：
     - 一个用于活动标题的 `h3` 元素。
     - 一个用于活动描述的 `p` 元素。你还可以在底部添加日期。
...
`#upcoming-events` 部分中的两个 `article` 元素都应有一个 `p` 元素用于活动描述。
...
`#past-events` 部分中的两个 `article` 元素都应有一个 `p` 元素用于活动描述。
