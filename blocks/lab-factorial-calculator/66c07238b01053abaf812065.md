...
阶乘是一个整数与其下面所有整数的乘积。例如，`5` 的阶乘是 `5 * 4 * 3 * 2 * 1 = 120`。在这个实验中，你将创建一个阶乘计算器，从用户那里获取一个数字并计算该数字的阶乘。

**目标：** 实现以下用户需求并通过所有测试以完成实验。

**用户需求：**

1. 你应该声明一个变量 `num` 并为其分配一个你选择的数字。分配的数字应该在 `1` 到 `20` 之间（包括 `1` 和 `20`）。
1. 创建一个名为 `factorialCalculator` 的函数，该函数接受一个数字作为参数并返回该数字的阶乘。
1. 在函数内部，声明一个 `result` 变量并将其赋值为 `1`。使用循环，遍历从 `1` 到输入数字（包括输入数字）的所有数字，对于每个数字，将 `result` 变量乘以当前数字并将结果赋值给 `result` 变量。你可以选择使用 `for` 循环、`while` 循环或 `do...while` 循环。
1. 你应该使用 `num` 作为参数调用 `factorialCalculator` 函数，并将结果赋值给变量 `factorial`。
1. 你应该以 `Factorial of [num] is [factorial]` 的格式存储最终输出，并将其赋值给变量 `resultMsg`。
...
`factorialCalculator` 函数应该接受一个数字作为参数。
...
你应该使用 `num` 变量作为参数调用 `factorialCalculator` 函数。
...
你的 `resultMsg` 应该包含格式为 `Factorial of [num] is [factorial]` 的字符串。
