...
在这个实验中，你将使用HTML创建房屋的结构。然后，你将使用CSS定位来安排房屋的元素，如窗户和门。
...
1. 你的`#house`应该将`position`设置为`relative`，这样它的子元素就可以相对于它进行定位。
1. 你的`#house`应该有`500px`的宽度和`400px`的高度。不要设置最小或最大值。
1. 你的`#house`应该有背景颜色和边框设置。
1. 你应该在`#house`内有五个`div`元素，它们的`id`值分别为：`chimney`、`roof`、`window-1`、`window-2`和`door`。
1. `#house`的所有直接子元素都应该有`absolute`的`position`。
1. `#roof`、`#chimney`、`#window-1`、`#window-2`和`#door`都应该设置宽度、高度、边框和背景颜色。
1. 你的`#roof`应该有`0`的top值。
1. 你的`#door`应该放置在房屋的底部。
1. 你的窗户应该放置在`#roof`下方，并且至少高于`#door`高度的三分之一。
1. 你的窗户和门都应该设置`left`或`right`的值，将它们放置在房屋边界内。
1. 你的`#chimney`应该有top值，将其放置在`#house`的顶部。
1. 你的`#chimney`应该有z-index值，将其放置在房屋后面。
...
你应该定位`#window-1`并设置其`background-color`属性。
...
你应该定位`#window-2`并设置其`background-color`属性。
...
你应该在`#door`上设置`left`或`right`，并将其安排在房屋边界内。
...
你的`#chimney`应该有top值，将其放置在`#house`的顶部。
...
你的`#chimney`应该有z-index值，将其放置在房屋后面。
...
你的窗户应该放置在`#roof`下方，并且至少高于`#door`高度的三分之一。
...
你应该在窗户上设置`left`或`right`，并将其安排在房屋边界内。
