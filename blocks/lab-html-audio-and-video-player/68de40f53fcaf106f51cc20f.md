...
在第一个 `section` 元素内，你应该有一个 `h2` 元素作为视频播放的标题。
...
`video` 元素应该有一个设置为 `640` 的 `width` 属性。
...
在第二个 `section` 元素内，你应该有一个 `h2` 元素作为歌曲播放的标题。
