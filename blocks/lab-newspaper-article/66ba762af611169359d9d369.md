...
1. 你应该将 HTML 文档的根 `font-size` 设置为 `24px`。
...
你应该有一个 `.newspaper` 选择器，将其元素 `font-size` 设置为 `16px`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.newspaper')?.fontSize, '16px');
```

你应该有一个 `.newspaper` 选择器，将其元素 `font-family` 设置为 `'Open Sans', sans-serif`。
...
你应该有一个 `.name` 选择器，将其元素 `font-size` 设置为 `2rem`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.name')?.fontSize, '2rem');
```

你应该有一个 `.name` 选择器，将其元素 `font-family` 设置为 `'Times New Roman', serif`。
