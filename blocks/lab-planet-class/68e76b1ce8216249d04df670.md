...
1. `Planet`类应该有一个`__init__`方法，该方法：
   - 有四个参数：`self`、`name`、`planet_type`和`star`。
   - 如果传入的任何参数不是字符串类型，则引发带有消息`name, planet type, and star must be strings`的`TypeError`。
   - 如果传入的任何参数是空字符串，则引发带有消息`name, planet_type, and star must be non-empty strings`的`ValueError`。
   - 将传入的值分配给实例属性`name`、`planet_type`和`star`。
1. `Planet`类应该有一个`orbit`方法，该方法返回格式为`{name} is orbiting around {star}...`的字符串。
1. `Planet`类应该有一个`__str__`方法，该方法返回格式为`Planet: {name} | Type: {planet_type} | Star: {star}`的字符串。
1. 你应该创建三个名为`planet_1`、`planet_2`和`planet_3`的`Planet`类实例。
1. 你应该打印每个行星对象以查看`__str__`方法的输出。
1. 你应该调用每个行星对象的`orbit`方法并打印结果。
...
`__init__`方法应该有四个参数：`self`、`name`、`planet_type`和`star`，按此顺序。
...
如果传入的第一个参数不是字符串类型，`__init__`方法应该引发带有消息`name, planet type, and star must be strings`的`TypeError`。
...
如果传入的第二个参数不是字符串类型，`__init__`方法应该引发带有消息`name, planet type, and star must be strings`的`TypeError`。
...
如果传入的第三个参数不是字符串类型，`__init__`方法应该引发带有消息`name, planet type, and star must be strings`的`TypeError`。
...
如果传入的第一个参数是空字符串，`__init__`方法应该引发带有消息`name, planet_type, and star must be non-empty strings`的`ValueError`。
...
如果传入的第二个参数是空字符串，`__init__`方法应该引发带有消息`name, planet_type, and star must be non-empty strings`的`ValueError`。
...
如果传入的第三个参数是空字符串，`__init__`方法应该引发带有消息`name, planet_type, and star must be non-empty strings`的`ValueError`。
...
你应该在`__init__`方法中将`planet_type`分配给`self.planet_type`。
...
你应该在`__init__`方法中将`star`分配给`self.star`。
...
`orbit`方法应该返回格式为`{name} is orbiting around {star}...`的字符串。
...
`__str__`方法应该返回格式为`Planet: {name} | Type: {planet_type} | Star: {star}`的字符串。
...
print(planet_1.orbit()) # 地球围绕太阳运行...
print(planet_1) # 行星: 地球 | 类型: 类地行星 | 恒星: 太阳

print(planet_2.orbit()) # 木星围绕太阳运行...
print(planet_2) # 行星: 木星 | 类型: 气体巨星 | 恒星: 太阳

print(planet_3.orbit()) # Kepler-452b围绕Kepler-452运行...
print(planet_3) # 行星: Kepler-452b | 类型: 超级地球 | 恒星: Kepler-452
```