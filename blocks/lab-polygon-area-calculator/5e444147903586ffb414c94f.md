...
1. 您应该创建一个 `Rectangle` 类。

2. 当创建 `Rectangle` 对象时，它应该用 `width` 和 `height` 属性进行初始化。该类还应包含以下方法：

    - `set_width`：设置矩形的宽度。
    - `set_height`：设置矩形的高度。
    - `get_area`：返回面积（\\( \text{width} \times \text{height} \\)）。
    - `get_perimeter`：返回周长 \\( 2(\text{width} + \text{height}) \\)。
    - `get_diagonal`：返回对角线 \\( \sqrt{\text{width}^2 + \text{height}^2} \\)。
    - `get_picture`：返回一个使用 `*` 行表示形状的字符串。行数应该等于高度，每行中的 `*` 数量应该等于宽度。每行末尾应该有换行符（`\n`）。如果宽度或高度大于 `50`，则应返回字符串：`Too big for picture.`。
    - `get_amount_inside`：接受另一个形状（正方形或矩形）作为参数。返回传入的形状可以 fitting 在形状内的次数（不旋转）。例如，一个宽度为 `4`、高度为 `8` 的矩形可以 fitting 两个边长为 `4` 的正方形。

3. 如果 `Rectangle` 的实例表示为字符串，它应该看起来像：`Rectangle(width=5, height=10)`。

4. 您应该创建一个继承 `Rectangle` 的 `Square` 类。
5. 当创建 `Square` 对象时，它应该用单个边长进行初始化。`__init__` 方法应该将边长存储在 `Rectangle` 类的 `width` 和 `height` 属性中。
6. `Square` 类应该包含以下方法：

    - `set_width`：覆盖 `Rectangle` 类的 `set_width` 方法。它应该将宽度和高度设置为边长。
    - `set_height`：覆盖 `Rectangle` 类的 `set_height` 方法。它应该将宽度和高度设置为边长。
    - `set_side`：将正方形的高度和宽度设置为等于边长。

7. `Square` 类应该能够访问 `Rectangle` 类的方法。
8. 如果 `Square` 的实例表示为字符串，它应该看起来像：`Square(side=9)`。
...
正方形对象应该是 `Square` 类和 `Rectangle` 类的实例。
...
`Rectangle(3, 6)` 的字符串表示应该是 `Rectangle(width=3, height=6)`。
...
`Square(5).get_diagonal()` 应该返回 `7.0710678118654755`。
...
`Rectangle` 类的实例在设置新值后应该有不同的字符串表示。
...
`Square` 类的实例在使用 `.set_side()` 设置新值后应该有不同的字符串表示。
...
`Square` 类的实例在使用 `.set_width()` 或 `set_height()` 设置新值后应该有不同的字符串表示。
...
`.get_picture()` 方法应该返回 `Rectangle` 实例的不同字符串表示。
...
`.get_picture()` 方法应该返回 `Square` 实例的不同字符串表示。
...
如果 `width` 或 `height` 属性大于 `50`，`.get_picture()` 方法应该返回字符串 `Too big for picture.`。
