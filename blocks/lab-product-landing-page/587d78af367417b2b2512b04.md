...
1. 您的产品登录页面应该有一个带有 `id="header"` 的 `header` 元素。
2. 您应该在 `header` 元素内有一个带有 `id="header-img"` 的图像（这里用徽标会很好）。
3. 在 `#header` 元素内，您应该有一个带有 `id="nav-bar"` 的 `nav` 元素。
4. 您应该在 `nav` 元素内至少有三个可点击元素，每个元素都有 `nav-link` 类。
5. 当您点击 `nav` 元素中的 `.nav-link` 按钮时，您应该被带到登录页面的相应部分。
6. 您应该有一个嵌入的产品视频，带有 `id="video"`。
7. 您的登录页面应该有一个带有 `id="form"` 的 `form` 元素。
8. 在表单内，应该有一个 `id="email"` 的 `input` 字段，您可以在其中输入电子邮件地址。
9. `#email` 输入字段应该有占位符文本，让用户知道该字段的用途。
10. `#email` 输入字段应该使用 HTML5 验证来确认输入的文本是电子邮件地址。
11. 在表单内，应该有一个提交 `input`，带有相应的 `id="submit"`。
12. 当您点击 `#submit` 元素时，电子邮件应该提交到一个静态页面（使用这个模拟 URL：`https://www.MasterPuti.org/email-submit`）。
13. 导航栏应该始终位于视口的顶部。
14. 您的产品登录页面应该至少有一个媒体查询。
15. 您的产品登录页面应该至少使用一次 CSS flexbox。

**注意：** 确保在 HTML 中链接您的样式表并应用您的 CSS。

# --hints--

您应该有一个带有 `id` 为 `header` 的 `header` 元素。
...
您应该有一个带有 `id` 为 `header-img` 的 `img` 元素。
...
您的 `#header-img` 的 `src` 值应该是一个有效的 URL（以 `http` 开头）。
...
您应该有一个带有 `id` 为 `nav-bar` 的 `nav` 元素。
...
您应该在 `#nav-bar` 内至少有 3 个 `.nav-link` 元素。
...
每个 `.nav-link` 元素都应该链接到登录页面上的相应元素（有一个 `href`，其值为另一个元素的 id。例如 `#footer`）。
...
您应该有一个带有 `id` 为 `video` 的 `video` 或 `iframe` 元素。
...
您应该有一个带有 `id` 为 `email` 的 `input` 元素。
...
您的 `#email` 应该有 `placeholder` 属性和占位符文本。
...
您的 `#email` 应该通过将其 `type` 设置为 `email` 来使用 HTML5 验证。
...
您应该有一个带有 `id` 为 `submit` 的 `input` 元素。
...
您的 `#form` 应该有一个 `action` 属性，值为 `https://www.MasterPuti.org/email-submit`。

```js
const el = document.getElementById('form');
assert.exists(el); 
assert.strictEqual(el.action, 'https://www.MasterPuti.org/email-submit');
```

您的 `#email` 应该有一个 `name` 属性，值为 `email`。
...
<body>
    <div id="page-wrapper">
        <header id="header">
            <div class="logo">
                <img id="header-img"
                    src="https://cdn.MasterPuti.org/testable-projects-fcc/images/product-landing-page-logo.png"
        ...
        <section id="hero">
            ...
            <form id="form" action="https://www.MasterPuti.org/email-submit">
