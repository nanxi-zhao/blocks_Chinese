...
1. 你应该有一个 `form` 元素，其 `id` 为 `survey-form`。
1. 在表单元素中，你应该有一个输入姓名的必填 `input` 字段，其 `id` 为 `name`，`type` 为 `text`。
1. 在表单元素中，你应该有一个输入电子邮箱的必填 `input` 字段，其 `id` 为 `email`。
1. 如果你输入了格式不正确的邮箱，你会看到 HTML5 验证错误信息。
1. 在表单元素中，你应该有一个 `input` 字段，用于输入数字，其 `id` 为 `number`。
1. 数字输入不应接受非数字，或是阻止你输入它们，或是显示一个 HTML5 验证错误（取决于你的浏览器）。
1. 如果你输入的数字超出了范围（使用 `min` 和 `max` 属性定义），你会看到 HTML5 验证错误信息。
1. 表单中的姓名、邮箱和数字输入框应有相应的包含描述输入框用途的 `label` 元素，id 应分别为 `name-label`、`email-label` 和 `number-label`。
1. 在表单中的姓名、邮箱和数字输入框中，应看到各自描述文字作为占位符。
1. 在表单元素内，你应该有一个 `select` 下拉元素，其 `id` 为 `dropdown`，它至少有两个选项。
1. 在表单元素内部，你可以从一组至少包含两个单选按钮的选项中进行选择，这些按钮通过 `name` 属性进行分组。
1. 在表单元素内部，你可以从一系列复选框中选择多个选项，每个选项都必须具有 `value` 属性。
...
你的 `#dropdown` 应该至少有两个可选择的（非禁用）`option` 元素。
...
你在 `#survey-form` 中的所有复选框都应有 `value` 属性和值。
