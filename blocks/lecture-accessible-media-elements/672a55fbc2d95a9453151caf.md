...
`tabindex="0"` 将元素添加到自然 tab 顺序中。

按 Tab 键时，焦点会按 HTML 顺序依次从 `button` 到 `div`，再到链接。

在预览窗口的空白处单击任意位置。然后使用 `Tab` 键查看焦点在不同元素之间的移动。
...
`tabindex="-1"` 使元素可以通过编程方式获得焦点。这对于管理通常无法获得焦点的元素（如标题、容器、对话框或错误消息）的焦点非常有用：

```html
<p tabindex="-1">Sorry, there was an error with your submission.</p>
```

在此示例中，该段落不属于正常制表符顺序的一部分，因此用户按下 `Tab` 键后无法到达该段落。但是，如果你通过脚本将焦点设置为该元素，该信息就会引起用户的注意。你将在 JavaScript 课程中了解更多关于这种技术的知识。

当 `tabindex` 大于 `0` 时，它会设置自定义 tab 顺序。因此，具有较低正值的元素会首先获得焦点。
...
`accesskey` 是另一个属性，你可以使用它来使你的网络项目具有键盘无障碍。它允许你定义一个按键来聚焦或激活特定元素。

下面是一个交互式示例，你可以按照以下建议进行尝试：
...
下面是为 `button` 元素设置焦点状态样式的示例。在预览窗口的空白处单击任意位置，然后按 `Tab` 键来聚焦按钮。
