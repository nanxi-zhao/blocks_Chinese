...
让我们看几个例子来了解它是如何工作的。在第一个示例中，我们有一个用于输入密码的 `form` 表单。

在密码输入框中输入几个字符，你会在预览窗口看到密码被屏蔽显示；在输入长度少于 8 个字符时，`password-help` 文本会保持红色，直到满足长度要求颜色才会变更。

注意：此互动示例使用 CSS 和 JavaScript 动态更新 `password-help` 文本的颜色。不必担心理解 JavaScript 代码，后续模块会讲到 JavaScript。
...
下面哪一种是在表单中正确使用 `aria-describedby` 属性的方式？
