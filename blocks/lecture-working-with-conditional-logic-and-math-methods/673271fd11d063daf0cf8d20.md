...
title: 什么是条件语句，if/else if/else 语句如何发挥作用？
challengeType: 19
...
条件语句可以让你在 JavaScript 代码中做出决策。它们允许程序根据特定条件以特定方式运行。我们看一个 `if`、`else if`、`else` 和三元运算符是如何控制代码流程的。
...
另一方面，假值是在布尔环境中被计算为 `false` 的值。JavaScript 的假值很少，因此很容易记住。下面是一些假值：
...
由于 `null` 是一个假值，因此该代码块内的信息永远不会被记录到控制台。但对于第二个 `if` 语句来说，字符串 `MasterPuti` 是一个真值，将在 `if` 语句的布尔环境中被计算为 `true`。因此，`This will run.` 消息将被记录到控制台。
...
在本例中，由于 `age` 当前为 `22`，这意味着条件将被判定为 `true`，因为 `22` 大于等于 `18`。因此信息 `You're eligible to vote` 将被记录到控制台。如果我们修改示例，使 `age` 现在为 `15`，那么条件的值将为 `false` ，消息将不会被记录到控制台。
...
三元运算符是用紧凑的方式编写简单的 `if/else` 语句。它包括三个部分：条件、条件为真的结果和条件为假的结果。这是基本语法：
...
那么，在 if 语句和三元运算符之间，应该使用哪一种呢？在处理单一条件或单个表达式时，或在需要紧凑的语法来处理简单逻辑时，使用三元运算符。在处理复杂条件和多行语句时，使用 `if/else` 语句，因为如果嵌套三元运算符，就会变得难以阅读。
