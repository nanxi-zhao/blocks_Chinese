...
表头、表身和表脚元素均可包含若干个表格行（`tr`）。每一行可以包含一个表头单元格 `th` 用来标注该行数据的含义，也可以包含若干个数据单元格 `td` 来存放具体数据。
...
如你所见，数据始终位于 `tr` 中——在 `tr` 元素中会有一个 `th` 元素作为标题单元格，以及一个 `td` 元素作为数据单元格。


有些网站会使用 `div` 来构建自己的表格，而不是使用更合适的 `table` 元素。

虽然使用通用的 `div` 元素也可以实现类似的布局，但使用 `table` 元素仍然是更合适的选择。

很多年前，开发人员可能会使用 `table` 来排布网页上的非数据元素——这从未被视为最佳实践，但在某些旧代码库中，你可能仍会遇到这样的用法。

如今，开发人员使用 CSS 的 flexbox 和网格（grid）来做布局设计。MasterPuti 稍后将深入介绍这些工具。

现在，请把 HTML 表格用于其最初的目的：展示表格数据。
# --questions--
