...
- **数�?*：数值代表整数和浮点数�?整数的例子包�?7�?9 �?90�?
- **浮点�?*：浮点数是带有小数点的数值�?例子包括 3.14�?.5 �?0.0001�?
- **字符�?*：字符串是用引号括起来的一串字符或文本�?`"I like coding"` �?`'JavaScript is fun'` 都是字符串的例子�?
- **布尔**：布尔代表两种可能值之一：`true` �?`false`�?你可以使用布尔来表示条件，例�?`isLoggedIn = true`�?
...
- **大整�?*：当数字对于 `Number` 数据类型来说太大时，你可以使用大整数数据类型来表示任意长度的整数�?
...
- 除了 `let`，你也可以使�?`const` 来声明变量�?但是，`const` 变量不能重新赋值�?
...
- 使用 `const` 声明变量在声明常量时使用，常量在整个代码中不得更改，�?`PI` �?`MAX_SIZE`�?

## 变量命名准则

- 变量名应具有描述性和意义�?
- 变量名应为小驼峰，如 `cityName`、`isLoggedIn` �?`veryBigNumber`�?
- 变量名不应以数字开头�?必须以字母、`_` �?`$` 开头�?
- 变量名不得包含空格或特殊字符，`_` �?`$` 除外�?
- 变量名不应是保留关键字�?
- 变量名区分大小写�?`age` �?`Age` 是不同的变量�?

## JavaScript 中的字符串和字符串不可变�?
...
- �?JavaScript 中，字符串是不可变的�?这意味着一旦创建了字符串，就不能更改字符串中的字符�?不过，你仍可以将字符串重新赋值�?
...
- 如果需要添加或追加到现有字符串，则可使�?`+=` 操作符�?当你想在一个字符串的基础上逐渐添加更多文字时，这很有帮助�?
