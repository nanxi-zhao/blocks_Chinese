...
在每个 `div.question-block` 内添加一个 `label`，为后续的下拉选择与文本域提供题干（与 CSS 相关的问题描述）。继续，题目语义补全！

# --hints--

你需要在第一个 `div.question-block` 中添加一个 `label`。

```js
assert.exists(document.querySelectorAll('.formrow > .question-block')?.[0]?.querySelector('label'));
```

你需要在第二个 `div.question-block` 中添加一个 `label`。
