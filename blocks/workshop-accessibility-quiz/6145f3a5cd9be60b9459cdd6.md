...
为第一题的 `label` 与下拉框建立显式关联：添加匹配的 `for` 与 `id`；同时给该 `select` 增加描述性的 `name` 属性用于提交。继续，表单语义更完整！
