...
让导航更像按钮：去掉链接的下划线，并为列表项添加悬停效果——背景与文字颜色对调，指针变为 `pointer`。这样用户更直观感知可点击区域。继续，交互反馈更友好！

# --hints--

你需要用 `li > a` 去除下划线 (`text-decoration: none`)。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('li > a')?.textDecoration, 'none');
```

你需要用 `nav > ul > li:hover` 或 `nav li:hover` 添加悬停样式。
