...
在窄屏下主标题内容显得局促，需要让 `h1` 文本居中；同时给 `main` 顶部增加足够内边距，确保 `Student Info` 标题不被遮挡。

先居中 `h1`，再只增加 `main` 的 `padding-top`（至少 25px）。继续，移动端体验更棒！

# --hints--

你需要让 `h1` 的 `text-align` 为 `center`。
...
你需要给 `main` 设置至少 `25px` 的 `padding-top`。
