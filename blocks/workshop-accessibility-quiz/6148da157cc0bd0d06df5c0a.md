...
为了让输入框和标签整齐对齐，在 `.info` 中为全部 `label` 与 `input` 建立一套新规则：设定 `display: inline-block`。然后把 `label` 的文字右对齐，这样标签靠右、输入框紧随其后。排版进步，继续闪闪发光！

# --hints--

你应该使用 `.info > label, .info > input` 或 `.info label, .info input` 这样的组合选择器。
...
你应该给这些 `label` 与 `input` 设置 `display: inline-block`。
...
你应该给所有 `.info` 内的 `label` 设置 `text-align: right`。
