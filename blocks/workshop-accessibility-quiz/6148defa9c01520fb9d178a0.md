...
`ul`/`li` 很适合展示项目符号，但这里的单选按钮不需要默认小点。你可以用 `list-style` 控制符号，比如：
...
现在请去掉 `.answers-list` 的默认样式（设为 `none`），并移除该列表的内边距，让界面更清爽。继续加速变美！
