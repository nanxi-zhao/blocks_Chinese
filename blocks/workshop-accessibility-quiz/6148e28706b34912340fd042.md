...
让 `address` 中的所有文字水平居中，并为它添加适量的内边距。脚注信息就会更整齐亲切。继续，你已经快完成整套样式啦！
...
你应该让 `address` 的 `padding-right` 不小于 `1px`。
...
你应该让 `address` 的 `padding-bottom` 不小于 `1px`。
...
你应该让 `address` 的 `padding-left` 不小于 `1px`。
