...
最后一步：某些基于运动的动画会让一部分用户不适，尤其是有前庭（<dfn>vestibular</dfn>）相关敏感的用户。

`@media` 里有一个媒体特性 `prefers-reduced-motion`，可依据用户的动画偏好应用不同样式，取值包括：
...
你将来会进一步深入这个特性。现在，请把原先设置 `scroll-behavior: smooth` 的那条通配选择器规则包裹在 `@media (prefers-reduced-motion: no-preference)` 里，仅在用户未声明“减少动画”时使用平滑滚动。

太棒啦！你已经完成了整个 Accessibility Quiz 练习项目！撒花庆祝～
