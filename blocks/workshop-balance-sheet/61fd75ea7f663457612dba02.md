...
在每个新的 `th` 中添加一个 `span`，并把它的 `class` 设置为 `sr-only year`。按顺序给这些 `span` 填入文本：`2019`、`2020`、`2021`。

同时给第三个 `th` 添加 `class="current"`。保留 `td` 元素为空 —— 它只是用来确保表格是四列的并将表头与正确的列对齐。做得很好，继续完成下一步！
