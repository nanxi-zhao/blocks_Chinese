...
在第二个表格的 `tbody` 中添加四个 `tr` 行：前三个设置 `class="data"`，第四个设置 `class="total"`，用于区分数据行与汇总行。很棒，继续加油！

# --hints--

提示：确认第二个表格的 `tbody` 包含 4 个 `tr` 元素。
