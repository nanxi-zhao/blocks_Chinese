...
在第二个 `tr` 中，添加一个 `th` 元素，文本为 `Expenses Annual anticipated expenses, such as payroll.`。请把除 `Expenses` 之外的说明文字包裹在 `class="description"` 的 `span` 中。

接着添加三个 `td`，按顺序填入 `$200`、`$300`、`$400`，并将第三个 `td` 的 `class` 设置为 `current`。继续保持，你做得很好！
...
提示：`th` 的文本应为 `Expenses Annual anticipated expenses, such as payroll.`（请保留原文以通过测试）。
...
提示：把说明文字 `Annual anticipated expenses, such as payroll.` 包裹在 `span` 中，并设置 `class="description"`。
...
提示：该行应包含 3 个 `td` 元素，分别填入 `$200`、`$300` 和 `$400`，并把第三个 `td` 的 `class` 设为 `current`。
