...
现在处理第三个表格：为它添加一个 `caption`（文本为 `Net Worth`），并设置好 `thead` 与 `tbody`。你做得很棒，继续保持！

# --hints--

提示：第三个 `table` 应包含 `caption`。
...
提示：`caption` 的文本应为 `Net Worth`（保持原文以通过测试）。
...
提示：第三个 `table` 应包含 `thead` 和 `tbody`。
