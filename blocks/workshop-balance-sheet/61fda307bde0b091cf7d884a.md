...
创建一个 `body` 选择器，并将其 `font-family` 设置为 `sans-serif`，`color` 设置为 `#0a0a23`。继续很棒的进度！
...
提示：`body` 选择器的 `font-family` 应设置为 `sans-serif`。
...
提示：`body` 选择器的 `color` 属性应设置为 `#0a0a23`（测试以 RGB 形式验证）。
