...
为你的 flex 容器添加 `h1 .flex` 选择器。设置 `display: flex` 启用 flex 布局，接着把 `flex-direction` 设为 `column-reverse`（这样嵌套元素会从下往上显示），最后设置 `gap: 1rem` 让元素之间有间距。做得很好，继续加油！

# --hints--

你应该有一个 `h1 .flex` 选择器，棒极了，继续完善样式！
