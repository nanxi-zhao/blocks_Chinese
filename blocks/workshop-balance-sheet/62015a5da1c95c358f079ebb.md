...
`:first-of-type` 伪选择器用于选中第一个匹配的元素。创建一个 `h1 .flex span:first-of-type` 选择器来选中 `.flex` 容器内的第一个 `span`。记住你的 `span` 元素在视觉上是反序的，所以它看起来会像是页面上的第二个元素。

为这个选择器设置 `font-size: 0.7em`，让它看起来像一个副标题。干得漂亮，继续优化！

# --hints--
你应该有一个 `h1 .flex span:first-of-type` 选择器，加油，做得很棒！
