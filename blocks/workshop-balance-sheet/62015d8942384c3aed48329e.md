...
你已将表格包裹在 `section` 元素中——现在可以为它添加样式以显示边框。创建一个 `section` 选择器，设置 `max-width: 40rem` 以便响应式显示，设置 `margin: 0 auto` 居中，并添加 `border: 2px solid #d0d0d5`。做得很好，继续完善页面样式！

# --hints--
你应该有一个 `section` 选择器，很好，继续完善布局与样式！
