...
表格标题的最后一部分是年份。创建一个 `#years` 选择器并启用 flex 布局（flexbox），将内容对齐到主轴的末端（`justify-content: flex-end`），并将该元素设置为 `position: sticky`，用 `top: 0` 固定在容器顶部。做得很棒，继续保持！

# --hints--
你应该有一个 `#years` 选择器，做得很好，继续保持细节关注！
