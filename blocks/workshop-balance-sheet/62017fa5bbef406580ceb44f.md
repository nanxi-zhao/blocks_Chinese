...
既然你已为隐藏辅助文本添加了 `!important`，现在可以从 `span` 选择器中移除 `:not(.sr-only)`。做得很好，继续注意可访问性细节！

# --hints--

请确保不要使用 `span:not(.sr-only)` 选择器，已经很棒，继续保持！

```js
assert.isNull(new __helpers.CSSHelp(document).getStyle('span:not(.sr-only)'));
```

你应该有一个 `span` 选择器，继续加油！

```js
assert.isNotNull(new __helpers.CSSHelp(document).getStyle('span'));
```

不要更改 `font-weight`，请保持为 `normal`，做得很好！
