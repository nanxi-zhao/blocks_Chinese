...
`[attribute="value"]` 选择器用于匹配具有特定属性值的元素。创建一个 `tr[class="total"]` 选择器，用于选中 `class` 为 `total` 的 `tr`。为其设置下边框 `4px double #0a0a23` 并加粗字体，使小计行更显眼。继续加油！

# --hints--
你应该有一个 `tr[class="total"]` 选择器，做得很好，继续优化细节！
