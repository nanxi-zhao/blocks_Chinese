...
选择 `class` 为 `data` 的 `tr` 元素，并为它们设置背景图像：`linear-gradient(to bottom, #dfdfe2 1.845rem, white 1.845rem)`，这样可以使数据行的样式更易读。继续保持，很棒！

# --hints--
你应该有一个 `tr.data` 选择器，棒极了，继续保持！

```js
assert.isNotNull(new __helpers.CSSHelp(document).getStyle('tr.data'));
```
```js
