...
正如你在之前的课程中学到的，函数可以有参数，这些参数是在函数内可以引用的变量。下面是一个带有两个参数 `num1` 和 `num2` 的 `sum` 函数。
...
待加密的消息和移动位数仍然在你的函数中硬编码。给你的函数两个名为 `text` 和 `shift` 的参数，并从 `caesar` 函数体中删除 `text` 和 `shift` 变量的声明。

# --hints--

你的 `caesar` 函数应该有两个参数，`text` 和 `shift`。
