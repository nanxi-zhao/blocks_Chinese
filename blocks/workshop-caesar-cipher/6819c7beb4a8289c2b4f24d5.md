...
你的函数不应接受负数或零移动。因此，在你的第一个 `if` 语句之后，创建另一个 `if` 语句，检查 `shift` 是否小于 `1`，并返回字符串 `Shift must be a positive integer.`。

# --hints--

你应该有一个第二个 `if` 语句，检查 `shift` 是否小于 `1`。
...
你的第二个 `if` 语句应该返回 `Shift must be a positive integer.`。
