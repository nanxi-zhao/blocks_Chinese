...
你可以通过各种`padding`属性给菜单的内容和边缘之间留出一些空间。

给`menu`类添加值为`20px`的`padding-left`和`padding-right`。
...
你的`.menu`元素应该有`20px`的`padding-left`。
...
你的`.menu`元素应该有`20px`的`padding-right`。
