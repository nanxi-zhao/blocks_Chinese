...
字符串有一个 `.match()` 方法，用来接受一个正则参数并返回匹配结果的数组——当使用全局标志时会返回所有匹配项，否则返回第一个匹配。
...
请返回对 `str` 调用 `.match()` 并将你的 `regex` 变量作为参数的结果，之后会用到这个匹配结果。

# --hints--

你的 `isInvalidInput` 函数应在 `str` 上调用 `.match()` 方法。

```js
assert.match(isInvalidInput.toString(), /str\.match\(/);
```

请将 `regex` 作为参数传入 `.match()`。
