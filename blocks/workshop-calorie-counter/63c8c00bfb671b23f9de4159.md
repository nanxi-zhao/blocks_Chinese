...
还需要确认输入是否有效。请声明 `invalidInputMatch` 变量，并将调用 `isInvalidInput(currVal)` 的结果赋给它。

# --hints--

请声明 `invalidInputMatch` 变量。

```js
assert.match(getCaloriesFromInputs.toString(), /invalidInputMatch\s*=/);
```

将 `isInvalidInput` 的调用结果赋给 `invalidInputMatch`。

```js
assert.match(getCaloriesFromInputs.toString(), /invalidInputMatch\s*=\s*isInvalidInput\(/);
```

调用 `isInvalidInput` 时请传入 `currVal`。
