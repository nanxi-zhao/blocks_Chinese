...
现在声明一个 `lunchCalories` 变量，并将调用 `getCaloriesFromInputs(lunchNumberInputs)` 的结果赋给它。

# --hints--

在 `calculateCalories` 中声明 `lunchCalories` 变量。

```js
assert.match(calculateCalories.toString(), /lunchCalories\s*=/);
```

调用 `getCaloriesFromInputs(lunchNumberInputs)`。

```js
assert.match(calculateCalories.toString(), /getCaloriesFromInputs\s*\(\s*lunchNumberInputs\s*\)/);
```

将调用结果赋给 `lunchCalories`。
