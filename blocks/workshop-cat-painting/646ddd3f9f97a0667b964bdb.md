...
创建两个[div]元素，一个带有[cat-left-inner-eye]类的元素放在[.cat-left-eye]元素内，另一个带有[cat-right-inner-eye]类的元素放在[.cat-right-eye]元素内。

# --hints--

你不应该更改现有的带有[cat-left-eye]类的[div]元素。

```js
assert.lengthOf(document.querySelectorAll('div.cat-left-eye'), 1);
```

你不应该更改现有的带有[cat-right-eye]类的[div]元素。

```js
assert.lengthOf(document.querySelectorAll('div.cat-right-eye'), 1);
```

你应该在[.cat-left-eye]元素内有一个[div]元素。

```js
assert.lengthOf(document.querySelectorAll('.cat-left-eye div'), 1);
```

你应该在[.cat-right-eye]元素内有一个[div]元素。

```js
assert.lengthOf(document.querySelectorAll('.cat-right-eye div'), 1);
```

在[.cat-left-eye]内的新[div]元素应该有[cat-left-inner-eye]类。

```js
assert.isTrue(document.querySelectorAll('.cat-left-eye div')[0]?.classList.contains('cat-left-inner-eye'));
```

在[.cat-right-eye]内的新[div]元素应该有[cat-right-inner-eye]类。
