...
通过设置[position]属性为[absolute]，[top]为[54px]，[left]为[134px]，将右眼移动到正确位置。

# --hints--

你的[.cat-right-eye]选择器应该有一个[position]属性，设置为[absolute]。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.position, 'absolute')
```

你的[.cat-right-eye]选择器应该有一个[top]属性，设置为[54px]。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.top, '54px')
```

你的[.cat-right-eye]选择器应该有一个[left]属性，设置为[134px]。
