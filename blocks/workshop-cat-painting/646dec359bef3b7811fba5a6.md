...
通过设置[position]为[absolute]，[top]为[8px]，[left]为[18px]，将右内眼移动到正确位置。同时，给它一个[60%]的边框半径，并将其旋转[-5deg]。

# --hints--

你的[.cat-right-inner-eye]选择器应该有一个[position]属性，设置为[absolute]。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-eye')?.position, 'absolute');
```

你的[.cat-right-inner-eye]选择器应该有一个[top]属性，设置为[8px]。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-eye')?.top, '8px');
```

你的[.cat-right-inner-eye]选择器应该有一个[left]属性，设置为[18px]。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-eye')?.left, '18px');
```

你的[.cat-right-inner-eye]选择器应该有一个[border-radius]属性，设置为[60%]。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-inner-eye')?.borderRadius, '60%');
```

你的[.cat-right-inner-eye]选择器应该有一个[transform]属性，设置为[-5deg]。
