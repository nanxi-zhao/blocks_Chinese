...
给 [.foreground-buildings] 添加与你在 [.background-buildings] 上使用的相同的 `display`、`align-items` 和 `justify-content` 属性和值。同样，这将使用 Flexbox 来在容器底部均匀分布建筑物。

# --hints--

你应该给 [.foreground-buildings] 设置 `display` 属性为 `flex`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foreground-buildings')?.display, 'flex');
```

你应该给 [.foreground-buildings] 设置 `align-items` 属性为 `flex-end`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.foreground-buildings')?.alignItems, 'flex-end');
```

你应该给 [.foreground-buildings] 设置 `justify-content` 属性为 `space-evenly`。
