...
给 [.bb1d] 添加一个 `linear-gradient`，第一种颜色是 `orange`，第二种是 [--building-color1]，第三种是 [--window-color1]。记住要在 `background` 属性上使用渐变。
...
你应该使用 [--window-color1] 作为 `linear-gradient` 中的第三种颜色。
