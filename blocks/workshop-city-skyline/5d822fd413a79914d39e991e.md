...
给 [.fb2-window] 元素设置 [width] 为 [22%]，[height] 为 [100%]，以及 [background-color] 为你的 [--window-color3] 变量。

# --hints--

你应该给 [.fb2-window] 元素设置 [width] 为 [22%]。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2-window")?.width, "22%");
```

你应该给 [.fb2-window] 元素设置 [height] 为 [100%]。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb2-window")?.height, "100%");
```

你应该给 [.fb2-window] 元素设置 [background-color] 为 [--window-color3]。
