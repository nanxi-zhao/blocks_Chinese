...
只剩下三栋建筑了。在`.fb4`元素内嵌套两个新的`div`元素，并给它们分别添加`fb4a`和`fb4b`类。记住你已经调换了`.fb4`和`.fb5`的位置，所以现在你正在处理的是最右边的紫色建筑。

# --hints--

你应该在`.fb4`内添加两个`div`元素。

```js
assert.lengthOf(document.querySelectorAll("div.fb4 > div"), 2);
```

你应该给第一个新的`div`添加`fb4a`类。

```js
assert.exists(document.querySelector("div.fb4a"));
```

你应该给第二个新的`div`添加`fb4b`类。
