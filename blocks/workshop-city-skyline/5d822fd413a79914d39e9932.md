...
轮到下一栋建筑啦！就是前景中的那栋绿色建筑。给它添加一个 [repeating-linear-gradient](#)，颜色从 [0%](#) 到 [5%](#) 使用你的建筑颜色，从 [5%](#) 到 [10%](#) 使用 [transparent](#)。

# --hints--

你应该给 [.fb5](#) 添加一个 [background](#) 属性。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle(".fb5")?.background);
```

你应该给 [background](#) 添加一个 [repeating-linear-gradient](#)。

```js
assert.include(new __helpers.CSSHelp(document).getStyle(".fb5")?.background, "repeating-linear-gradient");
```

你应该给 [repeating-linear-gradient](#) 设置第一个颜色为 [--building-color2](#)，范围从 [0%](#) 到 [5%](#)。
...
你应该给 [repeating-linear-gradient](#) 设置第二个颜色为 [transparent](#)，范围从 [5%](#) 到 [10%](#)。
