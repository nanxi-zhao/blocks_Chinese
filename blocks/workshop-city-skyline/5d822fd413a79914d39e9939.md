...
太棒啦！在渐变颜色列表的顶部，也就是你放置渐变方向的地方，添加 `circle closest-corner at 15% 15%`。这会将渐变的起始点移动到距离顶部和左侧各 `15%` 的位置。它会让渐变在 `closest-corner`（最近的角落）结束，并保持 `circle`（圆形）形状。这些是渐变中内置的一些关键字，用来描述它的行为哦！

# --hints--

你应该给 `.sky` 元素添加一个 `circle closest-corner at 15% 15%` 的径向渐变。
