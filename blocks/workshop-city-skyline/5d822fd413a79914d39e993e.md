...
太棒啦！最后一步，让我们在媒体查询的 [`:root`](https://developer.mozilla.org/zh-CN/docs/Web/CSS/:root) 选择器中，重新定义所有四个 `--window-color` 变量，将它们的值都设为 `#777` 吧！完成之后，调整窗口大小，看着它从白天变成黑夜，真是太神奇了！

变量主要用于颜色，这也是你在这里使用它们的方式。但它们可以被赋予任何值并用于任何属性。你的项目看起来真棒！

# --hints--

你应该添加 `--window-color1` 并将其值设为 `#777`。
...
你应该添加 `--window-color2` 并将其值设为 `#777`。
...
你应该添加 `--window-color3` 并将其值设为 `#777`。
...
你应该添加 `--window-color4` 并将其值设为 `#777`。
