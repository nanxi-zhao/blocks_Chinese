...
在你开始编写动画代码之前，让我们先看看你将用来从DOM中添加和删除元素的函数：[setTimeout]。

[setTimeout]函数接受两个参数：一个回调函数和一个表示等待执行回调函数的毫秒数的数字。

例如，如果你想在[3]秒后将["Hello, world!"]记录到控制台，你会这样写：
...
使用[setTimeout]函数在文本["Code"]记录到控制台之前添加一秒的延迟。然后看看在数字输入框中输入[5]并点击[Convert]按钮后会发生什么。
