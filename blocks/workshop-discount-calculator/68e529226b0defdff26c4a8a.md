...
现在，创建一个名为 `Wireless Mouse` 且价格为 `50.0` 的 `Product` 实例。将其分配给名为 `product` 的变量。然后，将 `product` 打印到控制台以查看它的样子。
...
你应该将 `Product` 的一个实例分配给 `product` 变量。
...
你的 `Product` 实例应该有名称 `Wireless Mouse` 和价格 `50.0`。
...
当你打印一个名称为 `'Wireless Mouse'` 且价格为 `50.0` 的 `Product` 时，它应该显示 `Wireless Mouse - $50.0`。
