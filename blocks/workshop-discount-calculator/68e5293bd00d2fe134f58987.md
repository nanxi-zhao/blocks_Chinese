...
现在你将创建另一种折扣策略。创建一个名为 `FixedAmountDiscount` 的类，该类继承自 `DiscountStrategy`。

给它一个 `__init__` 方法，接受 `self` 和 `amount` 作为参数。给 `amount` 参数一个 `int` 类型提示，方法的返回类型提示为 `None`。在 `__init__` 方法中，将 `amount` 存储为实例属性。
...
你的 `FixedAmountDiscount` 类应该继承自 `DiscountStrategy`。
...
你的 `FixedAmountDiscount` 类应该有一个 `__init__` 方法。
...
你的 `__init__` 方法应该有参数 `self` 和 `amount: int`。
...
你应该在 `__init__` 方法中将 `amount` 分配给 `self.amount`。
