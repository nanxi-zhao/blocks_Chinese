...
现在为 `FixedAmountDiscount` 实现 `apply_discount` 方法。这个方法应该从产品价格中减去固定金额并返回新价格。

# --hints--

你的 `FixedAmountDiscount` 类应该有一个 `apply_discount` 方法。
...
你的 `apply_discount` 方法应该返回 `product.price - self.amount`。
