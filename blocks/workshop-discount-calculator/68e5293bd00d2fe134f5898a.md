...
让我们测试一下你的新折扣策略。在现有代码之后，创建一个金额为 `5` 的 `FixedAmountDiscount` 实例，并将其分配给名为 `fixed_discount` 的变量。然后将此折扣应用于产品并打印结果。

# --hints--

你应该创建一个名为 `fixed_discount` 的变量，其中包含一个 `FixedAmountDiscount` 实例。
...
你的 `FixedAmountDiscount` 实例应该初始化为金额 `5`。
