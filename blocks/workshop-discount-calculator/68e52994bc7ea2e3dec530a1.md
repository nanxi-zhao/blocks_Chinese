...
创建一个名为 `DiscountEngine` 的类。这个类将管理所有折扣策略并计算产品的最佳价格。

给它一个 `__init__` 方法，接受 `self` 和 `strategies` 作为参数，其中 `strategies` 有类型提示 `List[DiscountStrategy]`。方法的返回类型应该是 `None`。在 `__init__` 方法中，将 `strategies` 存储为实例属性。
...
你的 `__init__` 方法应该有参数 `self` 和 `strategies: List[DiscountStrategy]`。
...
你应该在 `__init__` 方法中将 `strategies` 分配给 `self.strategies`。
