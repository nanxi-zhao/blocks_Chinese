...
现在让我们测试完整的折扣系统！添加一个 `if` 语句，检查 `__name__ == '__main__'` 并将 `product = Product('Wireless Mouse', 50.0)` 移动到其中。

之后，在 `if` 代码块内，创建一个变量 `user_tier` 并将其设置为字符串 `Premium`。然后创建一个变量 `strategies`，设置为包含 `PercentageDiscount(10)`、`FixedAmountDiscount(5)` 和 `PremiumUserDiscount()` 的策略列表。

# --hints--

你应该有一个检查 `__name__ == '__main__'` 的 `if` 语句。
...
你应该将 `product = Product('Wireless Mouse', 50.0)` 移动到 `if` 代码块内。
...
你应该在 `if` 代码块内将 `user_tier` 设置为字符串 `Premium`。
...
你应该创建一个名为 `strategies` 的列表，其中包含 `PercentageDiscount(10)`、`FixedAmountDiscount(5)` 和 `PremiumUserDiscount()`。
