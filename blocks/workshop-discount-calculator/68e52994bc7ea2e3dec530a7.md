...
为了完成这个工作坊，在最后添加一个打印语句，使用 f-string 和以下格式显示带有两位小数的最佳价格：`Best price for [product.name] for [user_tier] user: $[best_price]`，其中 `[product.name]`、`[user_tier]` 和 `[best_price]` 应该替换为实际的变量值。

输出示例可能是：`Best price for Wireless Buds for Premium user: $93.00`。注意价格的小数点后两位。

这样，折扣计算器工作坊就完成了！

# --hints--

你的输出应该匹配格式：`Best price for {product.name} for {user_tier} user: ${best_price:.2f}`。
