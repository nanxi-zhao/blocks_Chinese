...
创建一个 `DiscountEngine` 实例并将其存储在变量 `engine` 中。之后，使用 `calculate_best_price` 方法计算最佳价格并将其存储在名为 `best_price` 的变量中。

# --hints--

你应该使用 `strategies` 列表创建一个 `DiscountEngine` 实例。
...
你应该调用 `engine.calculate_best_price(product, user_tier)` 并将结果存储在 `best_price` 中。
