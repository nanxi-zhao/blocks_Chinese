...
现在，你需要从 `0/10` 文本中获取第一个数字，这种方式即使对于 `5/10` 或 `10/10` 也有效。

解析 `.count` 元素的文本内容以提取当前数字。

将计数从字符串转换为数字。

以这种格式记录当前计数：`console.log("Current count:", currCount)`。

提示：记住你可以使用 `split` 方法将字符串分割成子字符串数组。
