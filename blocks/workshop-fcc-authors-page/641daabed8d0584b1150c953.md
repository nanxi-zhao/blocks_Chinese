...
一些作者的传记比其他作者的要长得多。为了让卡片看起来统一，你可以提取每个传记的前 50 个字符，并用省略号 `"..."` 替换其余部分。否则，你可以显示完整的传记。

在段落元素内，用三元运算符替换 `bio`。对于条件，检查 `bio` 的长度是否大于 50。如果是，则使用 `.slice()` 方法提取 `bio` 的前 50 个字符并在末尾添加省略号。否则，显示完整的 `bio`。
...
如果 `bio` 文本大于 50 个字符，你应该使用 `slice()` 提取前 50 个字符，并用 `"..."` 替换其余部分。别忘了索引是从零开始的。
...
如果 `bio` 文本小于 50 个字符，直接使用 `bio` 文本。
