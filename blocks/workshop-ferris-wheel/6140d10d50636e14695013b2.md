...
为第二个 `.line` 元素创建一个选择器，并将 `transform` 设置为 `rotate(60deg)`。

提示：`transform` 可以改变元素的形状；此处使用 `rotate(60deg)` 会使元素围绕其 `transform-origin` 顺时针旋转 60 度。


# --hints--

应创建 `.line:nth-of-type(2)` 选择器。

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('.line:nth-of-type(2)'));
```

`.line:nth-of-type(2)` 的 `transform` 应为 `rotate(60deg)`。
