...
现在开始将座舱摆放到摩天轮周围。选择第一个 `.cabin` 元素，将 `right` 设置为 `-8.5%`，`top` 设置为 `50%`。


# --hints--

应包含 `.cabin:nth-of-type(1)` 选择器。
...
`.cabin:nth-of-type(1)` 的 `right` 属性应为 `-8.5%`。
...
`.cabin:nth-of-type(1)` 的 `top` 属性应为 `50%`。
