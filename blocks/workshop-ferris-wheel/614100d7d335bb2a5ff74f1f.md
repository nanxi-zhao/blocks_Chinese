...
在你的 `.wheel` 选择器中，你用四个不同的属性来控制动画。对于 `.cabin` 选择器，可以使用 `animation` 属性一次性设置这些值。

将 `.cabin` 规则的 `animation` 属性设置为 `cabins 10s linear infinite`。这样会按顺序设置 `animation-name`、`animation-duration`、`animation-timing-function` 和 `animation-iteration-count`。


# --hints--

`.cabin` 选择器的 `animation` 属性应设置为 `cabins 10s linear infinite`。
