...
选择 `.penguin-head` 元素，给它设置宽度为其父元素的一半，高度为 `45%`。然后，将 `background` 设置为 `45deg` 的线性渐变，从 `gray` 开始，到 `rgb(239, 240, 228)` 结束。
