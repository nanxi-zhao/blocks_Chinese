...
在[.face.right]元素下方，添加一个[class]为[chin]的[div]元素。

# --hints--

你应该在[.penguin-head]内添加一个[div]元素。期望找到3个[.penguin-head > div]元素，但找到了--fcc-actual--个。

```js
assert.lengthOf(document.querySelectorAll('.penguin-head > div'), 3);
```

你应该给[div]元素设置[class]为[chin]。

```js
assert.exists(document.querySelector('.penguin-head > div.chin'));
```

你应该将[div]元素放置在[.face.right]元素下方。
