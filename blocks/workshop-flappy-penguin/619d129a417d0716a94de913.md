...
选择具有 `bottom` 类的 `.beak` 元素，将其 `width` 设置为比 `.beak.top` 小 `4%`，`top` 设置为比 `.beak.top` 距离顶部多 `5%`，`left` 设置为比 `.beak.top` 距离左侧多 `2%`。
