...
向输入框添加一个 `onChange` 属性来捕获用户输入的内容。将其设置为一个以 `e` 为参数的箭头函数。在函数内部，通过将 `e.target.value` 传递给 setter 函数来更新 `query` 状态，该 setter 函数保存输入框的当前值。这是一个示例：
...
你的 `onChange` 处理程序应该使用 `e.target.value` 调用 `setQuery()`。
