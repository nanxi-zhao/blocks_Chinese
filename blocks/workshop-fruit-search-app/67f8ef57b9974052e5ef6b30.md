...
在 `#results` 元素内部，使用三元运算符检查 `results` 数组的长度是否大于零。如果是，则映射这些项目并在带有 `result-item` 类的 `p` 元素中显示每个项目。确保为每个段落包含一个 `key` 属性。

如果 `results` 为空，则渲染一个包含消息 `No results found` 的 `p` 元素。

# --hints--

你的 `#results` 容器应该包含一个基于查询的匹配水果列表。每个结果应该是一个带有 `result-item` 类的 `p` 元素，并显示水果的名称。
...
当 `results` 数组为空时，你的 `#results` 元素应该包含一个文本为 `No results found` 的 `p` 元素。
