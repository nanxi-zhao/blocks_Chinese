...
在 `useEffect` 回调函数内部，添加一个 `if` 条件来检查 `query.trim() === ''`。这会检查 `query` 是否为空字符串或仅包含空格。如果是，则调用 `setResults([])` 并提前返回。
...
如果 `query.trim() === ''` 为真，则调用 `setResults()` 传入一个空数组，然后在之后使用 `return` 关键字。
