...
现在你可以再次看到你的复选框了，你将为它完成一些样式选项。通过为你的`input[type="checkbox"]`选择器添加`4px`的`border-radius`来给它们一个圆角边缘。

然后给它一个`white`的`background-color`，使你的复选框中心从容器背景中突出显示。

由于你将为用户点击复选框时设置自定义过渡效果，设置一个`transition`属性，值为`all`和`0.3s`，这样过渡效果会在0.3秒内平滑发生，而不是瞬间完成。

# --hints--

你应该为`input[type="checkbox"]`选择器添加`border-radius`属性。
