...
在`input[type="checkbox"]:checked::after`选择器的声明中，将`display`设置为`block`。将你的`display`属性设置为`block`会使元素成为块级元素，这意味着它会占据容器的全部宽度并从新行开始，让你更容易控制其宽度、高度和间距。

接下来，将`text-align`设置为`center`。这将使行内内容（如文本或行内元素）在块内水平居中。块仍然会占据容器的全部宽度，但其中的所有内容都会对齐到中心。

# --hints--

你应该为`input[type="checkbox"]:checked::after`的组合类型选择器、伪类选择器和伪元素选择器添加`display`属性。
...
你应该为`input[type="checkbox"]:checked::after`的组合类型选择器、伪类选择器和伪元素选择器添加`text-align`属性。
