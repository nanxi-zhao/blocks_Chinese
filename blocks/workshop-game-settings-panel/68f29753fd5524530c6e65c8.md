...
`vertical-align`属性控制`inline`或`inline-block`元素如何与周围的文本或其他内联元素垂直对齐。它通常用于调整图像、图标或文本在一行中的垂直位置。

然后用`15px`的`margin`属性完成你的复选框设置。

# --hints--

你应该在`input[type="checkbox"]`选择器中添加一个`vertical-align`属性。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('input[type="checkbox"]')?.verticalAlign);
```

你应该有一个值为`middle`的`vertical-align`属性。

```js
assert.strictEqual(new __helpers.CSSHelp(document).getStyle('input[type="checkbox"]')?.verticalAlign, "middle");
```

你应该在`input[type="checkbox"]`选择器中添加一个`margin-right`属性。

```js
assert.isNotEmpty(new __helpers.CSSHelp(document).getStyle('input[type="checkbox"]')?.marginRight);
```

你应该有一个值为`15px`的`margin-right`属性。
