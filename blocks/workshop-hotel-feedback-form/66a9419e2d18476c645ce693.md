...
`min` 和 `max` 属性用于设置输入字段中可接受的最小值和最大值。

在你的 `label` 元素下方，添加一个输入元素，`type` 属性设置为 `"number"`，`id` 属性设置为 `"age"`。

`name` 属性应该设置为 `"age"`，`min` 属性应该设置为 `"3"`，`max` 属性应该设置为 `"100"`。

# --hints--

你的 `label` 元素下方应该有一个数字 `input` 元素。

```js
assert.isNotNull(document.querySelector('label + input[type="number"]'));
```

你的数字 `input` 元素应该有一个 `id` 属性，值为 `age`。

```js
assert.isNotNull(document.querySelector('label + input#age'));
```

你的数字 `input` 元素应该有一个 `name` 属性，设置为 `age`。

```js
assert.isNotNull(document.querySelector('label + input[name="age"]'));
```

你的数字 `input` 元素应该有一个 `min` 属性，设置为 `"3"`。

```js
assert.isNotNull(document.querySelector('label + input[min="3"]'));
```

你的数字 `input` 元素应该有一个 `max` 属性，设置为 `"100"`。
