...
在你的 `label` 元素下方，添加一个 `radio` 按钮，`id` 属性设置为 `"no-option"`，`name` 属性设置为 `"hotel-stay"`。

在你的新 `radio` 按钮下方，添加另一个 `label` 元素，其 `for` 属性设置为 `"no-option"`。`label` 的文本内容应该是 `No`。

完成后，你就可以尝试使用单选按钮，每次只能选择一个选项。
