...
你的酒店反馈表还应该允许用户对食物进行评分。

首先添加一个 `label` 元素，其文本内容为 `How was the food?`。这个 `label` 元素应该将 `for` 属性设置为 `"food"`。

在你的 `label` 元素下方，添加一个 `select` 元素，`id` 和 `name` 属性都设置为 `"food"`。

# --hints--

你应该有一个 `label` 元素，`for` 属性设置为 `"food"`。

```js
assert.exists(document.querySelector('label[for="food"]'));
```

你的 `label` 元素应该包含文本 `How was the food?`。

```js
assert.strictEqual(document.querySelector('label[for="food"]')?.textContent.trim(), 'How was the food?');
```

你应该在 `label` 元素下方有一个 `select` 元素，`id` 属性设置为 `"food"`。

```js
assert.exists(document.querySelector('label + select#food'));
```

你的 `select` 元素应该将 `name` 属性设置为 `"food"`。
