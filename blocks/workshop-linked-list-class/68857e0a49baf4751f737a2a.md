...
现在，在 `is_empty` 方法之后，在 `LinkedList` 类中创建一个名为 `add` 的方法。新方法应该有两个参数，`self` 和 `element`。

在你的新方法中，创建一个名为 `node` 的变量，并将其赋值为使用 `self.Node(element)` 创建的 `Node` 新实例。这将创建一个要添加到链表中的新节点。太厉害了！
...
你的 `add` 方法应该有两个参数，`self` 和 `element`。
...
你应该将 `self.Node(element)` 赋值给 `add` 方法中的 `node`。
