...
如果列表为空，那么新节点就成为第一个元素（`head`）。在 `add` 方法中，创建一个 `if` 语句，使用 `self.is_empty()` 检查列表是否为空，并将 `node` 赋值给 `self.head`。你做得真棒！

# --hints--

你应该通过使用 `is_empty()` 方法来检查列表是否为空。
...
你应该在 `if` 语句中将 `node` 赋值给 `self.head`。
