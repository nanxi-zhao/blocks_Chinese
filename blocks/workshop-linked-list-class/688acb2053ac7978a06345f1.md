...
接下来，创建一个检查 `previous_node` 是否不为 `None` 的 `elif` 语句。在其中将 `current_node.next` 赋值给 `previous_node.next`。

这样，如果找到了要移除的元素且它不在头节点中，前一个节点的 `next` 指针将被更新以跳过当前节点（要移除的元素），从而将其从链表中移除。太厉害了！

# --hints--

你应该创建一个检查 `previous_node` 是否不为 `None` 的 `elif` 语句。
...
你应该在 `elif` 语句中将 `current_node.next` 赋值给 `previous_node.next`。
