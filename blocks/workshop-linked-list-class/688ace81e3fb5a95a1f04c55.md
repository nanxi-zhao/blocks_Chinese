...
接下来，创建一个 `else` 语句块，将 `current_node.next` 赋值给 `self.head`。

这处理了在头节点中找到要移除的元素的情况。通过将 `self.head` 更新为指向 `current_node.next`，头节点将从链表中移除。你做得真棒！

# --hints--

你应该创建一个 `else` 语句块，将 `current_node.next` 赋值给 `self.head`。
