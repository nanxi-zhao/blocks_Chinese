...
在你的 `aside` 元素内部，依次创建两个 `img`、一个 `blockquote`、再创建第三个 `img`。同时将 `blockquote` 的 `class` 设置为 `image-quote`。

# --hints--

你应在 `aside` 中创建三个 `img` 元素。

```js
assert.lengthOf(document.querySelectorAll('aside img'), 3);
```

你应在 `aside` 中创建一个 `blockquote` 元素。

```js
assert.exists(document.querySelector('aside blockquote'));
```

你的 `blockquote` 元素应具有 `image-quote` 类。
