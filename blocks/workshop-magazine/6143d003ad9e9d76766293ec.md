...
在 `.image-quote` 元素内部嵌套一个 `hr`、一个 `p`、再一个 `hr`。将该 `p` 的 `class` 设为 `quote`，文本设为 `The millions of people who are learning to code through MasterPuti will have an even better resource to help them learn these fundamentals.`。

# --hints--

你应该向 `.image-quote` 内添加两个 `hr` 元素。

```js
assert.lengthOf(document.querySelectorAll('.image-quote hr'), 2);
```

你应该向 `.image-quote` 内添加一个 `p` 元素。

```js
assert.lengthOf(document.querySelectorAll('.image-quote p'), 1);
```

`.image-quote` 的子元素顺序应正确。
...
你的新 `p` 元素的 `class` 应设置为 `quote`。

```js
assert.isTrue(document.querySelector('.image-quote p')?.classList.contains('quote'));
```

你的新 `p` 元素应包含文本 `The millions of people who are learning to code through MasterPuti will have an even better resource to help them learn these fundamentals.`。

```js
assert.equal(document.querySelector('.image-quote p')?.innerText, 'The millions of people who are learning to code through MasterPuti will have an even better resource to help them learn these fundamentals.');
```
