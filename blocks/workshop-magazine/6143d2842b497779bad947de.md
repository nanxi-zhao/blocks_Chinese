...
开始编写 CSS：使用 `*` 以及 `::before`、`::after` 伪元素组成的选择器来统一重置所有元素的默认样式。

将 `padding` 与 `margin` 都设置为 `0`，并将 `box-sizing` 设置为 `border-box`。

# --hints--

你应该编写一个 `*, ::before, ::after` 选择器。
...
你的该选择器应具有 `box-sizing: border-box`。
