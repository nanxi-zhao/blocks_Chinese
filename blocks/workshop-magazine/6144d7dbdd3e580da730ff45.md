...
创建一个 `body` 选择器。将 `font-family` 设置为 `Baskervville`，并提供回退 `serif`。再将文本颜色 `color` 设置为 `linen`，背景色 `background-color` 设置为 `rgb(20, 30, 40)`。
...
你的 `body` 选择器应包含一个 `font-family` 属性，值为 `Baskervville`，回退为 `serif`。
...
你的 `body` 选择器应包含一个 `color` 属性，值为 `linen`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('body')?.color, 'linen');
```

你的 `body` 选择器应包含一个 `background-color` 属性，值为 `rgb(20, 30, 40)`。
