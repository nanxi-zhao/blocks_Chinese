...
现在你可以开始设置网格布局的样式。CSS Grid 与 Flexbox 类似，都是通过在父元素上添加特殊属性来影响其子元素的排列方式。

这里的父元素是 `main`。给它添加 `grid-template-columns: 1fr 94rem 1fr`，构建三列布局：中间列固定为 `94rem`，左右两列各用 `1fr` 占据剩余空间的一半。

# --hints--

你的 `main` 选择器应包含一个值为 `1fr 94rem 1fr` 的 `grid-template-columns` 属性。
