...
使用 `minmax` 函数让列在各种设备上具备响应性。`minmax(最小值, 最大值)` 的两个参数可以是长度、百分比、`fr` 单位，甚至是诸如 `max-content` 的关键字。

将你之前 `grid-template-columns` 的每个值包裹进一个 `minmax`，作为第二个参数；第一个参数分别设置为 `2rem`、`min-content`、`2rem`。这样组合后得到：`minmax(2rem, 1fr) minmax(min-content, 94rem) minmax(2rem, 1fr)`。

# --hints--

你的 `main` 选择器应包含一个 `grid-template-columns` 属性，值为 `minmax(2rem, 1fr) minmax(min-content, 94rem) minmax(2rem, 1fr)`。
        ...
        <aside class="image-wrapper">
          ...
          <blockquote class="image-quote">
            ...
            <p class="quote">
              ...
              through MasterPuti will have an even better resource to help
