...
你的杂志将拥有三个主要区块。你已经在 `main` 规则中设置了整体布局，但可以在子元素的规则中进一步调整它们的具体位置。

一个常用的方式是使用 `grid-column` 属性，它是 `grid-column-start` 与 `grid-column-end` 的简写，用来指定网格子项从哪条网格线开始、在哪条结束。

创建一个 `.heading` 规则，将其 `grid-column` 设置为 `2 / 3`。这表示 `.heading` 元素从第 2 条网格线开始，到第 3 条网格线结束。
...
你的 `.heading` 选择器应包含一个值为 `2 / 3` 的 `grid-column` 属性。
