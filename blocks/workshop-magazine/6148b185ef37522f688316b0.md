...
你可以在 CSS Grid 中使用 `repeat()` 来重复列或行定义，从而避免重复书写相同的值。例如 `grid-template-columns: repeat(20, 200px)` 会创建 20 列，每列宽度为 `200px`。

给 `.heading` 选择器添加 `grid-template-columns`，并将其设置为 `repeat(2, 1fr)`，以创建两个等宽列。

# --hints--

你的 `.heading` 选择器应包含 `grid-template-columns: repeat(2, 1fr)`。
