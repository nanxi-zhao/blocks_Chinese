...
记住 `grid-column` 属性决定元素开始与结束所处的列；有时你不知道网格会有多少列，但希望元素延伸到最后一列，可以在结束列位置使用 `-1`。

创建一个 `.hero` 选择器，并将其 `grid-column` 设置为 `1 / -1`，使该元素横跨整个网格宽度。
...
你的 `.hero` 选择器应包含一个值为 `1 / -1` 的 `grid-column` 属性。
