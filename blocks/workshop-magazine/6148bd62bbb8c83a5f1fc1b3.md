...
创建一个 `.hero-title` 选择器，并给它一个 `text-align` 属性设置为 `center`，一个 `color` 属性设置为 `orangered`，以及一个 `font-size` 属性设置为 `8rem`。
