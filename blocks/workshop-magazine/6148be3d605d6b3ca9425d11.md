...
副标题也需要设置样式。创建一个 `.hero-subtitle` 选择器，并给它一个 `font-size` 属性设置为 `2.4rem`，一个 `color` 属性设置为 `orangered`，以及一个 `text-align` 属性设置为 `center`。
