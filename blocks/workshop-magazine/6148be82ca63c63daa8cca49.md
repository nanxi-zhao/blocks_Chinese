...
创建一个 `.author` 选择器，并将其 `font-size` 设置为 `2rem`，`font-family` 设置为 `Raleway`，回退为 `sans-serif`。
...
你的 `.author` 选择器应包含一个值为 `2rem` 的 `font-size` 属性。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.author')?.fontSize, '2rem');
```

你的 `.author` 选择器应包含一个 `font-family` 属性，值为 `Raleway`，回退为 `sans-serif`。
