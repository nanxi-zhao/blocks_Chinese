...
创建一个 `.author-name a:hover` 选择器，并将其 `background-color` 设置为 `#306203`。

这样在鼠标悬停 `.author-name` 内的 `a` 元素时，会显示 MasterPuti 原始的绿色背景。
...
你的 `.author-name a:hover` 选择器应包含一个值为 `#306203` 的 `background-color` 属性。
