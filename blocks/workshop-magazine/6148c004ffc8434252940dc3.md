...
创建一个 `.social-icons` 选择器。将其 `display` 设置为 `grid`，并将 `font-size` 设置为 `3rem`。
...
你的 `.social-icons` 选择器应包含一个值为 `grid` 的 `display` 属性。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.social-icons')?.display, 'grid');
```

你的 `.social-icons` 选择器应包含一个值为 `3rem` 的 `font-size` 属性。
