...
CSS Grid 默认会根据需要创建额外的行，这与 Flexbox 不同。将 `.social-icons` 选择器的 `grid-template-columns` 属性设置为 `repeat(5, 1fr)` 可把图标排列成一行。

# --hints--

你的 `.social-icons` 选择器应包含 `grid-template-columns: repeat(5, 1fr)`。
