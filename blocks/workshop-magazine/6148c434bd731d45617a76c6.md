...
如果想添加更多社交图标但仍保持在同一行，可以使用 `grid-auto-flow` 来自动为内容创建列，而无需手动调整 `grid-template-columns`。

该属性第一个值可以是 `row` 或 `column`，可选的第二个值 `dense` 会让自动布局尝试回填空位（可能导致元素顺序变化）。

将 `.social-icons` 选择器的 `grid-auto-flow` 属性设置为 `column`。

# --hints--

你的 `.social-icons` 选择器应包含 `grid-auto-flow: column`。
