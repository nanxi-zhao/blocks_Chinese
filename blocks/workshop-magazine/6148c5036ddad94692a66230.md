...
当自动放置算法在添加新图标时创建新列时，新列的宽度默认是 `auto`，这可能与现有列不一致。

可以使用 `grid-auto-columns` 覆盖默认宽度。将 `.social-icons` 选择器的 `grid-auto-columns` 属性设置为 `1fr`。

# --hints--

你的 `.social-icons` 选择器应包含 `grid-auto-columns: 1fr`。
