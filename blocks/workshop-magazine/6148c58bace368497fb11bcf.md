...
与 Flexbox 类似，CSS Grid 提供 `align-items`（沿列轴，即纵向）和 `justify-items`（沿行轴，即横向）来对齐网格项的内容。

将 `.social-icons` 选择器的 `align-items` 属性设置为 `center`，使图标在列方向上垂直居中。

# --hints--

你的 `.social-icons` 选择器应包含 `align-items: center`。
