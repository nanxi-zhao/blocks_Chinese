...
其他文本已经被移出原位置。通过给 `.first-paragraph::first-letter` 选择器添加一个 `float` 属性设置为 `left` 和一个 `margin-right` 属性设置为 `1rem` 来将其移回原位。

# --hints--

你的 `.first-paragraph::first-letter` 选择器应该有一个 `float` 属性设置为 `left`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.first-paragraph::first-letter')?.float, 'left');
```

你的 `.first-paragraph::first-letter` 选择器应该有一个 `margin-right` 属性设置为 `1rem`。
