...
创建一个 `.quote` 选择器。给它一个 `color` 属性设置为 `#00beef`，一个 `font-size` 属性设置为 `2.4rem`，以及一个 `text-align` 属性设置为 `center`。
