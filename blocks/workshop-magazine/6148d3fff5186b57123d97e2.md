...
没有合适的引号，引语就不是真正的引语。你可以使用CSS伪选择器来添加这些引号。

创建一个 `.quote::before` 选择器，并将 `content` 属性设置为 `"` 后面跟一个空格。

同时，创建一个 `.quote::after` 选择器，并将 `content` 属性设置为一个空格后跟 `"`。
...
你的 `.quote::before` 选择器应该有一个 `content` 属性设置为 `'" '`。
...
你的 `.quote::after` 选择器应该有一个 `content` 属性设置为 `' "'`。
