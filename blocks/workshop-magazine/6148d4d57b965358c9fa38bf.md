...
现在是为第三个 `section` 添加样式的时候了。注意它的 `class` 属性包含 `text` 与 `text-with-images`，因此它会继承 `.text` 的样式。

创建一个 `.text-with-images` 选择器，并将其 `display` 设置为 `grid`。
...
你的 `.text-with-images` 选择器应包含一个值为 `grid` 的 `display` 属性。
