...
需要为文本和图片分别设置列。将 `.text-with-images` 选择器的 `grid-template-columns` 设置为 `1fr 2fr`，并将 `column-gap` 设置为 `3rem`，以在列间增加间距。

# --hints--

你的 `.text-with-images` 选择器应包含 `grid-template-columns: 1fr 2fr`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.text-with-images')?.gridTemplateColumns, '1fr 2fr');
```

你的 `.text-with-images` 选择器应包含 `column-gap: 3rem`。
