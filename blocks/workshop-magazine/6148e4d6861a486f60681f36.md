...
请将图片放入一个两列三行的网格布局中：为 `.image-wrapper` 选择器设置 `grid-template-columns: 2fr 1fr`（两列，左列较宽），并设置 `grid-template-rows: repeat(3, min-content)`（三行，高度根据内容自动调整）。

# --hints--

你的 `.image-wrapper` 选择器应包含 `grid-template-columns: 2fr 1fr`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.image-wrapper')?.gridTemplateColumns, '2fr 1fr');
```

你的 `.image-wrapper` 选择器应包含 `grid-template-rows: repeat(3, min-content)`。
