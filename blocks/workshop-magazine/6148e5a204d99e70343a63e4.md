...
`gap` 属性是同时设置 `column-gap` 和 `row-gap` 值的简写方式。如果给定一个值，它会将 `column-gap` 和 `row-gap` 都设置为该值。如果给定两个值，它会将第一个值设置为 `row-gap`，将第二个值设置为 `column-gap`。

给 `.image-wrapper` 选择器添加一个 `gap` 属性，设置为 `2rem`。
