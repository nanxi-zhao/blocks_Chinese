...
以针对 `only screen` 的 `@media` 查询开始，将 `max-width` 设置为 `720px`。在里面，创建一个 `.image-wrapper` 选择器，并给它一个值为 `1fr` 的 `grid-template-columns` 属性。
