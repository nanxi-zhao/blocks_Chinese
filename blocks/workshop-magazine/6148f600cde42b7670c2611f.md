...
为 `only screen` 创建另一个 `@media` 查询，其 `max-width` 为 `600px`。在其中，创建一个 `.text-with-images` 规则，并为其设置一个值为 `1fr` 的 `grid-template-columns` 属性。
