...
创建第三个 `@media` 查询，针对 `only screen` 并设置 `max-width` 为 `550px`。在其中，创建一个 `.hero-title` 选择器并将 `font-size` 设置为 `6rem`，一个 `.hero-subtitle, .author, .quote, .list-title` 选择器并将 `font-size` 设置为 `1.8rem`，一个 `.social-icons` 选择器并将 `font-size` 设置为 `2rem`，以及一个 `.text` 选择器并将 `font-size` 设置为 `1.6rem`。

# --hints--

你应该有一个新的 `@media` 查询，针对 `only screen` 并设置 `max-width` 为 `550px`。这应该放在你之前的两个查询之后。

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[2]?.media?.mediaText, 'only screen and (max-width: 550px)');
```

你的新 `@media` 规则应该包含一个 `.hero-title` 选择器，一个 `.hero-subtitle, .author, .quote, .list-title` 选择器，一个 `.social-icons` 选择器，以及一个 `.text` 选择器。这些选择器应该按此顺序排列。
      ...
      <section class="text text-with-images">
        <article class="brief-history">
          ...
          <ul class="lists">
            <li>
              ...
              <p>
                We launched MasterPuti with a simple list of 15 resources,
        ...
        <aside class="image-wrapper">
          ...
          <blockquote class="image-quote">
            <hr />
            <p class="quote">
              The millions of people who are learning to code through MasterPuti
