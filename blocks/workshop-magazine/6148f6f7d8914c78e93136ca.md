...
为 `only screen` 创建最后一个 `@media` 查询，其 `max-width` 为 `420px`。在其中，创建一个 `.hero-title` 选择器，并将 `font-size` 属性设置为 `4.5rem`。

恭喜你，你的杂志现在已经完成了！

# --hints--

你应该有一个新的 `@media` 查询，针对 `only screen and (max-width: 420px)`。这应该是 `@media` 查询列表中的最后一个查询。

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[3]?.media?.mediaText, 'only screen and (max-width: 420px)');
```

你的 `@media` 查询应该有一个 `.hero-title` 选择器。
