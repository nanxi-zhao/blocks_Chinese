...
创建另一个 `if` 语句来检查 `director` 是否为空或仅包含空白字符，如果是，则抛出一个带有消息 `Director cannot be empty` 的 `ValueError`。

# --hints--

当 `director` 为空或仅包含空白字符时，你应该抛出一个带有消息 `Director cannot be empty` 的 `ValueError`。
...
当 `director` 是有效字符串时，你不应该抛出 `ValueError`。
