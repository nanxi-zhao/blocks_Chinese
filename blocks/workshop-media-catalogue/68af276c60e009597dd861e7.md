...
最后，创建最后一个 `if` 语句来验证 `duration` 是否大于 `0`，如果不是，则抛出一个带有消息 `Duration must be positive` 的 `ValueError`。

# --hints--

当 `duration` 不是正数时，你应该抛出一个带有消息 `Duration must be positive` 的 `ValueError`。
...
当 `duration` 是有效数字时，你不应该抛出 `ValueError`。
