...
在 `try` 块之前，创建一个 `MediaCatalogue` 的实例，并将其分配给一个名为 `catalogue` 的变量。

然后，使用 `add` 方法将 `movie1` 追加到目录中。最后将 `catalogue` 打印到控制台。

# --hints--

你应该在 `try` 块之前将 `MediaCatalogue` 的一个实例分配给一个名为 `catalogue` 的变量。
...
你的 `try` 块中应该有 `catalogue.add(movie1)`。
