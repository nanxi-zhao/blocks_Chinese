...
`TVSeries` 对象将相对于其父类 `Movie` 使用附加属性进行实例化。因此，创建一个 `__init__` 方法，包含以下参数：`self`、`title`、`year`、`director`、`duration`、`seasons` 和 `total_episodes`。

在 `__init__` 方法内部，将 `seasons` 和 `total_episodes` 分配给同名属性。
...
你的 `__init__` 方法应该有以下参数：`self`、`title`、`year`、`director`、`duration`、`seasons` 和 `total_episodes`。
...
你应该在 `__init__` 方法中将 `seasons` 分配给 `self.seasons`。
...
你应该在 `__init__` 方法中将 `total_episodes` 分配给 `self.total_episodes`。
