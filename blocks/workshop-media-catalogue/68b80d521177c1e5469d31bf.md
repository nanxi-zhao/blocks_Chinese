...
现在 `__init__` 方法已经完成，在 `try` 块内创建一个名为 `series1` 的变量。为其分配一个 `TVSeries` 的实例，传入你选择的参数。然后，将 `series1` 打印到控制台。

# --hints--

你应该在 `try` 块内将 `TVSeries` 的一个实例分配给名为 `series1` 的变量。
...
try:
    ...
    series1 = TVSeries('Friends', 1994, 'David Crane', 22, 10, 236)
    print(series1)
--fcc-editable-region--
    print(catalogue)
