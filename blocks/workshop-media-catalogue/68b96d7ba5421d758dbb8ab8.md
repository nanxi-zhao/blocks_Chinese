...
创建另一个捕获 `MediaError as e` 的 `except` 子句。在 `except` 块中，使用 f-string 打印 `Media Error:` 后跟一个空格和错误消息。

# --hints--

你应该有一个捕获 `MediaError as e` 的 `except` 子句。
...
你应该在新的 `except` 块中使用 f-string 打印 `Media Error:` 后跟一个空格和错误消息。
