...
在你的 `__str__` 方法中的 `if` 语句之后，创建两个名为 `movies` 和 `series` 的变量，分别赋值为对 `get_movies` 方法和 `get_tv_series` 方法的调用。

# --hints--

你的 `__str__` 方法中应该有一个名为 `movies` 的变量。
...
你应该将 `self.get_movies()` 赋值给你的 `movies` 变量。
...
你的 `__str__` 方法中应该有一个名为 `series` 的变量。
...
你应该将 `self.get_tv_series()` 赋值给你的 `series` 变量。
