...
在 `constraints` 字典中添加键 `gender`。参照之前的写法，先验证 `gender` 是否为字符串，然后使用 `and` 运算符检查 `gender.lower()` 是否在 `('male', 'female')` 中。

# --hints--

请在 `constraints` 中添加 `gender` 键，并将其值设为 `isinstance(gender, str) and gender.lower() in ('male', 'female')`。
