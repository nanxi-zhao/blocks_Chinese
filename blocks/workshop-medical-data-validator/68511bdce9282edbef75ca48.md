...
`medications` 列表中的每一项都应该是字符串。在本步骤及下一步你将实现相应的检查。请使用 `and` 运算符为 `medications` 的约束添加一个表达式。

在 `and` 的右侧使用列表推导语法，对 `medications` 中的每个 `i` 计算 `isinstance(i, str)`，以生成布尔值列表。

# --hints--

`constraints` 中 `medications` 的值应为 `isinstance(medications, list) and [isinstance(i, str) for i in medications]`。
