...
`all` 函数会在传入的可迭代对象中所有元素都为真时返回 `True`，否则返回 `False`：
...
请将列表 `[isinstance(i, str) for i in medications]` 传给 `all`，以确保其中的每个元素均为字符串。

# --hints--

请将 `medications` 的约束值修改为 `isinstance(medications, list) and all([isinstance(i, str) for i in medications])`。
