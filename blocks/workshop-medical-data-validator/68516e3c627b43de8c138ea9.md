...
现在我们将再使用一个正则表达式。请使用 `and` 运算符为 `constraints['last_visit_id']` 添加一个表达式。

在 `and` 的右侧，使用 `re.fullmatch` 确保 `last_visit_id` 以字母 `v`（大小写均可）开头，后接一个或多个数字。

# --hints--

`constraints` 中 `last_visit_id` 的值应为 `isinstance(last_visit_id, str) and re.fullmatch('v\\d+', last_visit_id, re.IGNORECASE)`。
