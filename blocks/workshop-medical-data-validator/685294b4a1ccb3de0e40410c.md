...
回到 `validate` 函数，在两个 `if` 语句之后并且仍在 `for` 循环内部，创建一个名为 `invalid_records` 的变量。

然后，将其赋值为对 `find_invalid_records` 的调用，使用 `**` 操作符解包 `dictionary`。

# --hints--

在 `validate` 函数的 `for` 循环底部应有一个名为 `invalid_records` 的变量。
...
你的 `invalid_records` 变量的值应为 `find_invalid_records(**dictionary)`。
