...
现在按顺序在下拉菜单中添加四个选项：`All`、`Woodwinds`、`Brass` 和 `Percussion`。对于每个选项，请使用小写文本作为它的 `id` 和 `value` 属性值。

很棒，按顺序来就不会出错！

# --hints--

请确保你的 `select` 元素中有四个 `option`。

```js
assert.lengthOf(document.querySelectorAll("select>option"), 4);
```

第一个 `option` 的文本应为 `All`。

```js
assert.equal(document.querySelectorAll("select>option")[0].innerText,"All");
```

`All` 这个选项的 `id` 和 `value` 应为 `all`。
...
`Woodwinds` 的 `id` 与 `value` 应为 `woodwinds`。
