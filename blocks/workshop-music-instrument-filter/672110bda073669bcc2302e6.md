...
`instrumentsArr` 会包含每个乐器卡对应的对象，每个对象应包含 `category`、`instrument` 和 `price` 键。

请先向数组中添加第一个对象，用来表示 Flute（长笛）。相应的值应为 `woodwinds`、`Flute` 和 `500`。你做得很好，继续！

# --hints--

你的 `instrumentsArr` 数组现在应该包含一个元素。
...
该对象应具有 `category: "woodwinds"`。

```js
assert.equal(instrumentsArr[0].category, "woodwinds");
```

该对象应具有 `instrument: "Flute"`。
