...
为 `instrumentsArr` 再添加两个对象，分别表示剩下的 woodwinds 乐器：Clarinet（单簧管）和 Oboe（双簧管）。

做得很好，数据结构已经完整啦！

# --hints--

你的 `instrumentsArr` 中应包含 `{ category: "woodwinds", instrument: "Clarinet", price: 200 }`。

```js
assert.deepInclude(instrumentsArr, { category: "woodwinds", instrument: "Clarinet", price: 200 });
```

你的 `instrumentsArr` 中应包含 `{ category: "woodwinds", instrument: "Oboe", price: 4000 }`。

```js
assert.deepInclude(instrumentsArr, { category: "woodwinds", instrument: "Oboe", price: 4000 });
```

同时应该仍然保留 `{ category: "woodwinds", instrument: "Flute", price: 500 }`。
