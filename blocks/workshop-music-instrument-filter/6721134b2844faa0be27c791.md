...
剩余的乐器对象已为你添加到 `instrumentsArr` 中。

现在声明两个变量：`selectContainer` 和 `productsContainer`，分别将它们赋值为页面上的下拉元素和具有类名 `products-container` 的元素。

太棒了，接下来我们会把交互做起来！

# --hints--

请创建 `selectContainer` 变量。

```js
assert.exists(selectContainer);
```

`selectContainer` 应引用你的 `select` 元素。

```js
assert.strictEqual(selectContainer, document.querySelector("select"));
```

请创建 `productsContainer` 变量。

```js
assert.exists(productsContainer);
```

`productsContainer` 应引用页面上 `.products-container` 元素。
