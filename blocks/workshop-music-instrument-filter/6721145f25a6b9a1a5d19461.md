...
正如之前学习的，`change` 事件会在用户修改某些输入元素的值时触发。你希望每当下拉菜单选择变化时就更新页面，因此请为 `selectContainer` 添加一个监听 `change` 事件的事件处理器。

先在回调内部用 `console.log` 输出字符串 `this is a test`，然后在页面中切换不同选项，查看控制台以确认事件监听器正常工作。

# --hints--

请为 `selectContainer` 添加 `change` 事件的监听器，并在回调中用 `console.log` 输出 `this is a test`。
