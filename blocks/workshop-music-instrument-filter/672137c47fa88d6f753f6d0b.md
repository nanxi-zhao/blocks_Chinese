...
目前你的 `instrumentCards` 函数返回的是乐器对象数组，为了在页面上显示卡片，你还需要把这些对象转换为 HTML 字符串。

请修改函数，使其返回一个字符串数组——每个字符串都是一段用于展示乐器卡片的 HTML，格式应为 `<div class="card"><h2>[instrument]</h2><p>$[price]</p></div>`。
...
`instrumentCards("all")` 应返回一个由卡片 HTML 字符串组成的数组，数组中每个字符串的格式应匹配 `<div class="card"><h2>[instrument]</h2><p>$[price]</p></div>`。
...
`instrumentCards("woodwinds")` 应返回对应 woodwinds 的卡片字符串数组。
...
`instrumentCards("brass")` 应返回对应 brass 的卡片字符串数组。
...
`instrumentCards("percussion")` 应返回对应 percussion 的卡片字符串数组。
...
`instrumentCards("all")` 应返回十个卡片字符串，拼接后可以生成页面上对应的十个 `.card` 元素。
...
`instrumentCards("woodwinds")` 应返回三个对应的卡片字符串。
...
`instrumentCards("brass")` 应返回三个对应的卡片字符串。
...
`instrumentCards("percussion")` 应返回四个对应的卡片字符串。
