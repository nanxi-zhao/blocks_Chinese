...
在 `.progress-container` 元素内部，添加一个 `div` 元素，其 `className` 为 `progress-bar`。
...
你的 `div` 元素应该有一个 `className` 值为 `progress-bar`。

```js
assert.equal(document.querySelector('.progress-container div')?.className, 'progress-bar');
```
