...
如果你稍微玩一下你的播放器，你会注意到一些bug。首先，当你播放一首歌曲，暂停它，然后按上一首按钮或下一首按钮时，歌曲不会从头开始播放。

为了开始修复这个问题，给你的 [playSong] 函数添加一个名为 [start] 的第二个参数。给它默认值 [true]。

# --hints--

你的 [playSong] 函数应该有一个名为 [start] 的第二个参数，默认值为 [true]。
