...
接下来，你需要在播放器显示中显示当前歌曲的标题和艺术家。

作为第一步，访问负责显示歌曲标题和艺术家的`#player-song-title`和`#player-song-artist`元素，并将它们分别存储到变量`playingSong`和`songArtist`中。

太棒了！你已经学会了如何获取DOM元素并将其存储在变量中。这是前端开发中的基础技能，你正在稳步进步呢！
...
你的`playingSong`变量应该具有`#player-song-title`元素的值。
...
你的`songArtist`变量应该具有`#player-song-artist`元素的值。
