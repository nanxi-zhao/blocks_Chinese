...
接下来的步骤会涉及添加代码，用于动态显示“笔记已保存”状态，并将当前笔记记录到控制台，帮助你调试和观察变化。

首先创建一个变量 `noteEl`，并把查询到 `id` 为 `note` 的元素结果赋值给它。

然后创建另一个变量 `statusEl`，并把查询到 `id` 为 `status` 的元素结果赋值给它。

# --hints--

你应该定义 `noteEl` 变量，并将 `document.getElementById("note")` 的结果赋值给它。

```js
assert.equal(noteEl, document.getElementById("note"));
```

你应该定义 `statusEl` 变量，并将 `document.getElementById("status")` 的结果赋值给它。
