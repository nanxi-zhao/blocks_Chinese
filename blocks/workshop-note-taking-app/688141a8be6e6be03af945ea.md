...
当用户编辑笔记时，我们希望记录编辑后的内容。

使用 `let` 创建一个名为 `currentContent` 的变量，并将其初始化为空字符串。

注意：在真实应用中，通常会把笔记保存到数据库或本地存储（localStorage）。但数据库和本地存储的内容超出本练习范围，会在后续课程中讲解。

# --hints--

请使用 `let` 创建 `currentContent` 变量。

```js
assert.match(code, /let\s+currentContent/);
```

`currentContent` 应为字符串类型。

```js
assert.isString(currentContent);
```

`currentContent` 应初始化为空字符串。
