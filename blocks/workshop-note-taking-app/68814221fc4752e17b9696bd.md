...
当用户在笔记内编辑并在编辑后点击到笔记外部时，我们需要获取编辑后的内容。可以使用 `blur` 事件来实现：当元素失去焦点时会触发该事件。例如：
...
请在 `noteEl` 上添加 `addEventListener`，第一个参数为 `"blur"`，第二个参数为箭头函数。在箭头函数体内使用 `const` 声明 `newContent`，并将 `noteEl.innerHTML` 的值赋给它。

这里使用 `innerHTML` 可以更好地保留笔记中的空格和格式。

# --hints--

请在 `noteEl` 上添加 `addEventListener`。
...
在箭头函数体内使用 `const` 创建 `newContent`，并将 `noteEl.innerHTML` 的值赋给它。
