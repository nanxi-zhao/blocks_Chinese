...
把编辑后的内容输出到控制台会很有帮助，方便你调试与查看。

请添加一行 `console.log(currentContent);`。

你可以编辑笔记然后点击卡片外部来测试，随后应能在控制台看到更新后的笔记内容。

# --hints--

在事件监听器中、将 `newContent` 赋给 `currentContent` 之后，添加 `console.log(currentContent);`。
