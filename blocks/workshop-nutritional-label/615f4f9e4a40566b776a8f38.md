...
创建一个新的 `.large` 选择器，并将其 `height` 属性设为 `10px`。同时创建一个 `.large, .medium` 选择器，把 `background-color` 设为 `black`，这样更容易绘制粗细不同的分隔条。你做得很好，继续努力！
