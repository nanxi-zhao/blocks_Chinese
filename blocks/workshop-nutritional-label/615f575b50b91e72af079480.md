...
创建一个新的 `.left-container p` 选择器，将上下边距设置为 `-5px`，左右边距设置为 `-2px`。同时将 `font-size` 设置为 `2em`，`font-weight` 设置为 `700`。
...
你的新 `.left-container p` 选择器应该将 `margin` 属性设置为 `-5px -2px`。
...
你的新 `.left-container p` 选择器应该将 `font-size` 属性设置为 `2em`。

```js
assert(new __helpers.CSSHelp(document).getStyle('.left-container p')?.fontSize === '2em');
```

你的新 `.left-container p` 选择器应该将 `font-weight` 属性设置为 `700`。
