...
在你的 `.calories-info` 元素下方，添加一个 `class` 属性设置为 `divider medium` 的 `div`。

# --hints--

你应该在 `.label` 元素内创建一个新的 `div`。

```js
assert(document.querySelectorAll('.label > div')?.length === 3)
```

你的新 `div` 应该将 `class` 属性设置为 `divider medium`。这个 div 应该是你的 `.label` 元素中的最后一个元素。
