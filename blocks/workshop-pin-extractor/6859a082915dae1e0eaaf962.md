...
最后，返回 `secret_codes`，然后取消注释函数调用并将参数更新为 `[poem, poem2, poem3]`。现在你的函数可以同时处理多首诗歌，并且可以提取多个PIN码！

`pin_extractor` 函数已完成。

# --hints--

函数应该在末尾返回 `secret_codes`。
...
你应该有函数调用 `print(pin_extractor([poem, poem2, poem3]))`。
