...
下一步是添加负责在不同行星及其事实之间切换的按钮。

在 `tablist` 元素内部，添加三个 `button` 元素，每个元素的 `role` 属性都设置为 `"tab"`。第一个 `button` 元素应该有文本 `🌍 Earth`。第二个 `button` 应该有文本 `🪐 Saturn`。第三个 `button` 元素应该有文本 `🔴 Mars`。

# --hints--

你应该在 `tablist` 元素内部有三个 `button` 元素。
...
你的每个 `button` 元素都应该有一个设置为 `"tab"` 的 `role` 属性。
