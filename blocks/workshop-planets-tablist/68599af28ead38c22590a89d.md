...
需要有一种方法将按钮与其所代表的内容连接起来。这是通过在每个按钮上使用 `aria-controls` 属性来完成的。稍后，你将添加内容和相应的内容元素 `id` 属性。

首先为每个 `button` 元素添加 `aria-controls` 属性。第一个按钮的 `aria-controls` 属性值应该设置为 `"panel-earth"`。第二个按钮的 `aria-controls` 属性应该设置为 `"panel-saturn"`。第三个按钮的 `aria-controls` 属性应该设置为 `"panel-mars"`。

# --hints--

你的每个 `button` 元素都应该有一个 `aria-controls` 属性。
...
第一个 `button` 元素应该有一个设置为 `"panel-earth"` 的 `aria-controls` 属性。
...
第二个 `button` 元素应该有一个设置为 `"panel-saturn"` 的 `aria-controls` 属性。
...
第三个 `button` 元素应该有一个设置为 `"panel-mars"` 的 `aria-controls` 属性。
