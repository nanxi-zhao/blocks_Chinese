...
默认情况下，其他选项卡不应该被选中。你可以通过向第二个和第三个 `button` 元素添加 `aria-selected` 属性并将它们的值设置为 `"false"` 来实现这一点。

# --hints--

你的第二个 `button` 元素应该有一个 `aria-selected` 属性。
...
你的第二个 `button` 元素应该有一个设置为 `"false"` 的 `aria-selected` 属性。
...
你的第三个 `button` 元素应该有一个 `aria-selected` 属性。
...
你的第三个 `button` 元素应该有一个设置为 `"false"` 的 `aria-selected` 属性。
