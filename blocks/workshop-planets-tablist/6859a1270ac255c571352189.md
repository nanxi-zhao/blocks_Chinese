...
现在是时候为每个面板创建标记了，这些面板将显示有关每个行星的事实。

首先添加三个 `div` 元素，每个元素的 `role` 属性都设置为 `"tabpanel"`。在每个 `div` 元素内部，应该有一个 `p` 元素。

# --hints--

你应该有三个 `role` 属性设置为 `"tabpanel"` 的 `div` 元素。

```js
assert.lengthOf(document.querySelectorAll('div[role="tabpanel"]'), 3);
```

每个 `tabpanel` 元素内部应该有一个 `p` 元素。
