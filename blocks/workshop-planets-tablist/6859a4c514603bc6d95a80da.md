...
现在是时候将选项卡面板与其各自的选项卡关联起来了。

对于第一个 `tabpanel` 元素，添加值为 `"panel-earth"` 的 `id` 属性和值为 `"tab-earth"` 的 `aria-labelledby` 属性。

# --hints--

你的第一个 `tabpanel` 元素应该有一个 `id` 属性。

```js
assert.isTrue(document.querySelector('[role="tabpanel"]').hasAttribute("id"));
```

你的 `id` 属性应该有值 `"panel-earth"`。

```js
assert.equal(document.querySelector('[role="tabpanel"]').getAttribute("id"), "panel-earth");
```

你的第一个 `tabpanel` 元素应该有一个 `aria-labelledby` 属性。

```js
assert.isTrue(document.querySelector('[role="tabpanel"]').hasAttribute("aria-labelledby"));
```

你的 `aria-labelledby` 属性应该有值 `"tab-earth"`。
