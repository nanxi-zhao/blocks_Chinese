...
如果你查看你的 HTML，第一个 `tab` 默认已经被选中，并且其 `aria-selected` 属性设置为 `"true"`。当用户点击不同的选项卡时，所有选项卡应该首先设置为 `aria-selected="false"`。

为了实现这一点，你可以使用 `setAttribute` 方法来更改每个选项卡的 `aria-selected` 属性。

以下是在一组 `div` 元素上使用 `setAttribute` 方法的示例：
...
在 `tabs` 集合上使用 `forEach` 来循环遍历每个选项卡，并将其 `aria-selected` 属性设置为 `"false"`。对于回调函数，你可以使用任何你喜欢的参数名，但最好不要使用 `tab`，因为它已经在外部的 `forEach` 循环中使用了。

# --hints--

当用户点击一个选项卡时，所有选项卡的 `aria-selected` 属性都应该设置为 `"false"`。
