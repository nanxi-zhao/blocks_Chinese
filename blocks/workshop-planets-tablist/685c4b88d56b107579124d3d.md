...
如果你查看 HTML，你会看到第二个和第三个面板是隐藏的。这是通过使用 `hidden` 属性完成的。第一个面板默认是可见的。当用户点击一个选项卡时，所有面板都应该被隐藏。

使用 `forEach` 方法遍历 `panels` 并将每个面板的 `hidden` 属性设置为 `true`。

# --hints--

你应该遍历 `panels` 并将每个面板的 `hidden` 属性设置为 `true`。
