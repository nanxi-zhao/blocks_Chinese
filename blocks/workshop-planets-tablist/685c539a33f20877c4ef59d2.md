...
现在是时候处理当点击选项卡时显示正确面板的问题了。每个面板都有一个 `id`，它与相应选项卡的 `aria-controls` 属性匹配。例如，第一个面板的 `id` 是 `panel-earth`，它与第一个选项卡的 `aria-controls` 属性值 `panel-earth` 相匹配。

创建一个名为 `associatedPanel` 的变量，并将其赋值为 `tab.getAttribute("aria-controls")`。这将给你提供与被点击选项卡相对应的面板的 `id`。

# --hints--

你应该有一个名为 `associatedPanel` 的变量，它保存与被点击选项卡相关联的面板的 `id`。
