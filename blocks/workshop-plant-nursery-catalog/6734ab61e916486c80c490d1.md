...
当与键关联的值是一个对象时，`get`方法将返回对该对象的引用。你可以使用该引用来修改map中的对象。

为了测试这一点，将`getBallerina`的`small`属性设置为一个不同于`20`的整数。然后，查看浏览器控制台，检查目录中的值是否已更改。

# --hints--

你应该将`getBallerina`的`small`属性设置为一个不同于`20`的整数。
