...
在这一步中，为 [recipeForm](file:///e:/1.work/byy/%E5%9C%A8%E7%BA%BF%E8%AF%BE%E7%A8%8B/blocks/node_modules/@types/node/stream.d.ts#L53-L53) 附加一个 `submit` 事件监听器，这样当点击 `Convert` 按钮时，会调用 [updateResultsList](file:///e:/1.work/byy/%E5%9C%A8%E7%BA%BF%E8%AF%BE%E7%A8%8B/blocks/node_modules/@types/node/stream.d.ts#L53-L53) 函数。

为 [recipeForm](file:///e:/1.work/byy/%E5%9C%A8%E7%BA%BF%E8%AF%BE%E7%A8%8B/blocks/node_modules/@types/node/stream.d.ts#L53-L53) 附加一个 `submit` 事件监听器。当表单提交时，调用 `event.preventDefault()` 来阻止默认行为，然后调用 [updateResultsList](file:///e:/1.work/byy/%E5%9C%A8%E7%BA%BF%E8%AF%BE%E7%A8%8B/blocks/node_modules/@types/node/stream.d.ts#L53-L53)。

这将允许你通过 UI 触发函数，通过填写表单并按下 `Convert` 来调试，同时在下一步中添加转换逻辑。

# --hints--

通过为 [recipeForm](file:///e:/1.work/byy/%E5%9C%A8%E7%BA%BF%E8%AF%BE%E7%A8%8B/blocks/node_modules/@types/node/stream.d.ts#L53-L53) 添加一个 `"submit"` 事件监听器，使点击 `Convert` 按钮运行 [updateResultsList](file:///e:/1.work/byy/%E5%9C%A8%E7%BA%BF%E8%AF%BE%E7%A8%8B/blocks/node_modules/@types/node/stream.d.ts#L53-L53)，该监听器调用 `event.preventDefault()` 来阻止默认的表单行为。
