...
通过设置`display`为`block`，提交按钮紧贴其父元素的左边缘。

使用与居中[form](file:///E:/1.work/byy/%E5%9C%A8%E7%BA%BF%E8%AF%BE%E7%A8%8B/blocks/blocks_english/workshop-registration-form/60fad99e09f9d30c1657e790.md#L59-L93)元素相同的技术来居中提交按钮。

# --hints--

你应该给提交按钮一个`margin`为`0 auto`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('input[type="submit"]')?.margin, '0px auto');
```

你不应该给提交按钮设置`min-width`或`max-width`。
