...
最后，对于提交按钮，你想要将它与上面的[fieldset](file:///E:/1.work/byy/%E5%9C%A8%E7%BA%BF%E8%AF%BE%E7%A8%8B/blocks/blocks_english/workshop-registration-form/60ffe9cb47809106eda2f2c9.md#L56-L71)分开，并调整其宽度以确保不低于`300px`。

将`margin`属性更改为在顶部和底部包含`1em`，而左右边距保持为`auto`。然后按上述要求设置宽度。

# --hints--

你不应该更改`width`属性。使用`min-width`属性。
...
你应该使用现有的`margin`属性，在顶部和底部包含`1em`。
