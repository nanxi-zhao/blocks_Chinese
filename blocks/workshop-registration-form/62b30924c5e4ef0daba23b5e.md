...
最后一个 `fieldset` 元素的边框看起来有些不协调。

在之前的课程中，你已经学会了如何使用 `last-of-type` CSS 伪类，就像这样：
...
创建一个新选择器，目标是最后一个 `fieldset` 元素，并将其 `border-bottom` 设置为 `none`。

# --hints--

你可以使用 `:last-of-type` 伪类来选择特定类型的最后一个元素。
