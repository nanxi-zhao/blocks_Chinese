...
在第三个列表项内，创建一个 `button` 元素，其 `aria-expanded` 属性设置为 `false`。同时，将 `button` 的文本设置为 `Apps`。

`aria-expanded` 属性将向屏幕阅读器用户传达该按钮触发了一个可展开的子菜单。

# --hints--

你应该在第二个列表项内创建一个 `button` 元素。

```js
assert.exists(document.querySelector('li button'));
```

你的 `button` 元素应该有一个 `aria-expanded` 属性，其值设置为 `false`。

```js
assert.equal(document.querySelector('button')?.getAttribute('aria-expanded'), 'false');
```

你的 `button` 元素应该有文本 `Apps`。
