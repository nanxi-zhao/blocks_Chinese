...
# --description--
回顾一下之前学过的用 `map` 来动态渲染列表的模式，我们这里也要把它用起来。示例（仅供回忆，不必复制）：

```jsx
// 示例：用 fruits.map 渲染多个 Card
```

请使用 `profiles.map(...)` 来渲染数组里的那一条数据，传入 `name`、`title`、`bio` 三个对应的属性给 `Card`。

这是迈向组件可扩展性的关键一步，你做得很棒！🌟

# --hints--
需要使用 `profiles.map(...)` 来遍历渲染。

```js
assert.match(code, /profiles\.map\s*\(/);
```

渲染时需传入 `profile` 的 `name`、`title`、`bio`。
