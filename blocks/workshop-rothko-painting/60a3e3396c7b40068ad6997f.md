---
id: 60a3e3396c7b40068ad6997f
title: 步骤 21
challengeType: 0
dashedName: step-21
---

# --description--

在画布的上下左右各加了 1 像素的 `padding` 后，画布的尺寸变成了 502px x 602px。

把 `.canvas` 上的 `padding` 移除，并将 `overflow` 设置为 `hidden`，画布尺寸就会恢复到原来的大小。加油，你很接近完成了！

# --hints--

请从 `.canvas` 选择器中移除 `padding` 属性。

```js
const canvasPadding = new __helpers.CSSHelp(document).getStyle('.canvas').getPropertyValue('padding');
assert(!canvasPadding);
```

接着把 `overflow` 设置为 `hidden`。

```js
const hasOverflow = new __helpers.CSSHelp(document).getCSSRules().some(x => x.style.overflow === 'hidden');
assert(hasOverflow);
```

`.canvas` 的 `overflow` 应为 `hidden`。

```js
const canvasOverflow = new __helpers.CSSHelp(document).getStyle('.canvas')?.getPropertyValue('overflow');
assert(canvasOverflow === 'hidden')
```

# --seed--

## --seed-contents--

```css
.canvas {
  width: 500px;
  height: 600px;
  background-color: #4d0f00;
--fcc-editable-region--
  padding: 1px;
--fcc-editable-region--
}

.frame {
  border: 50px solid black;
  width: 500px;
  padding: 50px;
  margin: 20px auto;
}

.one {
  width: 425px;
  height: 150px;
  background-color: #efb762;
  margin: 20px auto;
}
```

```html
<!DOCTYPE html>
<html lang="en">
  <head>
    <meta charset="UTF-8">
    <title>Rothko Painting</title>
    <link href="./styles.css" rel="stylesheet">
  </head>
  <body>
    <div class="frame">
      <div class="canvas">
        <div class="one"></div>
      </div>
    </div>
  </body>
</html>
```
