...
在这一步中，你将专注于确定玩家是否赢得了这一轮。

创建一个 `hasPlayerWonTheRound` 函数，它有两个参数 `playerChoice` 和 `computerChoice`。这些参数将接受 `"Rock"`、`"Paper"` 或 `"Scissors"` 的字符串值，代表玩家和电脑的选择。

如果玩家赢得了这一轮，函数应返回 `true`，如果玩家输了或平局则返回 `false`。

以下是玩家赢得一轮的条件：

- 玩家选择 `"Rock"`，电脑选择 `"Scissors"`
- 玩家选择 `"Scissors"`，电脑选择 `"Paper"`
- 玩家选择 `"Paper"`，电脑选择 `"Rock"`
...
你的函数应该有两个参数 `playerChoice` 和 `computerChoice`。
...
如果玩家选择 `"Rock"`，电脑选择 `"Scissors"`，你的 `hasPlayerWonTheRound` 函数应该返回 `true`。

```js
assert.isTrue(hasPlayerWonTheRound("Rock", "Scissors"));
```

如果玩家选择 `"Scissors"`，电脑选择 `"Paper"`，你的 `hasPlayerWonTheRound` 函数应该返回 `true`。

```js
assert.isTrue(hasPlayerWonTheRound("Scissors", "Paper"));
```

如果玩家选择 `"Paper"`，电脑选择 `"Rock"`，你的 `hasPlayerWonTheRound` 函数应该返回 `true`。

```js
assert.isTrue(hasPlayerWonTheRound("Paper", "Rock"));
```

如果玩家和电脑选择了相同的选项，你的 `hasPlayerWonTheRound` 函数应该返回 `false`。
...
如果电脑赢得了这一轮，你的 `hasPlayerWonTheRound` 函数应该返回 `false`。
