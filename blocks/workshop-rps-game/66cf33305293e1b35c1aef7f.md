...
如果电脑和玩家选择了相同的选项，返回消息 `"It's a tie! Both chose [player's choice]"`。

如果电脑赢得这一轮，将 `computerScore` 增加 `1` 并返回消息 `"Computer wins! [computer's choice] beats [player's choice]"`。
...
你的 `getRoundResults` 函数应该根据谁赢得这一轮返回正确的消息。如果没有人赢，消息应该说是平局。
