...
创建一个名为 `showResults` 的函数，它有一个参数 `userOption`。

在你的 `showResults` 函数中，`roundResultsMsg` 应该用回合的结果来更新。

然后，`playerScoreSpanElement` 和 `computerScoreSpanElement` 也应该更新，以显示玩家和电脑的更新分数。

记住，这里的顺序很重要。你需要先更新 `roundResultsMsg`，然后是 `playerScoreSpanElement`，最后是 `computerScoreSpanElement`，因为 `roundResultsMsg` 将用于确定分数。
...
你的 `showResults` 函数应该用回合的结果更新 `roundResultsMsg`。
...
你的 `showResults` 函数应该更新 `computerScoreSpanElement` 以显示电脑的更新分数。
...
你的 `showResults` 函数应该更新 `playerScoreSpanElement` 以显示玩家的更新分数。
