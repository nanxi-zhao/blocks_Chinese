...
如果你尝试玩游戏，你会看到你可以无限轮地玩下去。但规则规定，先得三分的人获胜。你想要检查是否有获胜者，并显示一条消息。

在你的 `showResults` 函数中，如果玩家达到了三分，将 `winnerMsgElement` 更新为 `"Player has won the game!"`。如果电脑达到了三分，将 `winnerMsgElement` 更新为 `"Computer has won the game!"`。

如果有获胜者，通过将 `resetGameBtn` 按钮的 `display` 设置为 `block` 来显示它，并通过将 `optionsContainer` 的 `display` 设置为 `none` 来隐藏它。

现在，尝试玩游戏，看看当玩家达到三分时是否显示获胜者消息。
...
如果玩家或电脑达到了三分，你应该隐藏 `optionsContainer`。
...
如果玩家或电脑达到了三分，你应该显示 `resetGameBtn` 按钮。
