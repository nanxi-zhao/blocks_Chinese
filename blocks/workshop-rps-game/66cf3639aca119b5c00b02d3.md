...
对于工作坊的最后一步，你需要构建重置游戏功能。

创建一个 `resetGame` 函数，完成以下任务：

- 将玩家和电脑的分数重置为 `0`。
- 更新 `playerScoreSpanElement` 和 `computerScoreSpanElement` 以显示新分数。
- 隐藏 `resetGameBtn` 按钮。
- 显示 `optionsContainer`，以便玩家可以再次游戏。
- 清除 `winnerMsgElement` 和 `roundResultsMsg` 元素的内容。

尝试通过玩几轮直到其中一个玩家达到 `3` 分来测试游戏。然后点击 `"Play again?"` 按钮，看看游戏是否正确重置。

通过这最后一步，你就完成了石头剪刀布游戏！
