...
对于工作坊的最后一部分，你将专注于构建获胜消息和重置按钮功能。

首先创建一个名为 `winnerMsgElement` 的变量，它将存储具有 id `winner-msg` 的元素。

然后，创建一个名为 `optionsContainer` 的变量，它将存储具有 `options-container` 类的元素。

最后，创建一个名为 `resetGameBtn` 的变量，它将存储具有 id `reset-game-btn` 的元素。
...
你的 `winnerMsgElement` 变量应该存储具有 id `winner-msg` 的元素。
...
你的 `optionsContainer` 变量应该存储具有 `options-container` 类的元素。
...
你的 `resetGameBtn` 变量应该存储具有 id `reset-game-btn` 的元素。
