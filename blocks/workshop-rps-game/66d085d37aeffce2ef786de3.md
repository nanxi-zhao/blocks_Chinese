...
如果玩家或电脑赢得了游戏，应该有一个选项来重置游戏并再次玩。

为 `resetGameBtn` 按钮添加一个事件监听器。你的事件监听器应该接受一个 `"click"` 事件和一个指向 `resetGame` 函数的引用。
...
你应该为 `resetGameBtn` 按钮添加一个事件监听器。事件监听器应该接受一个 `"click"` 事件和一个指向 `resetGame` 函数的引用。
