...
# --description--
最后再加上一层“晋升方向守护”：只有向上或平级验证（平级已被前一步禁止），拒绝降级。

新增第三个 `if`：当新等级的基础薪资 < 当前等级基础薪资时，抛出 `ValueError`，消息内容为 `Cannot change to lower level.`。这让等级变更逻辑更专业、更安全！

# --hints--
你需要在 `level` setter 中加入第三个 `if` 判断处理“降级”场景。

```js
({ test: () => assert(runPython(`_Node(_code).find_class("Employee").find_functions("level")[1].find_ifs()[2]`)) })
```

当新等级对应基础薪资更低时，应抛出 `ValueError`，提示不允许降级。
