...
# --description--
接下来轮到 `salary` 属性拥有自己的守护者啦：setter！

在现有 getter 之后，写一个 `@salary.setter` 方法，把传入的值直接赋给 `self._salary`，然后打印：`Salary updated to $<新值>.`。

此刻我们暂时不做验证，后面几步会慢慢增强它的“防御力”，先把结构搭好就已经很棒了！

# --hints--
`Employee` 类需要增加一个用 `@salary.setter` 装饰的 `salary` 方法。

```js
({ test: () => assert(runPython(`_Node(_code).find_class("Employee").find_functions("salary")[1].has_decorators("salary.setter")`)) })
```

`salary` setter 要把传入的值赋给 `self._salary`。
...
`salary` setter 需要打印更新提示：`Salary updated to $<新值>`。
