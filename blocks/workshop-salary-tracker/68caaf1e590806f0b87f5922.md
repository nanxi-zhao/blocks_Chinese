...
# --description--
现在让 `salary` setter 获得第一层验证护甲：

在方法开头加入一个 `if` 判断：若 `new_salary` 不是整数也不是浮点数，就抛出 `TypeError`，信息为 `'salary' must be a number.`。这样你的数据类型就更可靠了！

# --hints--
`salary` setter 内需要一个类型检查的 `if`。

```js
({ test: () => assert(runPython(`_Node(_code).find_class("Employee").find_functions("salary")[1].find_ifs()[0]`)) })
```

当类型不符合要求时应抛出 `TypeError`，提示 `'salary' must be a number.`。
