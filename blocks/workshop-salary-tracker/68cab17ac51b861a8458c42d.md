...
# --description--
最后的收尾冲刺来了！既然 `salary` setter 已经具备全部防护与输出能力，我们就让 `level` setter 使用它，而不是直接改底层属性。

把 `self._salary = Employee._base_salaries[new_level]` 改成通过 `self.salary = Employee._base_salaries[new_level]` 来调用 setter。这样所有薪资变更路径都统一、可控、可跟踪。

完成这一改动，你的薪资追踪器工作坊就圆满收官啦！恭喜你坚持到这里，超级棒！🎉

# --hints--
需要在 `level` setter 中通过 `self.salary = Employee._base_salaries[new_level]` 调用 setter 完成更新。
