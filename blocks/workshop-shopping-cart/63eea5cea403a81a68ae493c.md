...
```js
assert.exists(document.querySelectorAll('.product')[1]?.querySelector('button'));
```
...
```js
assert.equal(document.querySelectorAll('.product')[1]?.querySelector('button')?.textContent, '添加到购物车');
```
