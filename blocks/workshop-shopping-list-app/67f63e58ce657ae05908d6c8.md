...
在这个工作坊中，你将构建一个简单的购物清单应用程序，以练习在 React 中使用 `useMemo()` 和 `useCallback()` 钩子。你将学习如何使用这些钩子通过记忆化潜在的昂贵计算和函数来优化应用程序的性能。

所有你需要的基本 HTML 和 CSS 都已为你提供，还包括一个名为 `ShoppingList` 的 React 组件的基本结构。

在 `ShoppingList` 组件中，现在返回一对空的圆括号。
