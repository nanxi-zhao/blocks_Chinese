...
接下来，你需要创建一个状态变量、一个设置函数，并使用 `useState()` 钩子设置初始状态来存储用户的输入。

提醒一下，以下是创建名为 `age` 的状态变量、名为 `setAge` 的更新函数，并使用 `useState()` 钩子将 `age` 初始化为数字 `0` 的方法：

```js
const [age, setAge] = useState(0);
```

在 `ShoppingList` 组件的顶部，使用 `useState()` 钩子创建一个名为 `query` 的状态变量和一个名为 `setQuery` 的更新函数。将 `query` 初始化为空字符串。这将用于存储用户的搜索输入。
