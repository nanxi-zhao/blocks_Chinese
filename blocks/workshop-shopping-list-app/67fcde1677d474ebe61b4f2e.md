...
在输入元素中，添加一个 `value` 属性并将其设置为 `query` 状态变量。这将允许输入字段显示 `query` 的当前值。另外，记住你需要使用花括号 (`{}`) 来在 JSX 中嵌入像 `query` 这样的 JavaScript 表达式。

然后，给输入元素添加一个 `onChange` 事件处理程序。传递一个以 `e` 为参数的匿名函数，`e` 是事件对象。在匿名函数内部，调用 `setQuery()` 并传递 `e.target.value`。这将在用户在输入字段中输入时，使用输入字段的当前值更新 `query` 状态变量。
