...
虽然你还没有真正过滤任何东西，而且 `filteredItems` 与 `items` 数组匹配，但能在页面上看到项目列表会很好。

首先，在 `form` 中的 `p` 元素下方添加一个 `ul` 元素。在无序列表内部，使用 `map()` 方法遍历 `filteredItems` 数组，并向其传递一个匿名函数。匿名函数应该以 `item` 为参数，并为每个项目返回一个 `li` 元素。每个 `li` 应该显示 `item` 文本。
