...
搜索输入应该不区分大小写，还应该匹配部分字符串。例如，如果用户输入 `app`，它应该匹配 `Apples` 和 `Pineapples`。这是因为 `app` 的小写版本是 `Apples` 和 `Pineapples` 小写版本的子字符串。

首先，在 `filter()` 方法内部，使用 `toLowerCase()` 方法将 `item` 转换为小写字符串。
