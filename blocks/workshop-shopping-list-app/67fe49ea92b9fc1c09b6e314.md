...
接下来，使用 `includes()` 方法检查 `item` 的小写版本是否包含 `query` 的小写版本作为子字符串。

例如，以下是使用 `toLowerCase()` 和 `includes()` 方法检查字符串 `MasterPuti` 是否包含 `code` 的方法：

```js
const str1 = "MasterPuti";
const str2 = "code";
str1.toLowerCase().includes(str2.toLowerCase()); // true
```

将 `includes()` 方法链接到小写的 `item` 上，并检查它是否包含小写的 `query`。记住也要将 `query` 转换为小写。
