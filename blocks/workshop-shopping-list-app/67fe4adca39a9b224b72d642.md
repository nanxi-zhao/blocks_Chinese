...
现在搜索输入应该按预期工作，并根据用户的输入过滤购物清单。

接下来，我们将添加一种方法来勾选列表中的项目，这样用户就可以跟踪他们已经购买的物品。

为此，我们需要另一个状态变量和一个设置函数。使用 `useState` 钩子创建一个名为 `selectedItems` 的状态变量和一个名为 `setSelectedItems` 的设置函数。将 `selectedItems` 初始化为空数组。这个数组将存储用户选择的项目，我们稍后将对这些选中的项目应用一些样式。
