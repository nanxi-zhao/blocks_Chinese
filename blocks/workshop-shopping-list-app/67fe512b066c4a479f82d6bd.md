...
在 `li` 元素内，`item` 文本上方，添加一个 `input` 元素，将 `type` 属性设置为 `checkbox`。同时，将其 `onChange` 属性设置为一个匿名函数，该函数调用 `toggleItem()` 并传递 `item` 作为参数。

最后，将 `input` 和 `item` 文本包装在 `label` 元素中以获得更好的可访问性。
