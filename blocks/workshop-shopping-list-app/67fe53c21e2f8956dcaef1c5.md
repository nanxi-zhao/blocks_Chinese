...
如果你还记得之前课程的内容，你可以在 React 中比较过去和现在的状态，并使用它来确定即将到来的状态。在这种情况下，你将使用过去和现在的状态来从 `selectedItems` 数组中添加或删除项目。

首先，调用 `setSelectedItems()` 函数，并向其传递一个匿名函数作为参数。这个匿名函数应该以 `prev` 作为参数，`prev` 是 `selectedItems` 的先前状态。

在匿名函数内部，使用 `includes` 方法检查 `prev` 是否包含 `item` 并返回检查结果。
