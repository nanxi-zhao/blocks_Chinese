...
在 `filteredItems.map()` 调用中的 `return` 语句之前，创建一个名为 `isChecked` 的变量，并使用 `includes()` 方法检查当前 `item` 是否在 `selectedItems` 数组中。

然后，给 `li` 元素添加一个 `style` 属性，并使用三元运算符在 `isChecked` 为真时将 `textDecoration` 属性设置为 `line-through`，为假时设置为 `none`。记住，由于这是一个 JavaScript 表达式，你需要将三元运算符包装在花括号中。

例如，以下是如何在简单组件中使用三元运算符来有条件地设置段落颜色的方法：
