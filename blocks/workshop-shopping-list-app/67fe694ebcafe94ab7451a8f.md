...
你的应用程序可以正常工作了！继续测试一下。

但是，它的效率还可以更高。虽然 React 已经非常高效，但在某些情况下，缓存潜在昂贵计算的结果，或确保函数不会在每次渲染时重新创建会更好。

我们将在接下来的步骤中提高应用程序的性能。但首先，让我们添加一些日志记录，这样你就能更清楚地看到这个组件的生命周期。

在 `filteredItems` 上方，添加一个 `console.log()` 语句，将字符串 `Filtering items...` 记录到控制台。
