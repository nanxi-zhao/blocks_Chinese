...
现在，每次你在搜索栏中输入内容或勾选/取消勾选项目时，你都会在控制台中看到 `Filtering items...`。这是因为这些操作会更新组件的状态，从而导致组件重新渲染。

这通常不是问题。但是，如果你的列表中有很多项目，或者你正在从 API 获取大量数据并对其进行操作，你的应用程序可能会感觉很慢。

为了提高性能，你可以使用 `useMemo()` 钩子来记忆化，换句话说就是缓存过滤操作的结果。然后，React 只会在其依赖项发生变化时重新运行过滤操作，而不是在每次渲染时都运行。

首先，在文件顶部从 React 中解构出 `useMemo()` 钩子。
