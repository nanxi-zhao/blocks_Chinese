...
`useMemo()` 接受两个参数：一个返回你想要记忆化值的函数，以及一个依赖项数组。只有当其中一个或多个依赖项发生变化时，才会重新计算记忆化的值。
...
将 `filteredItems` 设置为等于 `useMemo()`，并向其传递一个带花括号的匿名函数。在花括号内部，使用 `return` 语句显式地返回你现有的过滤逻辑。同时，在依赖项数组中添加 `query` 作为唯一的依赖项。这确保了只有当 `query` 发生变化时，过滤操作才会重新运行。
