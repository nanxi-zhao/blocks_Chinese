...
`useCallback()` 钩子接受两个参数：一个函数和一个依赖项数组。只有当其中一个依赖项发生变化时，该函数才会被重新创建。
...
将 `toggleItem` 设置为等于 `useCallback()`，并向其传递一个接受 `item` 作为参数的匿名函数。将你现有的 `toggleItem()` 函数逻辑移动到匿名函数内部，在调用 `setSelectedItems` 的地方。最后，将 `setSelectedItems` 添加到依赖项数组中。

完成这些操作后，你的优化就完成了！你的应用程序应该仍然以相同的方式工作，但你只会看到当 `query` 状态发生变化或 `toggleItem()` 函数被重新创建时的控制台日志。
