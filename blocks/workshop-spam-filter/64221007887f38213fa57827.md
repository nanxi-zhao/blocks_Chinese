...
关于这个表达式的最后一件事。你实际上并不需要捕获组中的匹配值，所以你可以将其转换为<dfn>非捕获组</dfn>。这将允许你将字符组合在一起而不保留结果。

要在正则表达式中创建非捕获组，你可以在组的左括号后添加 `?:`。例如，`(?:a|b)` 将匹配 `a` 或 `b`，但它不会捕获结果。太棒了！
...
你的 `dollarRegex` 应该仍然匹配 `10hundred dollars`。
...
你的 `dollarRegex` 应该仍然匹配 `10 hundred  dollars`。
