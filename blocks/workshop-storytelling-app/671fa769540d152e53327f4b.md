...
# --description--
选择 `#scary-btn`、`#funny-btn` 和 `#adventure-btn` 这三位按钮伙伴，并分别存到变量 `scaryStoryBtn`、`funnyStoryBtn`、`adventureStoryBtn` 中。这样后面给它们加事件的时候就超顺手啦。继续走，你已进入“熟练操作”阶段～

# --hints--
你应该用 `getElementById` 选择 `#scary-btn`，并存入变量 `scaryStoryBtn`。
...
你应该用 `getElementById` 选择 `#funny-btn`，并存入变量 `funnyStoryBtn`。
...
你应该用 `getElementById` 选择 `#adventure-btn`，并存入变量 `adventureStoryBtn`。
