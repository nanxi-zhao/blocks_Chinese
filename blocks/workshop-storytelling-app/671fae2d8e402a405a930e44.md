...
# --description--
给 `scaryStoryBtn` 添加一个 `click` 事件监听器，并把 `displayStory` 作为回调传进去。然后试着点击按钮，看看控制台是否输出测试文字。验证成功就说明事件绑定没问题～继续加油！

# --hints--
你需要在 `scaryStoryBtn` 上添加 `click` 事件监听，并传入 `displayStory` 作为处理函数，点击后要出现测试日志。
