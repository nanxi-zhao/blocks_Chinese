...
创建一个名为`exampleSentence` 的新变量，并赋值为 `"I love cats and cats are so much fun!"`。
...
你应该创建一个`exampleSentence` 变量，让它闪亮登场。
...
你需要将字符串`"I love cats and cats are so much fun!"` 赋值给 `exampleSentence`。

```js
assert.equal(exampleSentence, "I love cats and cats are so much fun!");
```

你需要在控制台输出`"Original sentence:"` 作为标签。

```js
assert.equal(getLogs()[4], "Original sentence:");
```

你还需要输出`exampleSentence` 的内容，看看它的初始状态。继续冲鸭！
