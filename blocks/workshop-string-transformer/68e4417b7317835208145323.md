...
继续前进：创建一个名为`dogsOnlySentence` 的变量，并赋值为 `exampleSentence.replace("cats", "dogs")`。
...
你需要在控制台输出`"Replacing all occurrences of cats with dogs:"`。

```js
assert.equal(getLogs()[6], "Replacing all occurrences of cats with dogs:");
```

你还需要输出`dogsOnlySentence` 的内容，观察当前的替换效果。马上就要完美啦。
