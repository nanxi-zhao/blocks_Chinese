...
你可能注意到目前无法在该输入框中输入；打开控制台会看到一个警告。

为了解决它，给 `input` 添加 `onChange` 事件处理，使用一个以 `e` 为参数的箭头函数，隐式返回 `setHeroName(e.target.value)` 来更新 `heroName`。

# --hints--

你的 `input` 元素需要有一个 `onChange` 属性，太棒了！

```js
assert.match(code, /onChange\s*=\s*\{/)
```

你需要使用箭头函数的 `onChange` 来隐式设置 `heroName` 的值，继续加油！
