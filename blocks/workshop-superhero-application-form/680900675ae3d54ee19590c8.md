...
接下来，创建一个 `label`，`className` 为 `section column`，文本为 `How did you get your powers?`。在该 `label` 内部创建一个 `select` 元素。

# --hints--

你需要创建一个带有 `section column` 类名的 `label` 元素，太棒了！
...
你的 `label` 文本应为 `How did you get your powers?`，继续加油！
...
你需要在该 `label` 内创建一个 `select` 元素，做得很好！
