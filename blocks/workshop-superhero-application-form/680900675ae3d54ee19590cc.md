...
`powerSourceOptions` 的值现在已经正确渲染为下拉选项。

接下来，将 `powerSource` 作为 `select` 的 `value`，并添加一个以 `e` 为参数的箭头函数作为 `onChange`，隐式返回 `setPowerSource(e.target.value)` 来更新状态。

# --hints--

你需要使用 `{powerSource}` 作为 `select` 的值，太棒了！

```js
assert.match(code, /<\s*select\s+[\s\S]*value\s*=\s*\{\s*powerSource\s*\}/)
```

你需要添加箭头函数的 `onChange` 来隐式调用 `setPowerSource(e.target.value)`，继续加油！
