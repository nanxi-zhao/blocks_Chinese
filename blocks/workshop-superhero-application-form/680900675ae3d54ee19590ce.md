...
将会有六个复选框，因此最好使用一个数组并通过遍历来生成它们。我们已经为你提供了 `powersOptions` 数组。

使用箭头函数遍历 `powersOptions`，参数命名为 `power`。在 `map` 中创建一个 `key={power}` 的 `label`，暂时文本用 `Hello` 占位。

完成后你会看到六个 `Hello`，下一步再替换成真正的内容。

# --hints--

你需要遍历 `powersOptions`，太棒了！

```js
assert.match(code, /\{\s*powersOptions\s*\.\s*map\s*\(/)
```

你需要使用 `power` 作为 `map` 的参数，继续加油！

```js
assert.match(code, /{\s*powersOptions\s*\.\s*map\s*\(\s*(power|\(\s*power\s*\))\s*=>/)
```

你需要创建一个带 `key={power}` 的 `label` 元素，做得很好！

```js
assert.match(code, /\{\s*powersOptions\s*\.\s*map\s*\([\s\S]*<\s*label\s+key\s*=\s*\{\s*power\s*\}\s*>[\s\S]*<\s*\/\s*label\s*>/)
```

这些占位 `label` 的文本应为 `Hello`，你真聪明！
