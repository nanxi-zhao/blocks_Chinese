...
要选中复选框，需要给对应的 `input` 添加 `checked` 属性，并使用 `includes` 方法判断当前 `power` 是否在 `powers` 数组中。

将 `onChange` 指向一个名为 `handlePowersChange` 的函数，目前它尚未定义，会报错，下一步你将修复它。

# --hints--

你需要设置复选框的 `checked` 状态以反映当前 `power` 是否存在于 `powers` 中，太棒了！

```js
assert.match(code, /checked\s*=\s*\{\s*powers\s*\.\s*includes\s*\(\s*power\s*\)\s*\}/)
```

你需要将复选框的 `onChange` 属性设置为 `{handlePowersChange}`，继续加油！
