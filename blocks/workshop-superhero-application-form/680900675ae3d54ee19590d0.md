...
为修复前一步的错误，创建一个带 `e` 参数的 `handlePowersChange` 箭头函数。

在函数内部，从 `e.target` 解构出 `value` 与 `checked`，以获取复选框的值与选中状态。

# --hints--

你需要创建一个带 `e` 参数的 `handlePowersChange` 函数，太棒了！

```js
assert.match(code, /(const|let|var)\s+handlePowersChange\s*=\s*(e|\(\s*e\s*\))\s*=>\s*{/)
```

你需要从 `e.target` 解构出 `value` 与 `checked`，继续加油！
