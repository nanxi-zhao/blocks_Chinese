...
接下来需要告诉表单如何提交：为 `form` 添加 `method='post'` 和 `action='https://superhero-application-form.MasterPuti.org'`。

这样提交时就会发送到该地址，下一步你将进行实际提交测试。

# --hints--

你的 `form` 元素应具有 `method='post'`，太棒了！
...
你的 `form` 元素应具有 `action='https://superhero-application-form.MasterPuti.org'`，继续加油！
