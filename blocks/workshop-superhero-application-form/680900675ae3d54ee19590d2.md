...
创建一个 `className='submit-btn'`、`type='submit'` 的 `button`，文本设为 `Join the League`。

完成后尝试提交表单，观察效果。

# --hints--

你需要创建一个 `button` 元素，太棒了！

```js
assert.exists(document.querySelector("button"));
```

你的按钮应具有 `className='submit-btn'`，继续加油！

```js
assert.exists(document.querySelector("button.submit-btn"));
```

你的按钮文本应为 `Join the League`，做得很好！
