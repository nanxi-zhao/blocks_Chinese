...
你可能注意到当前即使未填写任何内容也能提交表单。

为避免空提交，为按钮添加 `disabled`：当 `heroName`、`realName` 或 `powerSource` 为空，或 `powers.length === 0` 时禁用提交按钮。

现在只有全部信息填写后才能提交。试着完成表单并点击提交加入联盟吧！
...
你的 `button` 元素需要具有一个 `disabled` 属性，继续加油！

```js
assert.match(code, /<\s*button\s+className\s*=\s*('|")\s*submit-btn\s*\1\s*type\s*=\s*('|")\s*submit\s*\2\s*disabled/)
```

如果 `heroName`、`realName` 或 `powerSource` 任一为空，或 `powers` 长度为 `0`，应禁用按钮，做得很好！
