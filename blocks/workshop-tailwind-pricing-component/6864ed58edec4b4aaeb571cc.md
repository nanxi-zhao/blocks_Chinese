...
现在处理 **Listener** 方案里的内部 `div`。给它加上 `grid`、`grid-rows-[auto_auto_auto_1fr]` 以及 `gap-y-2`。

这样它成为网格容器：前三行高度自适应，最后一行填满剩余空间；行间距 `0.5rem`，结构更整齐～
...
内部 `div` 需要包含类 `grid-rows-[auto_auto_auto_1fr]`。
