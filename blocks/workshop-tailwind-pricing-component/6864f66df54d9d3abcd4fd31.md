...
再来修饰价格后面的周期说明 `/month`：给那个 `span` 添加 `text-base`、`font-medium`、`text-gray-500`，让它比主价格更轻一点，层次更分明～

# --hints--

该 `/month` 的 `span` 需要包含类 `text-base`。
...
该 `/month` 的 `span` 需要包含类 `font-medium`。
...
该 `/month` 的 `span` 需要包含类 `text-gray-500`。
