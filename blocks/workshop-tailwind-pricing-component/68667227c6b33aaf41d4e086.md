...
为了排布 Premium 卡片内部内容，再次使用网格：给外层 `div` 添加 `grid`、`grid-rows-[1fr_auto]`、`gap-6`。

再加上 `rounded-xl` 去掉直角，让卡片更精致～

# --hints--

Premium 外层 `div` 需要类 `grid`。
...
Premium 外层 `div` 需要类 `grid-rows-[1fr_auto]`。
