...
# --description--
除了底部的 `Go Premium` 按钮外，其余内容都在一个 `div` 中，很适合用网格来整齐排布。

给包裹 `h2`、价格 `p`、说明 `p` 以及 `ul` 的那个内部 `div` 添加 `grid`、`grid-rows-[auto_auto_auto_1fr]`、`gap-y-2`，让结构清晰又留有呼吸空间～
...
该内部 `div` 需要类 `grid-rows-[auto_auto_auto_1fr]`。
