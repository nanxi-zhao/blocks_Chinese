...
# --description--
来让 Premium 的 CTA 像真正的按钮：添加背景渐变与交互反馈。

给它加上：`bg-gradient-to-r`、`from-fuchsia-500`、`to-indigo-600`、`text-white`、`hover:from-fuchsia-600`、`hover:to-indigo-700`，再加 `rounded-md` 与 `block` 完成形状与布局。瞬间高级感满满～

# --hints--

该按钮链接需要类 `bg-gradient-to-r`。
...
该按钮链接需要类 `hover:from-fuchsia-600`。
