...
继续收集更多小伙伴：这次获取`discard-btn`、`tasks-container` 和`title-input`。它们分别帮助你丢弃未保存、更好展示任务列表、填写任务标题。
用熟悉的 `getElementById()` 获取它们，并存入 `discardBtn`、`tasksContainer`、`titleInput` 变量。你正在为应用搭建越来越完整的骨架，太棒啦！
