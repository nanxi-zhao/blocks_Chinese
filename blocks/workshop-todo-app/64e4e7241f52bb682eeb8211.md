...
继续加油，我们要收集最后一组表单帮手：`date-input` 和`description-input`！它们负责日期与描述，让任务更完整。
使用`getElementById()` 获取它们，并分别存入变量 `dateInput`、`descriptionInput`。这一小步完成后，你的表单元素团队就齐了，给自己一个小赞！

# --hints--

你应该使用`getElementById()` 来获取`date-input` 元素。
```js
assert.match(code, /document\.getElementById\(\s*('|"|`)date\-input\1\s*\)\s*;?/)
```

你需要将 `date-input` 元素赋值给变量 `dateInput`，并使用 `const` 声明它。
```js
assert.match(code, /const\s+dateInput\s*=\s*document\.getElementById\(\s*('|"|`)date\-input\1\s*\)\s*;?/)
```

接着使用 `getElementById()` 获取 `description-input` 元素。
```js
assert.match(code, /document\.getElementById\(\s*('|"|`)description\-input\1\s*\)\s*;?/)
```

将它保存到变量`descriptionInput`，同样用 `const`。元素收集完成，太棒啦！
