...
如果用户点击 `Cancel`，我们就温柔地关闭确认框，让他们继续编辑。`dialog` 元素的`close()` 方法可以用来关闭模态窗口：

```js
dialogElement.close();
```

在`cancelBtn` 添加一个监听`click` 的事件，回调里调用`confirmCloseDialog.close()` 即可。

这一步让交互更人性化，继续保持节奏，你很棒！

# --hints--

你应该在 `cancelBtn` 上调用`addEventListener()`。
...
回调函数应调用`confirmCloseDialog.close()`。
