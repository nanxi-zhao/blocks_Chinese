...
只有在输入框里真的有内容的时候，才应该弹出包含 `Cancel` 和 `Discard` 按钮的确认对话框。
先删除那行 `confirmCloseDialog.showModal();`。
接着在 `closeTaskFormBtn` 的事件监听里写逻辑：检查 `titleInput`、`dateInput` 和 `descriptionInput` 是否至少有一个存在值。
如果任意一个有值，就调用 `confirmCloseDialog.showModal()` 显示对话框；否则（没有改动），调用 `reset()` 清空输入并关闭表单。这样用户体验就超级友好啦～

# --hints--

如果任意一个输入框有值，就应该显示确认对话框；否则应该调用 `reset` 函数。
```js
