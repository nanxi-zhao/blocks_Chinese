...
为了让代码更易读、更好维护，我们可以把提交事件监听拆成两个职责清晰的函数：第一个负责把输入值添加进 `taskData`，第二个负责把任务渲染到页面上。
请使用箭头函数语法创建一个 `addOrUpdateTask` 函数，并把 `dataArrIndex` 变量、`taskObj` 对象以及那个判断 `dataArrIndex === -1` 的 `if` 语句都移动进去。结构清晰后后面扩展会轻松许多，继续加油呀！
# --hints--

你应该使用 `const` 和箭头函数语法来创建一个 `addOrUpdateTask` 函数。
```js
assert.match(code, /const\s+addOrUpdateTask\s*=\s*\(\s*\)\s*=>\s*\{\s*/)
```

你应该把 `dataArrIndex` 变量移动到 `addOrUpdateTask` 函数内部。
```js
...
你应该把 `taskObj` 对象移动到 `addOrUpdateTask` 函数内部。
```js
...
你应该把带条件 `dataArrIndex === -1` 的那条 `if` 语句移动到 `addOrUpdateTask` 函数内部。
```js
