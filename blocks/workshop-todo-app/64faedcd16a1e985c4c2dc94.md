...
请使用箭头函数语法创建一个 `deleteTask` 函数。参数名 `buttonEl`，函数体先保持空的花括号。等会我们再往里面填删除逻辑。
# --hints--

你应该使用 `const` 和箭头函数语法创建一个 `deleteTask` 函数。
```js
assert.match(code, /const\s+deleteTask\s*=\s*(\(.*\)|[^\s()]+)\s*=>\s*/);
```

你的 `deleteTask` 函数应该接收一个名 `buttonEl` 的参数。
```js
assert.match(deleteTask.toString(), /\(\s*buttonEl\s*\)/);
```

你的 `deleteTask` 函数现在应该保持空函数体?
```js
