...
现在这些演示性的 `myTaskArr` 和一大段关于 `localStorage` 的示例代码已经完成它们的使命，可以优雅地下线啦～请把那几个与演示数组及其读写相关的部分全部移除，只保留对应用真正有用的逻辑。让代码更轻盈，也让你的 Todo 应用更专注，耶！

# --hints--

需要删除示例数组 `myTaskArr` 以及所有和它相关的演示 `localStorage` 读写代码。
```js
...
也不应再出现对键 `"data"` 的 `localStorage.getItem` 调用。
```js
assert.notMatch(code, /localStorage\.getItem\(\s*('|")data\1\s*\)/);
```

最后，`getTaskArrObj` 也应该被清理掉。
```js
