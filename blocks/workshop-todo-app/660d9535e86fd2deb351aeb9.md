...
还差最后一点点，这个 `id` 就能万里无一重复啦～

先把这段处理好的字符串包进模板字符串里的嵌入表达式：

```js
titleInput.value.toLowerCase().split(" ").join("-")
```

也就是写成：`` `${titleInput.value.toLowerCase().split(" ").join("-")}` ``。
下一步我们会再加上一个独特的数字，让它真正唯一。坚持一下，你超棒！

# --hints--

需要使用模板字符串并写成 `${titleInput.value.toLowerCase().split(" ").join("-")}`。
```js
