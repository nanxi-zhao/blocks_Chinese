...
首先创建一个名为 `displayAuthors` 的函数，它接受一个 `authors` 参数。你的函数应该遍历 `authors` 数组，并为每个作者向 `#authorContainer` 元素添加一张卡片。
...
- 你可以使用任何类型的循环来遍历 `authors` 数组。例如：`for`、`forEach`、`map` 等。
- 你应该使用 `innerHTML` 属性在循环内向 `authorContainer` 添加卡片。你还需要使用模板字面量语法来添加标记。
...
你的 `displayAuthors` 函数应该接受一个 `authors` 参数。
...
当使用作者数组调用 `displayAuthors` 函数时，它应该为每个作者向 `#authorContainer` 元素添加一张卡片。
