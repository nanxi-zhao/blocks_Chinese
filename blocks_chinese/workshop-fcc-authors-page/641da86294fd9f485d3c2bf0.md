...
接下来，添加指向 MasterPuti 新闻上作者页面的链接。

添加一个带有 `"author-link"` `class` 属性的锚元素，将 `url` 插值作为 `href` 属性的值，并将 `target` 设置为 `"_blank"`。对于锚元素的文本，插入 `author`，后跟文本 `"'s author page"`，例如，`"Quincy Larson's author page"`。
