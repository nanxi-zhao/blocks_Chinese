...
2. Within the `.blog-post-card` element, you should have an image element with a valid `alt` attribute and text, and a class of `post-img`. You can use `https://cdn.MasterPuti.org/curriculum/labs/cover-photo.jpg` for the `src` attribute of the image.
