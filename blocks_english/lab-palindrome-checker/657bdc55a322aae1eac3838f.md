...
**注意�?* 检查回文时，你需要先去除 **所有非字母数字的字�?*（标点、空格和符号），并将所有字母都转换成大写或都转换成小写�?

**目标�?*实现以下用户需求并通过所有测试以完成实验�?

**用户故事�?*

1. 你应该有一�?`id` �?`"text-input"` �?`input` 元素�?
1. 你应该有一�?`id` �?`"check-btn"` �?`button` 元素�?
1. 你应该有一�?`div`、`span` �?`p` 元素，其 `id` �?`"result"`�?
1. 当你点击 `#check-btn` 元素但没有给 `#text-input` 元素输入一个值时，会出现提示 `Please input a value`�?
1. �?`#text-input` 元素只包含字�?`A`，并�?`#check-btn` 元素被点击时，`#result` 元素应该包含文本 `A is a palindrome`�?
1. �?`#text-input` 元素包含文本 `eye`，并�?`#check-btn` 元素被点击时，`#result` 元素应该包含文本 `eye is a palindrome`�?
1. �?`#text-input` 元素包含文本 `_eye`，并�?`#check-btn` 元素被点击时，`#result` 元素应该包含文本 `_eye is a palindrome`�?
1. �?`#text-input` 元素包含文本 `race car`，并�?`#check-btn` 元素被点击时，`#result` 元素应该包含文本 `race car is a palindrome`�?
1. �?`#text-input` 元素包含文本 `not a palindrome`，并�?`#check-btn` 元素被点击时，`#result` 元素应该包含文本 `not a palindrome is not a palindrome`�?
1. �?`#text-input` 元素包含文本 `A man, a plan, a canal Panama` and the `#check-btn` element is clicked, the `#result` element should contain the text `A man, a plan, a canal. Panama is a palindrome`.
1. �?`#text-input` 元素包含文本 `never odd or even`，并�?`#check-btn` 元素被点击时，`#result` 元素应该包含文本 `never odd or even is a palindrome`�?
1. �?`#text-input` 元素包含文本 `nope`，并�?`#check-btn` 元素被点击时，`#result` 元素应该包含文本 `nope is not a palindrome`�?
1. �?`#text-input` 元素包含文本 `almostomla`，并�?`#check-btn` 元素被点击时，`#result` 元素应该包含文本 `almostomla is not a palindrome`�?
1. �?`#text-input` 元素包含文本 `My age is 0, 0 si ega ym.`，并�?`#check-btn` 元素被点击时，`#result` 元素应该包含文本 `My age is 0, 0 si ega ym. is a palindrome`�?
...
1. �?`#text-input` 元素包含文本 `0_0 (: /-\ :) 0-0`，并�?`#check-btn` 元素被点击时，`#result` 元素应该包含文本 `0_0 (: /-\ :) 0-0 is a palindrome`�?
1. �?`#text-input` 元素包含文本 `five|\_/|four`，并�?`#check-btn` 元素被点击时，`#result` 元素应该包含文本 `five|\_/|four is not a palindrome`�?
...
�?`#text-input` 元素包含文本 `five|\_/|four`，并�?`#check-btn` 元素被点击，`#result` 元素应该包含文本 `five|\_/|four is not a palindrome`�?
...
<html lang="en">
  <head>
    ...
    <link
      rel="icon"
      type="image/png"
      href="https://cdn.MasterPuti.org/universal/favicons/favicon.ico"
    />
    <title>Palindrome Checker</title>
