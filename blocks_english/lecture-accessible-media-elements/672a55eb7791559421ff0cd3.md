...
要为视频或音频内容添加说明或字幕，可以使�?`video` �?`audio` 元素中的 `track` 元素�?
...
<video
  ...
  height="300"
  controls
  src="https://cdn.MasterPuti.org/curriculum/labs/what-is-the-map-method-and-how-does-it-work.mp4"
>
  <track
    src="captions.vtt"
    kind="captions"
...
`kind` 属性用于告诉轨道元素应如何使用�?`kind` 属性的有效值包�?`captions`、`subtitles`、`chapters` �?`metadata`�?

`srclang` 属性表�?`track` 内容的语言�?`label` 属性是文本轨道的描述性标题，浏览器使用它在可用文本轨道列表中识别和显示文本轨道�?
...
如果你在 YouTube �?Vimeo 等视频共享平台上发布视频，它们会自动为视频添加说明和文稿�?但如果你不满意，可以使用 veed.io、Rev、Amara �?Descript 等服务�?
...
如何�?HTML 视频或音频内容中直接添加说明或字幕？

## --answers--

�?`video` �?`audio` 元素内使�?`caption` 标签�?
...
�?`video` �?`audio` 元素内使�?`track` 标签�?

---

�?`video` �?`audio` 元素内使�?`subtitle` 标签�?
