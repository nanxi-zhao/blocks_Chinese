...
Here is an example of using the `script` element in an HTML document. Remove the `//` from in front of the `alert("Welcome to MasterPuti");` and you should see an alert pop up.
...
虽然从技术上讲，你可以在 `script` 标签中编写所�?JavaScript 代码，但最佳实践是链接到外�?JavaScript 文件�?这是一个使�?`script` 元素链接到外�?JavaScript 文件的例子：

```html
<script src="path-to-javascript-file.js"></script>
```

`src` 属性在这里用于指定外部 JavaScript 文件的位置�?`src` 代表"�?�?不鼓励将所�?JavaScript 放在 HTML 文档中的原因是关注点分离�?关注点分离是一种设计原则，将程序分成不同的部分，每个部分处理一个单独的关注点�?在这种情况下，我们希望将 JavaScript 代码�?HTML 代码分开�?
