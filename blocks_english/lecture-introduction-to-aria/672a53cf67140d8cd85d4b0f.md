...
让静态内容可被访问相对简单，但动态内容则更具挑战性�?这就�?WAI-ARIA 的用武之地�?
...
请注意，WCAG �?WAI-ARIA 并不相同�?WCAG 提供了网络无障碍的一般指导原则，�?WAI-ARIA 则提供了使辅助技术用户能够无障碍访问动态和交互式内容的具体规则�?

因此，WAI-ARIA 的主要目的是提高动态内容和用户界面组件的可访问性，因为这些内容和组件没有原�?HTML 对等内容�?

WAI-ARIA 的工作原理是引入一组属性，你可以将其添加到 HTML 元素中，以提供额外的语义信息�?这些属性分为角色、状态和属性�?

ARIA 角色定义了网站或网络应用中元素的用途�?这是将一�?`div` 的角色设置为 `button` 的示例�?

```html
<div role="button">Click Me</div>
```

这样做就是向辅助技术表明该元素是一个按钮�?不过，角色并不提供任何功能�?仅将�?`div` �?`role` 设置�?`button` 不会让它像按钮一样工作�?要让它看起来像一个按钮，你需要使�?CSS �?JavaScript 来获得理想的效果�?
...
最好优先使用原生的带有 `type="button"` �?`button` �?`input` 元素�?
...
为了更好地使�?WAI-ARIA，请尽可能使用原�?HTML，因为它通常能提供更多的无障碍功能�?

只有�?HTML 达不到要求时才使�?WAI-ARIA，同时不要忘记使用屏幕阅读器等辅助技术进行测试，或让残障人士测试你的作品�?此外，确保你�?WAI-ARIA 状态和属性与内容实时更新�?Avoid overusing ARIA, as it can often be confusing.

# --questions--

## --text--

HTML �?WAI-ARIA 属性的主要用途是什么？
...
网络无障碍界�?- 无障碍资源互联网应用程序（Web Accessibility Interface - Accessible Resource Internet Applications�?
