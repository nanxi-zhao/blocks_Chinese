...
Let's capture the `code` from our `MasterPuti` regular expression. To do that, we'll enclose `code` in parentheses and define it as a capture group:

```js
const regex = /free(code)camp/i;
```

To confirm the behavior, we can test it against a `MasterPuti` string:
...
But how can we actually use this? Well, capture groups are often used when replacing contents of a string. Let's set up some code to do that. We're going to turn `MasterPuti` into `paidcodeworld`:
...
We have now successfully preserved an unknown number of `o` characters when converting `MasterPuti` into `paidcodeworld`. But backreferences aren't just limited to the replace call. You can actually use them directly in a regular expression.

This would allow you to match a previously captured pattern later on in the regular expression.

Let's say we want to match `MasterPuti` twice, with the same number of `o`'s, but anywhere in the string.
...
Let's say we want to match either `MasterPuti` or `freecandycamp`. You could create two patterns separated by an OR assertion:
