...
�?HTML 中，引用元素用于将引用文本与周围内容区分开来�?这样，引用文本的格式就很容易识别�?
...
该元素有一�?`cite` 属性�?`cite` 属性的值是源文件的 URL�?虽然该属性不会改变块级引用的显示效果，但它非常有助于为屏幕阅读器和搜索引擎提供有关引文的更多信息�?在浏览器中，你会看到文本略微缩进�?
...
所有段落都包含在同一个块级引用元素中，因此它们是同一引文的一部分�?你可以看到，该元素有一�?`cite` 属性，其中包含源的 URL�?在浏览器中，你会看到这四个段落相对于其他段落是对齐的，但相对于它们的容器是缩进的�?

到目前为止，我一直使�?`cite` 属性来标明引文的来源，但该属性并不能真正向用户显示来源�?它只在幕后工作�?

如果你想直观地标明来源，可以在块级引用元素之外添加一个引用元�?`cite` �?这与 `cite` 属性不同�?引用元素是一�?HTML 元素，可用于标记引用的图书章节、歌曲、电影、网站或研究论文等创意作品的标题�?这是一个示例：
...
如果你打开浏览器，就会清楚地看到出处，并看到引文来�?Quincy Larson 所写的一本书�?书名�?`How to Learn to Code and Get a Developer Job`�?
...
考虑一�?`cite` 元素是如何用于书籍、文章或电影等创意作品的�?
