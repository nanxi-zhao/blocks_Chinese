...
语义化 HTML 控制数据在服务器上的存储方式。

---

语义化 HTML 决定了网站的整体布局和视觉结构。
...
语义化 HTML 使用元素来控制内容的外观，而不是内容的含义或结构。
