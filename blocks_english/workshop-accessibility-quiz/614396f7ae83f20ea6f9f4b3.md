...
当前 `.sr-only` 文本仍可见，你需要应用业界常用的“仅屏幕阅读器可见”隐藏模式。

这个模式通过一组 CSS 属性把元素移出视觉布局但依旧可被辅助技术读取：
...
请用上述属性完成 `.sr-only` 的样式定义。继续，你正在做专业的无障碍优化！
