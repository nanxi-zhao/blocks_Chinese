...
与其�?`input` �?`label` 元素一样，�?`textarea` 链接到其对应�?`label` 元素，并赋予�?`name` 属性�?

# --hints--

你应该给 `textarea` 元素一�?`id` 属性�?

```js
assert.notEmpty(document.querySelectorAll('.answer')?.[1]?.querySelector('textarea')?.id);
```

你不能在 `textarea` 元素�?`id` 中添加空格�?
...
你应该给 `label` 元素一个与 `textarea` �?`id` 相匹配的 `for` 属性�?
