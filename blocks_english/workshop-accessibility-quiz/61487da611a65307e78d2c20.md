...
窄屏下导航列表会溢出右侧，需要允许 `ul` 自动换行并调整对齐与间距。

开启 Flex 换行（`flex-wrap: wrap`），并设置：
```css
align-items: center;
