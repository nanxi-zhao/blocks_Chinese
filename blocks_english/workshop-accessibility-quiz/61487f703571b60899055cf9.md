...
让每个 `section` 的宽度为父容器的 80%，并通过左右 `auto` 居中，同时底部增加 10px 间距。再限制最大宽度为 600px，避免在超宽屏下行长过大影响阅读。
