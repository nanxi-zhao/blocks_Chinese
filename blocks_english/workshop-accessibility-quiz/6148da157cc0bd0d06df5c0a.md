...
要使输入框相互对齐，�?`.info` 中为所�?`input` �?`label` 元素创建一个新的规则集合，并设�?`display` 属性为 `inline-block`�?

另外，将 `label` 元素的文本向右对齐�?

# --hints--

你应该使用一�?`.info > label, .info > input` �?`.info label, .info input` 选择器�?
...
你应�?`input` �?`label` �?`display` 设置�?`inline-block`�?
...
你应�?`label` 元素�?`text-align` 设置�?`right`�?
