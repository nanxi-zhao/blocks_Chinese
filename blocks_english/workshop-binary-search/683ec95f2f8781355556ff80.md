...
接下来要做的是创建一个条件，检查目标 [value] 是否在中间位置。

在 [while] 循环内部，创建一个 [if] 语句，检查目标 [value] 是否等于 [value_at_middle]。如果是，返回 [path_to_target] 变量。

# --hints--

你应该创建一个 [if] 语句，检查 [value] 是否等于 [value_at_middle]。
...
你应该在 [if] 语句内部返回 [path_to_target] 变量。
