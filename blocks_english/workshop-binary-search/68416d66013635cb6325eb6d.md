...
你可以做的最后一个小小的改进是显示找到目标 [value] 的索引。

所以，不要在 [if] 语句中只返回 [path_to_target]，而是添加一个带有消息 [Value found at index {mid}] 的 f-string。

这样，你的二分查找算法练习就完成啦！

# --hints--

你应该返回 [path_to_target, f'Value found at index {mid}'] 在 [if] 块中。
