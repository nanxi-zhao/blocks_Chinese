...
你可以更改文本的`font-family`，使其看起来与浏览器的默认字体不同。每个浏览器都有一些常见的字体可用。

通过添加值为`sans-serif`的`font-family`属性来更改`body`中的所有文本。这是一种相当常见的字体，非常易读。
...
你的`body`应该有`sans-serif`的`font-family`。
