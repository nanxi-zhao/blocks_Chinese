...
使用现有的选择器，选取嵌套在类名为 `item` 的元素中的所有 `p` 元素，将它们的上下边距设置为 `5px`。
...
嵌套在 `.item` 元素中的 `p` 元素应具有 `5px` 的 `margin-top`。
...
嵌套在 `.item` 元素中的 `p` 元素不应设置 `margin-left`。
...
嵌套在 `.item` 元素中的 `p` 元素不应设置 `margin-right`。
