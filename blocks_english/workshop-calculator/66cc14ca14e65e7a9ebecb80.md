...
现在你的计算器可以将两个数字相加了，是时候创建一个可以将两个数字相减的函数了。

声明一个名为[calculateDifference]的函数，它有两个参数[num1]和[num2]。

在函数内部，返回两个参数之间的差值。

# --hints--

你应该有一个名为[calculateDifference]的函数。

```js
assert.isFunction(calculateDifference);
```

你的[calculateDifference]函数应该有两个参数，分别叫做[num1]和[num2]。
...
你的[calculateDifference]函数应该返回两个参数之间的差值。
