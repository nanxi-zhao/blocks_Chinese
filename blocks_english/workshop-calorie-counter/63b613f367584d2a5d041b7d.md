...
获取页面中的 `#budget` 元素并将其赋值给 `budgetNumberInput`，同时获取 `#entry-dropdown` 元素并将其赋值给 `entryDropdown`。

# --hints--

请声明一个名为 `budgetNumberInput` 的变量，用以保存 `#budget` 元素。

```js
assert.isDefined(budgetNumberInput);
```

请使用 `document.getElementById()` 获取 `#budget` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|")budget\1\s*\)/g);
```

确认将 `#budget` 元素赋值给 `budgetNumberInput`。

```js
assert.equal(budgetNumberInput, document.getElementById('budget'));
```

请声明一个名为 `entryDropdown` 的变量，用以保存 `#entry-dropdown` 元素。

```js
assert.isDefined(entryDropdown);
```

请使用 `document.getElementById()` 获取 `#entry-dropdown` 元素。

```js
assert.match(code, /document\.getElementById\(\s*('|")entry-dropdown\1\s*\)/g);
```

确认将 `#entry-dropdown` 元素赋值给 `entryDropdown`。
