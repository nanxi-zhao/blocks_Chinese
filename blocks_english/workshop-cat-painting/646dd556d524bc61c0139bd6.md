...
大多数猫的耳朵和同一耳朵的内侧部分都有不同的颜色。你也可以实现同样的效果。这就是为什么你之前为左右内耳都定义了一个[div]元素。

使用类选择器，给你的[.cat-left-inner-ear]元素设置左右边框，均为[20px solid transparent]。同时给它设置下边框为[40px solid #3b3b4f]。
