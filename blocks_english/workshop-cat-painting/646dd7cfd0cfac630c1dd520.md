...
为了去除耳朵的所有尖锐边缘，设置右下角和左下角的边框半径为 `40%`，左上角边框半径为 `90px`，右上角边框半径为 `10px`。

# --hints--

你的 `.cat-left-inner-ear` 选择器应该有一个 `border-bottom-right-radius` 属性，设置为 `40%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear')?.borderBottomRightRadius, '40%')
```

你的 `.cat-left-inner-ear` 选择器应该有一个 `border-bottom-left-radius` 属性，设置为 `40%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear')?.borderBottomLeftRadius, '40%')
```

你的 `.cat-left-inner-ear` 选择器应该有一个 `border-top-left-radius` 属性，设置为 `90px`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-ear')?.borderTopLeftRadius, '90px')
```

你的 `.cat-left-inner-ear` 选择器应该有一个 `border-top-right-radius` 属性，设置为 `10px`。
