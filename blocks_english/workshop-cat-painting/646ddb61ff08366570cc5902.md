...
创建一个带有 `cat-eyes` 类的 `div` 元素。在 `.cat-eyes` 元素内，分别创建两个带有 `cat-left-eye` 和 `cat-right-eye` 类的 `div` 元素。

# --hints--

你应该创建一个带有 `cat-eyes` 类的 `div` 元素。
...
在 `.cat-eyes` 元素内的第一个 `div` 元素应该有 `cat-left-eye` 类。

```js
assert.isTrue(document.querySelectorAll('.cat-eyes div')[0].classList.contains('cat-left-eye'));
```

在 `.cat-eyes` 元素内的第二个 `div` 元素应该有 `cat-right-eye` 类。
