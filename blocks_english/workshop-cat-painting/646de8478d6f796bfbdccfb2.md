...
为了让右眼看起来像眼睛，给它一个 `60%` 的边框半径。同时，使用 `transform` 属性，将其旋转 `-25` 度。

# --hints--

你的 `.cat-right-eye` 选择器应该有一个 `border-radius` 属性，设置为 `60%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-right-eye')?.borderRadius, '60%');
```

你的 `.cat-right-eye` 选择器应该有一个 `transform` 属性，设置为 `rotate(-25deg)`。
