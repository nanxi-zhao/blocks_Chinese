...
这些看起来像眼睛，但你仍然可以让它们更好。这就是为什么你创建了两个内眼 `div` 元素。

使用类选择器，给你的 `.cat-left-inner-eye` 元素设置 `10px` 的宽度和 `20px` 的高度。同时，给它一个 `#fff` 的背景颜色。
