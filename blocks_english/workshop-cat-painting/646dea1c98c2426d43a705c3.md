...
通过设置 `position` 属性为 `absolute`，`top` 为 `8px`，`left` 为 `2px`，将左内眼移动到正确位置。同时，给它一个 `60%` 的边框半径，并将其旋转 `10` 度。

# --hints--

你的 `.cat-left-inner-eye` 选择器应该有一个 `position` 属性，设置为 `absolute`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye')?.position, 'absolute')
```

你的 `.cat-left-inner-eye` 选择器应该有一个 `top` 属性，设置为 `8px`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye')?.top, '8px')
```

你的 `.cat-left-inner-eye` 选择器应该有一个 `left` 属性，设置为 `2px`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye')?.left, '2px')
```

你的 `.cat-left-inner-eye` 选择器应该有一个 `border-radius` 属性，设置为 `60%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-left-inner-eye')?.borderRadius, '60%')
```

你的 `.cat-left-inner-eye` 选择器应该有一个 `transform` 属性，设置为 `10deg`。
