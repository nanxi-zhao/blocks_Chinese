...
使用类选择器，给你的 `.cat-right-inner-eye` 元素设置 `10px` 的宽度和 `20px` 的高度。同时，给它一个 `#fff` 的背景颜色。
