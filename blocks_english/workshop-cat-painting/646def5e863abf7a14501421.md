...
使用类选择器，给你的[.cat-nose]元素设置左右边框，均为[15px solid transparent]。同时给它设置下边框为[20px solid #442c2c]。
...
你的[.cat-nose]选择器应该有一个[border-left]属性，设置为[15px solid transparent]。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('.cat-nose')?.borderLeft, '15px solid transparent');
```

你的[.cat-nose]选择器应该有一个[border-right]属性，设置为[15px solid transparent]。
