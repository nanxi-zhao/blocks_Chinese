...
在`.cat-whiskers-left`元素内部，创建三个带有类名`cat-whisker-left-top`、`cat-whisker-left-middle`和`cat-whisker-left-bottom`的`div`元素。

# --hints--

你不应该更改现有的带有`cat-whiskers-left`类的`div`元素。

```js
assert.lengthOf(document.querySelectorAll('div.cat-whiskers-left'), 1);
```

你应该在`.cat-whiskers-left`元素内部创建三个`div`元素。

```js
assert.lengthOf(document.querySelectorAll('.cat-whiskers-left div'), 3);
```

在`.cat-whiskers-left`元素内部的第一个`div`元素应该有`cat-whisker-left-top`类。

```js
assert.isTrue(document.querySelectorAll('.cat-whiskers-left div')[0].classList.contains('cat-whisker-left-top'));
```

在`.cat-whiskers-left`元素内部的第二个`div`元素应该有`cat-whisker-left-middle`类。

```js
assert.isTrue(document.querySelectorAll('.cat-whiskers-left div')[1].classList.contains('cat-whisker-left-middle'));
```

在`.cat-whiskers-left`元素内部的第三个`div`元素应该有`cat-whisker-left-bottom`类。
