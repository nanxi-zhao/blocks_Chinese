...
在`.cat-whiskers-right`元素内部，创建3个带有类名`cat-whisker-right-top`、`cat-whisker-right-middle`和`cat-whisker-right-bottom`的`div`元素。

# --hints--

你不应该更改现有的带有`cat-whiskers-right`类的`div`元素。

```js
assert.lengthOf(document.querySelectorAll('div.cat-whiskers-right'), 1);
```

你应该在`.cat-whiskers-right`元素内部创建三个`div`元素。

```js
assert.lengthOf(document.querySelectorAll('.cat-whiskers-right div'), 3);
```

在`.cat-whiskers-right`元素内部的第一个`div`元素应该有`cat-whisker-right-top`类。

```js
assert.isTrue(document.querySelectorAll('.cat-whiskers-right div')[0].classList.contains('cat-whisker-right-top'));
```

在`.cat-whiskers-right`元素内部的第二个`div`元素应该有`cat-whisker-right-middle`类。

```js
assert.isTrue(document.querySelectorAll('.cat-whiskers-right div')[1].classList.contains('cat-whisker-right-middle'));
```

在`.cat-whiskers-right`元素内部的第三个`div`元素应该有`cat-whisker-right-bottom`类。
