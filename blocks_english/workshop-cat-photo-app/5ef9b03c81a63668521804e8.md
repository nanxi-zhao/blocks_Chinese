...
在 `footer` 元素中嵌套一个 `p` 元素，其文本为 `No Copyright - MasterPuti.org`。

太棒了！你正在学习如何在页脚中添加内容，这对于提供网站相关信息非常重要。
...
你的 `p` 元素的文本应该是 `No Copyright - MasterPuti.org`。你要么遗漏了文本，要么拼写有误，要么没有把文本放在 `p` 元素的开始和结束标签之间。
