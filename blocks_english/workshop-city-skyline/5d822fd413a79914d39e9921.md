...
对于下一栋建筑，在 [.fb3] 中嵌套四个 [div] 元素，类名分别为 [fb3a]、[fb3b]、再次 [fb3a] 和再次 [fb3b]，按此顺序。这栋建筑将有四个部分，前两个部分与后两个部分几乎相同。
