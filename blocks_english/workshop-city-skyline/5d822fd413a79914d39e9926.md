...
给`.fb3-window`元素设置`width`为`25%`，`height`为`80%`，并使用`--window-color1`变量作为`background-color`值。

# --hints--

你应该给`.fb3-window`设置`width`为`25%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3-window")?.width, "25%");
```

你应该给`.fb3-window`设置`height`为`80%`。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb3-window")?.height, "80%");
```

你应该给`.fb3-window`设置`background-color`为`--window-color1`。
