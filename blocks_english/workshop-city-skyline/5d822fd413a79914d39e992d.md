...
太棒啦！给`.fb4-window`元素添加`30%`的`width`、`10%`的`height`和`50%`的`border-radius`。这些将为这座建筑制作一些圆形窗户哦！

# --hints--

你应该给`.fb4-window`添加`30%`的宽度。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4-window")?.width, "30%");
```

你应该给`.fb4-window`添加`10%`的高度。

```js
assert.equal(new __helpers.CSSHelp(document).getStyle(".fb4-window")?.height, "10%");
```

你应该给`.fb4-window`添加`50%`的边框半径。
