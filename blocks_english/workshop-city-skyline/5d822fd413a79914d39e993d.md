...
太棒啦！现在让我们在媒体查询的顶部添加一个 [`:root`](https://developer.mozilla.org/zh-CN/docs/Web/CSS/:root) 选择器。然后在那里重新定义所有四个 `--building-color` 变量，将它们的值都设为 `#000` 吧！
...
你应该添加 `--building-color1` 并将其值设为 `#000`。
...
你应该添加 `--building-color2` 并将其值设为 `#000`。
...
你应该添加 `--building-color3` 并将其值设为 `#000`。
...
你应该添加 `--building-color4` 并将其值设为 `#000`。
