...
在你的 `checkUserInput` 函数中，移除 `console.log()` 语句。然后，调用 `decimalToBinary` 函数并传入 `numberInput` 的 `value` 作为参数。另外，确保使用 `parseInt()` 函数将输入转换为数字。

# --hints--

你应该从 `checkUserInput` 函数中移除 `console.log()` 语句。

```js
assert.notMatch(String(checkUserInput), /alert\(\s*('|"|`)\s*(\s|.)*\s*\1\s*\)\s*;?\s*return\s*;?\s*\}\s*console\.log\((\s|.)*\)/);
```

你应该在 `checkUserInput` 函数体内的 `if` 语句后调用 `decimalToBinary` 函数。
