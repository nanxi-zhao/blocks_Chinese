...
In your `callStack` array, add the following string: `'a(): returns "MasterPuti " + b()'`. This represents the function call and the code that will be executed.
...
The string in `callStack` should be the string `'a(): returns "MasterPuti " + b()'`.

```js
assert.match(callStack[0], /a\(\s*\):\s*returns\s*('|"|`)MasterPuti\s*\1\s*\+\s*b\s*\(\s*\)/);
```
