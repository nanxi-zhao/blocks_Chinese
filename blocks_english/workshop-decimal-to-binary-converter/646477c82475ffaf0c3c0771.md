...
虽然异步(async)代码一开始可能很难理解，但它有很多优点。其中最重要的一点是它允许你编写非阻塞代码。

例如，想象你在烤蛋糕，把蛋糕放进烤箱并设置了计时器。你不需要一直坐在烤箱前等待——你可以在等待计时器响起的同时洗碗、读书或做任何其他事情。

异步代码的工作方式类似。你可以启动一个异步操作，而代码的其他部分在该操作运行时仍然可以工作。

你将在未来的模块中了解更多关于异步代码的知识，但[setTimeout()]函数是一个很好的入门。

在文本["Camp"]记录到控制台之前添加[1500]毫秒的延迟。

# --hints--

你应该在你的[showAnimation()]函数中添加第三个[setTimeout()]函数。
