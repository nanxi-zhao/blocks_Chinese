...
由于你已经在[addElDelay]中存储了每个动画帧的时间，你可以将该值与[setTimeout()]一起使用来设置延迟，以便将元素添加到DOM中。

在[.forEach()]方法的回调函数体内，添加一个[setTimeout()]函数。将一个空的回调函数作为第一个参数传递，并将[obj.addElDelay]作为第二个参数。

# --hints--

你应该在[.forEach()]方法的回调函数体内添加[setTimeout()]函数。
