...
现在为 `PremiumUserDiscount` 实现 `apply_discount` 方法。这应该对高级用户应用 20% 的折扣。

返回 `product.price * 0.8`（这代表原价的 80%，即 20% 折扣）。

# --hints--

你的 `PremiumUserDiscount` 类应该有一个 `apply_discount` 方法。
...
你的 `apply_discount` 方法应该返回 `product.price * 0.8`。
...
你的 `apply_discount` 方法应该应用 20% 的折扣（原价的 80%）。
