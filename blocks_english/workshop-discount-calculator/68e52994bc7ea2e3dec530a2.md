...
现在在你的 `DiscountEngine` 类中创建一个名为 `calculate_best_price` 的方法。这个方法应该接受 `self`、`product` 和 `user_tier` 作为参数，并返回一个 `float`。

现在，在方法体中使用 `pass`。你将在接下来的步骤中实现逻辑。

# --hints--

你的 `DiscountEngine` 类应该有一个名为 `calculate_best_price` 的方法。
...
你的 `calculate_best_price` 方法应该有参数 `self`、`product: Product` 和 `user_tier: str`。
...
你的 `calculate_best_price` 方法应该有返回类型提示 `-> float`。
