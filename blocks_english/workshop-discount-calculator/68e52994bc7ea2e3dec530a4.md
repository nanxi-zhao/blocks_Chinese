...
现在创建一个 `for` 循环，遍历 `self.strategies`。对于每个 `strategy`，使用 `is_applicable` 方法检查它是否适用。如果适用，使用 `apply_discount` 计算折扣价格并将其存储在名为 `discounted` 的变量中。然后，将折扣价格追加到 `prices` 列表中。

# --hints--

你应该创建一个遍历 `self.strategies` 的 `for` 循环。
...
在循环内部，你的 `if` 语句应该是 `if strategy.is_applicable(product, user_tier):`。
