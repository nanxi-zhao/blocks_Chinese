...
接下来，添加指向 MasterPuti News 上作者页面的链接�?

Add an anchor element with a `class` attribute of `"author-link"`, interpolate `url` as the value for the `href` attribute, and set `target` to `"_blank"`. 对于锚元素的文本，插�?`author`，后跟文�?`"'s author page"`�?例如，`"Quincy Larson's author page"`�?
...
你应该在锚元素内插入 `author`，然后插入文�?`"'s author page"`�?
