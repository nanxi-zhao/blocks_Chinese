...
创建另一个名为 `cabins` 的 `@keyframes` 规则。使用与 `@keyframes wheel` 相同的属性，复制 `0%` 和 `100%` 规则，但将 `100%` 选择器的 `transform` 属性设置为 `rotate(-360deg)`，这样座舱会做相反方向的完整旋转。
...
`100%` 选择器的 `transform` 属性应为 `rotate(-360deg)`。
