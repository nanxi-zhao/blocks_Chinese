...
为了让山峰看起来更像一座山，你可以使用 `skew` 变换函数，它接受两个参数。第一个是x轴的倾斜角度，第二个是y轴的倾斜角度。

使用 `transform` 属性将山峰在x轴上倾斜 `0deg`，在y轴上倾斜 `44deg`。

# --hints--

你应该给 `.left-mountain` 添加一个 `transform` 属性。
...
你应该给 `.left-mountain` 设置 `transform` 为 `skew(0deg, 44deg)`。
