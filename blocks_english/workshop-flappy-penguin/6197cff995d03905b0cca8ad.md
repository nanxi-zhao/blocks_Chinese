...
你的企鹅将由两个主要部分组成：头部和身体。

在[.penguin]内，添加两个新的[div]元素。第一个设置[class]为[penguin-head]，第二个设置[class]为[penguin-body]。

# --hints--

你应该在[.penguin]中添加两个新的[div]元素。期望找到2个[.penguin > div]元素，但找到了--fcc-actual--个。

```js
assert.lengthOf(document.querySelectorAll('.penguin > div'), 2);
```

你应该给第一个[div]设置[class]为[penguin-head]。

```js
assert.include(document.querySelector('.penguin > div:nth-of-type(1)')?.className, 'penguin-head');
```

你应该给第二个[div]设置[class]为[penguin-body]。
