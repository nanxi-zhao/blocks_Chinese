...
开始制作企鹅的脸部，通过在[.penguin-head]内添加两个[div]元素，并给它们都设置[class]为[face]。

# --hints--

你应该在[.penguin-head]内添加两个[div]元素。期望找到2个[.penguin-head > div]元素，但找到了--fcc-actual--个。

```js
assert.lengthOf(document.querySelectorAll('.penguin-head > div'), 2);
```

你应该给第一个[div]设置[class]为[face]。

```js
assert.include(document.querySelector('.penguin-head > div:nth-of-type(1)')?.className, 'face');
```

你应该给第二个[div]设置[class]为[face]。
