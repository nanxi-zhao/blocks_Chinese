...
在 `.eye.right` 元素下方，添加两个 `class` 为 `blush` 的 `div` 元素。另外，给第一个 `.blush` 元素添加 `left` 类，第二个 `.blush` 元素添加 `right` 类。

# --hints--

你应该在 `.penguin-head` 内添加两个 `div` 元素。期望找到7个 `.penguin-head > div` 元素，但找到了--fcc-actual--个。

```js
assert.lengthOf(document.querySelectorAll('.penguin-head > div'), 7);
```

你应该给第一个新 `div` 设置 `class` 为 `blush`。

```js
assert.exists(document.querySelector('.penguin-head > div.blush'));
```

你应该给第二个新 `div` 设置 `class` 为 `blush`。
...
你应该将 `.blush.right` 放置在 `.blush.left` 之后。
