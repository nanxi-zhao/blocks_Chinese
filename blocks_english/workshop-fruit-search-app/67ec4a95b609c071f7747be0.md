...
在 `form` 元素内部，添加一个 `label` 元素。你的 `label` 元素应该包含文本 `Search for fruits:`，并且有一个 `htmlFor` 属性，其值为 `"search-input"`。

# --hints--

你应该在 `form` 元素内部有一个 `label` 元素。
...
`label` 元素应该有一个 `htmlFor` 属性，其值为 `"search-input"`。
...
你的 `label` 元素应该包含文本 `Search for fruits:`。
