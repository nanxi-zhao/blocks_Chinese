...
你将在未来的课程中了解更多关于这种模式的知识。

为了跟踪用户在搜索输入字段中输入的内容，创建一个名为 `query` 的状态变量，初始值为空字符串（`''`）。同时使用 `useState` Hook 定义其对应的 setter 函数 `setQuery`。

# --hints--

你应该使用数组解构语法来定义 `query` 和 `setQuery` 变量。
...
你的 `useState` Hook 应该有一个初始值为空字符串（`''`）。
