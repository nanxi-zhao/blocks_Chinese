...
当用户在输入字段中输入时，应该出现一个与他们查询匹配的水果列表。

为了处理这种情况，创建一个名为 `results` 的状态变量，初始值为空数组（`[]`）。同时使用 `useState` Hook 定义其 setter 函数 `setResults`。

# --hints--

你应该使用数组解构语法来定义 `results` 和 `setResults` 变量。
