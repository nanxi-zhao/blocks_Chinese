...
接下来，你将使你的应用程序在用户在输入框中输入时从 API 获取水果名称。

首先，添加一个 `useEffect` hook，使其在 `query` 状态更改时运行。确保将箭头函数作为第一个参数传递给 `useEffect`，并在依赖数组中包含 `query`。

# --hints--

你应该使用 `useEffect` hook。
...
你应该在 `useEffect` hook 的依赖数组中包含 `query`。
