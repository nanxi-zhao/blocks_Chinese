...
如果 `query` 不为空，你想要从 API 获取与用户输入匹配的水果，但只有在用户停止输入一小段时间后才获取，以避免过于频繁地进行获取调用。

首先，在 `if` 语句之后，调用 `setTimeout`，传入一个空箭头函数和延迟值 `700` 作为参数，并将其存储在名为 `timeoutId` 的变量中。这允许你在效果清理时取消超时。

# --hints--

你应该调用 `setTimeout` 并将返回值存储在名为 `timeoutId` 的变量中。

```js
assert.match(code, /(const|let|var)\s+timeoutId\s*=\s*setTimeout\s*\(/s);
```

`setTimeout` 应该有一个空回调函数和延迟值 `700`。
