...
要使 `option` 默认为选中状态，你可以向希望被选中�?`option` 元素添加 `selected` 属性�?

下面是一个使�?`selected` 属性的示例�?

```html
<option selected value="amazing">Amazing</option>
```

在你�?`select` 元素中，给值为 `"excellent"` �?`option` 元素添加 `selected` 属性�?

# --hints--

值为 `"excellent"` �?`option` 元素应具�?`selected` 属性�?
