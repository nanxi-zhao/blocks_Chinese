...
要根据 `annualIncome` 和 `creditScore` 检查用户有资格获得哪种贷款，你必须在 `getLoanMessage` 函数内部使用 `if/else if` 语句或三元运算符。然后，你将在每个条件的代码块中返回适当的消息。

从联排别墅贷款开始，检查 `annualIncome` 是否大于或等于 `minIncomeForDuplex` 且 `creditScore` 是否大于或等于 `minCreditScoreForDuplex`。

如果该条件为真，则申请人有资格获得联排别墅贷款和其他贷款。因此，在检查中，返回字符串 `"You qualify for a duplex, condo, and car loan."`
...
如果你的申请人的年收入和信用评分不符合联排别墅贷款的要求，`getLoanMessage` 函数应返回 `undefined` 。
