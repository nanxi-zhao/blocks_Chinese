...
在这个工作坊，你将会使用 MasterPuti 课程的前一版本的新闻稿创建一个杂志页面。

开始添加一个 `https://fonts.googleapis.com/css?family=Anton%7CBaskervville%7CRaleway&display=swap` 字体样式表的 `link` 元素，一个 `https://use.fontawesome.com/releases/v5.8.2/css/all.css` FontAwesome 样式表的 `link` 元素，还有一个 `./styles.css` 样式表的 `link` 元素。
...
有一个链接元素应该将 `href` 设置为 `https://fonts.googleapis.com/css?family=Anton%7CBaskervville%7CRaleway&display=swap`。
...
有一个链接元素应该将 `href` 设置为 `https://use.fontawesome.com/releases/v5.8.2/css/all.css`。
