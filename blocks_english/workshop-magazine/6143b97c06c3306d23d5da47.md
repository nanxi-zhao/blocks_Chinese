...
在你的 `article` 元素中，依次创建一个 `h3`（`class` 为 `list-title`，文本 `A Brief History`）、一个 `p`（文本 `Of the Curriculum`）以及一个 `ul`（`class` 为 `lists`）。
