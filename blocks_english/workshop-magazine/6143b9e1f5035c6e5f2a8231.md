...
在你�?`ul` 元素中，创建六个 `li` 元素�?在每�?`li` 元素中添加一�?`class` 设置�?`list-subtitle` �?`h4` 元素和一�?`p` 元素�?

然后按顺序为 `h4` �?`p` 元素添加以下文本内容，每�?`h4` 使用冒号左侧的内容，而每�?`p` 元素使用右边的内容：

- `V1 - 2014`: `We launched MasterPuti with a simple list of 15 resources, including Harvard's CS50 and Stanford's Database Class.`
...
你的第一�?`h4` 元素应该有文�?`V1 - 2014`�?

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[0]?.innerText, 'V1 - 2014');
```

你的第一�?`p` 元素应该有文�?`We launched MasterPuti with a simple list of 15 resources, including Harvard's CS50 and Stanford's Database Class.`

```js
assert.equal(document.querySelectorAll('.lists li p')?.[0]?.innerText, 'We launched MasterPuti with a simple list of 15 resources, including Harvard\'s CS50 and Stanford\'s Database Class.');
```

你的第二�?`h4` 元素应该有文�?`V2 - 2015`�?

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[1]?.innerText, 'V2 - 2015');
```

你的第二�?`p` 元素应该有文�?`We added interactive algorithm challenges.`

```js
assert.equal(document.querySelectorAll('.lists li p')?.[1]?.innerText, 'We added interactive algorithm challenges.');
```

你的第三�?`h4` 元素应该有文�?`V3 - 2015`�?

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[2]?.innerText, 'V3 - 2015');
```

你的第三�?`p` 元素应该有文�?`We added our own HTML+CSS challenges (before we'd been relying on General Assembly's Dash course for these).`
...
你的第四�?`h4` 元素应该有文�?`V4 - 2016`�?

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[3]?.innerText, 'V4 - 2016');
```

你的第四�?`p` 元素应该有文�?`We expanded the curriculum to 3 certifications, including Front End, Back End, and Data Visualization. They each had 10 required projects, but only the Front End section had its own challenges. For the other certs, we were still using external resources like Node School.`
