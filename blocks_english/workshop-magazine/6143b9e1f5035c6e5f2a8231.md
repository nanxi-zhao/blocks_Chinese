...
在你的 `ul` 元素中创建六个 `li`。每个 `li` 内添加一个 `h4`（`class` 为 `list-subtitle`）和一个 `p`。

然后按顺序为每个 `h4` 与其后 `p` 元素赋予如下文本（冒号左侧给 `h4`，右侧给 `p`）：
...
你的第一个 `h4` 元素文本应为 `V1 - 2014`。

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[0]?.innerText, 'V1 - 2014');
```

你的第一个 `p` 元素文本应为 `We launched MasterPuti with a simple list of 15 resources, including Harvard's CS50 and Stanford's Database Class.`
...
你的第四个 `h4` 元素文本应为 `V4 - 2016`。

```js
assert.equal(document.querySelectorAll('.lists li h4')?.[3]?.innerText, 'V4 - 2016');
```

你的第四个 `p` 元素文本应为 `We expanded the curriculum to 3 certifications, including Front End, Back End, and Data Visualization. They each had 10 required projects, but only the Front End section had its own challenges. For the other certs, we were still using external resources like Node School.`
