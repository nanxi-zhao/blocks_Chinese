...
�?`.image-wrapper` 元素中，给第一�?`img` 元素添加一个值为 `https://cdn.MasterPuti.org/testable-projects-fcc/images/random-quote-machine.png` �?`src`、一个值为 `image of the quote machine project` �?`alt`、一个值为 `image-1` �?`class`、一个值为 `lazy` �?`loading` 属性、一个值为 `600` �?`width` 属性和一个值为 `400` �?`height` 属性�?

# --hints--

你的第一�?`img` 元素应该有一�?`src` 属性，值为 `https://cdn.MasterPuti.org/testable-projects-fcc/images/random-quote-machine.png`�?

```js
assert.equal(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('src'), 'https://cdn.MasterPuti.org/testable-projects-fcc/images/random-quote-machine.png');
```

你的第一�?`img` 元素应该有一�?`alt` 属性，值为 `image of the quote machine project`�?

```js
assert.equal(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('alt'), 'image of the quote machine project');
```

你的第一�?`img` 元素应该有一�?`class` 属性，值为 `image-1`�?

```js
assert.isTrue(document.querySelectorAll('.image-wrapper img')?.[0]?.classList?.contains('image-1'));
```

你的第一�?`img` 元素应该有一�?`loading` 属性，值为 `lazy`�?

```js
assert.equal(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('loading'), 'lazy');
```

你的第一�?`img` 元素应该有一�?`width` 属性，值为 `600`�?

```js
assert.equal(document.querySelectorAll('.image-wrapper img')?.[0]?.getAttribute('width'), '600');
```

你的第一�?`img` 元素应该有一�?`height` 属性，值为 `400`�?
