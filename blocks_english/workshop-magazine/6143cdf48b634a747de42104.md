...
在你�?`.image-wrapper` 元素中，给第三个 `img` 元素添加一个值为 `https://cdn.MasterPuti.org/testable-projects-fcc/images/survey-form-background.jpeg` �?`src`、一个值为 `four people working on code` �?`alt`、一个值为 `lazy` �?`loading` 属性、一个值为 `image-3` �?`class`、一个值为 `600` �?`width` 属性和一个值为 `400` �?`height` 属性�?

# --hints--

你的第三�?`img` 元素应该有一�?`src` 属性，值为 `https://cdn.MasterPuti.org/testable-projects-fcc/images/survey-form-background.jpeg`�?

```js
assert.equal(
  document.querySelectorAll('.image-wrapper img')?.[2]?.getAttribute('src'),
  'https://cdn.MasterPuti.org/testable-projects-fcc/images/survey-form-background.jpeg'
);
```

你的第三�?`img` 元素应该有一�?`alt`，值为 `four people working on code`�?
...
你的第三�?`img` 元素应该有一�?`loading` 属性，值为 `lazy`�?
...
你的第三�?`img` 元素应该有一�?`class`，值为 `image-3`�?
...
你的第三�?`img` 元素应该有一�?`width` 属性，值为 `600`�?
...
你的第三�?`img` 元素应该有一�?`height` 属性，值为 `400`�?
