...
开始写 CSS，通过使用 `*`，包�?`::before` �?`::after` 伪选择器，定位所有元素来规范 CSS 规则，�?

�?`padding` �?`margin` 属性均设置�?`0`，并�?`box-sizing` 属性设置为 `border-box`�?

# --hints--

你应该有一�?`*, ::before, ::after` 选择器�?

```js
assert.exists(new __helpers.CSSHelp(document).getStyle('*, ::before, ::after'));
```

你的 `*, ::before, ::after` 选择器应该有一�?`padding` 属性，值为 `0`�?

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('*, ::before, ::after')?.padding, '0px');
```

你的 `*, ::before, ::after` 选择器应该有一�?`margin` 属性，值为 `0`�?

```js
assert.equal(new __helpers.CSSHelp(document).getStyle('*, ::before, ::after')?.margin, '0px');
```

你的 `*, ::before, ::after` 选择器应该有一�?`box-sizing` 属性，�?`border-box`�?
