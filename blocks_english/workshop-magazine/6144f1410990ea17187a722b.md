...
现在你可以设置网格布局的样式�?CSS Grid 类似�?Flexbox，因为它对父元素和子元素都有一个特殊的属性�?

在这里，你的父元素是 `main` 元素�?通过添加值为 `1fr 94rem 1fr` �?`grid-template-columns` 属性，将内容设置为具有三列的布局�?这将创建三列，中间列的宽度为 `94rem`，第一列和最后一列都是网格容器中剩余空间的一半�?

# --hints--

你的 `main` 部分应该有一�?`grid-template-columns` 属性，值为 `1fr 94rem 1fr`�?
