...
使用 `minmax` 函数使你的列在任何设备上都能响应�?`minmax` 函数有两个参数，第一个是最小值，第二个是最大值�?这些值可以是长度、百分比、`fr`，甚至是�?`max-content` 这样的关键字�?

�?`grid-template-columns` 属性的每个已定义的值放�?`minmax` 函数中作为第二个参数�?第一个参数应分别�?`2rem`、`min-content` �?`2rem`�?

# --hints--

你的 `main` 选择器应该有一�?`grid-template-columns` 属性。值为 `minmax(2rem, 1fr) minmax(min-content, 94rem) minmax(2rem, 1fr)`�?
