...
很像 Flexbox，使�?CSS Grid，你可以使用 `align-items` �?`justify-items` 对齐网格项目的内容�?`align-items` 将沿列轴对齐子元素，�?`justify-items` 将沿行轴对齐子元素�?

�?`.social-icons` 选择器一�?`align-items` 属性，设置�?`center`�?

# --hints--

你的 `.social-icons` 选择器应该有一�?`align-items` 属性，设置�?`center`�?
