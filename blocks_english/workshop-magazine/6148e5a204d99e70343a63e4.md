...
`gap` 属性是同时设置 `column-gap` �?`row-gap` 值的简写方式�?如果给定一个值，它会�?`column-gap` �?`row-gap` 都设置为该值�?如果给定两个值，它会�?`row-gap` 设置为第一个值，�?`column-gap` 设置为第二个值�?

�?`.image-wrapper` 选择器一�?`gap` 属性，设置�?`2rem`�?

# --hints--

你的 `.image-wrapper` 元素应该有一�?`gap` 属性，设置�?`2rem`�?
