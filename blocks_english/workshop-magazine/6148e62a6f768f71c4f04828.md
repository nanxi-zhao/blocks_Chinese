...
`place-items` 属性可用于同时设置 `align-items` �?`justify-items` 的值�?`place-items` 属性采用一个或两个值�?如果提供了一个值，则它同时用于 `align-items` �?`justify-items` 属性�?如果提供了两个值，则第一个值用�?`align-items` 属性，第二个值用�?`justify-items` 属性�?

�?`.image-wrapper` 选择器一�?`place-items` 属性，设置�?`center`�?

# --hints--

你的 `.image-wrapper` 选择器应该有一�?`place-items` 属性，设置�?`center`�?
