...
以针�?`only screen` �?`@media` 查询开始，�?`max-width` 设置�?`720px`�?在里面，创建一�?`.image-wrapper` 选择器，并给它一个值为 `1fr` �?`grid-template-columns` 属性�?

这将在较小的屏幕上将三个图像排成一列�?

# --hints--

你应该有一个新�?`@media` 规则，条件是 `only screen and (max-width: 720px)`�?

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.media?.mediaText, 'only screen and (max-width: 720px)');
```

你的�?`@media` 规则应该有一�?`.image-wrapper` 选择器�?

```js
assert.equal(new __helpers.CSSHelp(document).getCSSRules('media')?.[0]?.cssRules?.[0]?.selectorText, '.image-wrapper');
```

你的�?`.image-wrapper` 选择器应该有一个值为 `1fr` �?`grid-template-columns` 属性�?
