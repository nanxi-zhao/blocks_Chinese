...
现在删除你的 `print(movie1)` 行。然后，将你的 `movie1` 赋值放在 `try` 块中，并创建一个捕获 `ValueError as e` 的 `except` 子句，使用 f-string 打印 `Validation Error:` 后跟一个空格和错误消息。

你可以随意更改传递给实例化 `movie1` 的参数，以查看不同的错误消息。

# --hints--

你的代码中不应该有 `print(movie1)`。
...
你应该有一个 `try` 块，其中包含 `movie1 = Movie('The Matrix', 1999, 'The Wachowskis', 136)`。
...
你应该有一个捕获 `ValueError as e` 的 `except` 子句。
...
你应该使用 f-string 在 `except` 块中打印 `Validation Error:` 后跟一个空格和错误消息。
