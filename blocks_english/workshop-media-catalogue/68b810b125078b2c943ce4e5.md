...
在 `TVSeries` 类中，创建一个 `__str__` 方法，返回格式为 `title (year) - S seasons, N episodes, duration min avg, director` 的字符串，其中 `title`、`year`、`S`、`N`、`duration` 和 `director` 应该分别被 `title`、`year`、`seasons`、`total_episodes`、`duration` 和 `director` 属性替换。

# --hints--

你的 `TVSeries` 类应该有一个 `__str__` 方法。
...
你的 `__str__` 方法应该返回格式为 `title (year) - S seasons, N episodes, duration min avg, director` 的字符串。
