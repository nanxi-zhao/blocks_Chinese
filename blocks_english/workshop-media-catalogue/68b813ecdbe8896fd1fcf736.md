...
使用 `catalogue` 的 `add` 方法将 `series1` 追加到 `items` 列表中。

然后，创建另一个变量 `series2`。为其分配另一个 `TVSeries` 实例，并将 `series2` 也追加到目录中。

# --hints--

你应该使用 `catalogue` 的 `add` 方法，传入 `series1`。
...
你应该创建一个名为 `series2` 的变量，并为其分配一个 `TVSeries` 实例。
...
你应该使用 `catalogue` 的 `add` 方法，传入 `series2`。
...
--fcc-editable-region--
    ...
    series2 = TVSeries('Friends', 1994, 'David Crane', 22, 10, 236)
