...
在将项目添加到目录之前，最好验证传递给 `add` 方法的项目。

创建一个 `if` 语句，使用 `isinstance` 来检查 `media_item` 是否是 `Movie` 的实例，如果是则抛出一个带有消息 `Only Movie or TVSeries instances can be added` 的 `TypeError`。

请注意，`isinstance(obj, ClassName)` 在 `obj` 是 `ClassName` 或其任何子类的实例时返回 `True`，因为它考虑了完整的继承链。

# --hints--

当 `media_item` 不是 `Movie` 或其子类的实例时，你应该抛出一个带有消息 `Only Movie or TVSeries instances can be added` 的 `TypeError`。
...
当 `media_item` 是有效项目时，你不应该抛出 `TypeError`。
