...
在之前的模块中，你学会了可以通过继承内置的 `Exception` 类来构建自己的自定义异常：
...
创建一个名为 `MediaError` 的类。让它继承自 `Exception` 类，并为其添加一个文档字符串：`Custom exception for media-related errors.`。
...
你的 `MediaError` 类应该继承自 `Exception`。
...
你应该向你的 `MediaError` 类添加文档字符串 `Custom exception for media-related errors.`。
