...
给你的 `MediaError` 类添加一个 `__init__` 方法，包含以下参数：`self`、`message` 和 `obj`。

在其中，调用 `super().__init__(message)` 以确保存储和显示错误消息的机制（已在 `Exception` 类中存在）能够正常工作。

之后，将 `obj` 分配给 `self.obj`。

# --hints--

你的 `MediaError` 类应该有一个 `__init__` 方法，包含 `self`、`message` 和 `obj` 参数。
...
你应该在你的 `__init__` 方法中调用 `super().__init__(message)`。
...
你应该在你的 `__init__` 方法中将 `obj` 分配给 `self.obj`。
