...
目前，在打印目录时，电影和电视系列剧都显示在一起。在接下来的步骤中，你将负责将项目按两个类别进行过滤并分别显示。

在 `MediaCatalogue` 类中，创建一个名为 `get_movies` 的方法，使其返回一个仅包含在 `self.items` 中找到的父类 `Movie` 实例的列表。

使用你喜欢的方法来实现这一点，但记住要使用 `type` 函数，因为 `isinstance` 无法区分父类和子类。

# --hints--

你的 `MediaCatalogue` 类应该有一个名为 `get_movies` 的方法。
...
你的 `get_movies` 方法应该返回一个包含 `self.items` 中所有 `Movie` 类实例的列表。
