...
创建另一个名为 `get_tv_series` 的方法，使其返回一个包含在 `self.items` 中找到的所有 `TVSeries` 实例的列表。

# --hints--

你的 `MediaCatalogue` 类应该有一个名为 `get_tv_series` 的方法。
...
你的 `get_tv_series` 方法应该返回一个包含 `self.items` 中所有 `TVSeries` 类实例的列表。
