...
使用相同的逻辑，创建一个 `if` 语句，当 `series` 列表不为空时运行。在其中，将字符串 `=== TV SERIES ===`（以换行符结尾）连接到 `result` 的当前值。

然后，创建一个 `for` 循环，使用枚举将 `series` 列表中的项目添加到 `result` 变量中。像之前一样，从 `1` 开始索引值。

这样，媒体目录工作坊就完成了。

# --hints--

你的 `__str__` 方法中应该有第二个 `if` 语句。
...
你的第二个 `if` 语句中应该有 `result += '=== TV SERIES ===\n'`。
...
当 `self.items` 包含 `TVSeries("Game of Thrones", 2011, "David Benioff & D.B. Weiss", 57, 8, 73)` 和 `TVSeries("Stranger Things", 2016, "The Duffer Brothers", 50, 4, 34)` 时，你的 `__str__` 方法应该返回 `Media Catalogue (2 items):
...
当 `self.items` 包含 `Movie('Barry Lyndon', 1975, 'Stanley Kubrick', 184)`、`Movie('Fahrenheit 451', 1966, 'Francois Truffaut', 112)`、`TVSeries("Game of Thrones", 2011, "David Benioff & D.B. Weiss", 57, 8, 73)` 和 `TVSeries("Stranger Things", 2016, "The Duffer Brothers", 50, 4, 34)` 时，你的 `__str__` 方法应该返回 `Media Catalogue (4 items):
