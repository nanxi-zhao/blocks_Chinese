...
如前面的课程所示，集合（set）是一个无序且不重复元素的集合：
...
我们将使用集合来确保每个字典没有多余或拼写错误的键。

使用 `set()` 构造函数，将应包含的键列表 `['patient_id', 'age', 'gender', 'diagnosis', 'medications', 'last_visit_id']` 转为集合，并赋值给变量 `key_set`。

# --hints--

请在 `validate` 函数中声明一个名为 `key_set` 的变量。
...
把 `set(['patient_id', 'age', 'gender', 'diagnosis', 'medications', 'last_visit_id'])` 赋值给 `key_set`。
