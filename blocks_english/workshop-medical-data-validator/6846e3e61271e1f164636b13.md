...
现在我们将把验证做得更细致一些。创建一个名为 `find_invalid_records` 的函数，用于查找字典中不合规范的值。函数应按顺序接收以下参数：`patient_id`、`age`、`gender`、`diagnosis`、`medications`、`last_visit_id`。

在函数内部，创建一个空字典 `constraints`，并从函数返回该字典。

# --hints--

请实现 `find_invalid_records` 函数。
...
函数应按顺序接收参数：`patient_id, age, gender, diagnosis, medications, last_visit_id`。
...
在函数内部声明一个空字典 `constraints`（例如 `constraints = {}` 或 `constraints = dict()`），并在函数末尾返回它。
