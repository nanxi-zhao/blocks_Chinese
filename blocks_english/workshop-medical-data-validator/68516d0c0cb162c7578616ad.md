...
最后在 `constraints` 字典中添加 `last_visit_id` 键。请使用 `isinstance(last_visit_id, str)` 来验证其类型为字符串。

# --hints--

请在 `constraints` 中添加 `last_visit_id` 键，并把它的值设为 `isinstance(last_visit_id, str)`。
