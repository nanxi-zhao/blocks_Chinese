...
在 `invalid_records` 变量之后，创建一个 `for` 循环来遍历它。对于每个无效记录，打印
`Unexpected format '<key>: <val>' at position <index>.`（其中 `<key>`、`<val>` 和 `<index>` 分别替换为当前的键、值和索引）。然后将 `is_invalid` 设为 `True`。

可以用包含无效数据的示例来测试 `validate` 函数，查看验证输出信息。

到此，医疗验证器练习即已完成。

# --hints--

你应该有一个遍历 `invalid_records` 的 `for` 循环。
...
在你的 `for` 循环内部，应打印 `Unexpected format '<key>: <val>' at position <index>.`（相应替换占位符）。
...
在新的 `for` 循环内应把 `is_invalid` 设为 `True`。
