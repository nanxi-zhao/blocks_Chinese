...
在本工作坊中，你将创建一个音乐播放器应用程序，能够播放、暂停和跳过播放列表中的歌曲。

该项目的HTML和CSS已经为你提供好了，因此你可以专注于JavaScript部分。

首先，获取 `#playlist-songs`、`#play` 和 `#pause` 元素，并将它们分别赋值给变量 `playlistSongs`、`playButton` 和 `pauseButton`。

太棒了！你已经学会了如何使用 `getElementById` 方法来获取DOM元素。这是构建交互式网页的重要第一步。继续加油！
...
你应该将 `#playlist-songs` 元素赋值给 `playlistSongs` 变量。
...
你应该将 `#pause` 元素赋值给 `pauseButton` 变量。
