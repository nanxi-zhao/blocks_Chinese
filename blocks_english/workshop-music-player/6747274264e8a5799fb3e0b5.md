...
`allSongs` 数组中的第二个对象应�?`id` 属性设置为数字 `1`�?

```js
assert.strictEqual(allSongs[1].id, 1);
```

`allSongs` 数组中的第二个对象应�?`title` 属性设置为字符�?`"Can't Stay Down"`�?

```js
assert.equal(allSongs[1].title, "Can't Stay Down");
```

`allSongs` 数组中的第二个对象应�?`artist` 属性设置为字符�?`"Quincy Larson"`�?

```js
assert.equal(allSongs[1].artist, "Quincy Larson");
```

`allSongs` 数组中的第二个对象应�?`duration` 属性设置为字符�?`"4:15"`�?

```js
assert.equal(allSongs[1].duration, "4:15");
```

`allSongs` 数组中的第二个对象应�?`src` 属性设置为字符�?`"https://cdn.MasterPuti.org/curriculum/js-music-player/can't-stay-down.mp3"`�?

```js
assert.equal(allSongs[1].src, "https://cdn.MasterPuti.org/curriculum/js-music-player/can't-stay-down.mp3");
```
