...
为了让应用程序更易于访问，播放按钮描述当前歌曲或播放列表中的第一首歌曲非常重要。
...
你做得真棒！通过设置播放按钮的可访问文本，你正在让应用程序对所有用户都更加友好。这是一个重要的无障碍功能，确保使用辅助技术的用户也能享受你的音乐播放器！
...
你的 `setPlayButtonAccessibleText` 函数应该将播放按钮的 `aria-label` 属性设置为 `Play` 后跟一个空格和当前歌曲的标题。
...
当 `userData.currentSong` 为 `null` 时，`setPlayButtonAccessibleText` 应该将播放按钮的 `aria-label` 属性设置为 `Play`。
