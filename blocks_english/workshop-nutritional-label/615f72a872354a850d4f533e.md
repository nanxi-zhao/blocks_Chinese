...
创建一个 `.daily-value p` 选择器来定位 `daily-value` 部分中的所有 `p` 元素。给这个新选择器设置 `border-bottom` 为 `1px solid #888989`。
...
你的 `.daily-value p` 选择器应该将 `border-bottom` 属性设置为 `1px solid #888989`。
