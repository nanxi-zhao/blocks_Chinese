...
你的 `% Daily Value *` 和 `Saturated Fat 1g 5%` 元素下方的底部分隔线没有延伸到标签的整个宽度。为这两个元素的 `class` 添加 `no-divider`。

# --hints--

你的文本为 `% Daily Value *` 的 `p` 元素应该在 `class` 属性中添加 `no-divider`。不要移除现有的类。
...
你的文本为 `Saturated Fat 1g 5%` 的 `p` 元素应该在 `class` 属性中添加 `no-divider`。不要移除现有的类。
