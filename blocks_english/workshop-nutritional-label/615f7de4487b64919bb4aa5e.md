...
在最后一个 `.divider` 之后，创建一个新的 `p` 元素，文本为 `Cholesterol 0mg 0%`。然后，将文本 `Cholesterol` 包装在 `span` 元素中，将 `0%` 包装在另一个 `span` 元素中，并给它们各自添加 `bold` 类。

最后，将包含文本 `Cholesterol` 的 `span` 元素与文本 `0mg` 一起嵌套在另一个用于对齐的 `span` 元素中。
...
你的新 `p` 元素应该有文本 `Cholesterol 0mg 0%`。
...
包含文本 `Cholesterol` 的 `span` 元素应该将其 `class` 属性设置为 `bold`。
...
包含文本 `0%` 的 `span` 元素应该将其 `class` 属性设置为 `bold`。
...
包含文本 `Cholesterol` 和文本 `0mg` 的 `span` 元素应该被包装在另一个 `span` 元素中。
