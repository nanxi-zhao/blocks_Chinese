...
[Set]构造函数也接受一个可迭代对象作为参数，并从中创建一个集合：
...
为了完成这个工作坊，修改你的[displayPlantsSet]函数，使其返回一个包含目录中所有植物唯一名字的集合。

为了实现这一点，创建一个数组，包含存储在[catalog]映射中的每个植物对象的[commonName]。将该数组传递给[Set]构造函数，并从函数中返回这个集合。

# --hints--

你的[displayPlantsSet]函数应该返回一个包含目录中每种植物[commonName]的集合。
