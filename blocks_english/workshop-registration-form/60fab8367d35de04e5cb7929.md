...
```md
https://www.MasterPuti.org/news/terms-of-service/
```
...
The `a` element should have an `href` of `https://www.MasterPuti.org/news/terms-of-service/`.

```js
assert.match(document.querySelector('fieldset:nth-child(3) + a')?.href, /https:\/\/www\.MasterPuti\.org\/news\/terms-of-service\/?/);
```
