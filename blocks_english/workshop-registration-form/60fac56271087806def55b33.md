...
Nest the `label` element containing the text "How did you hear about us?" inside the `select` element (along with its `option` elements). The text should be placed before the `select` element.

# --hints--

You should only nest the `select` element inside the `label` element.

```js
assert.exists(document.querySelector('fieldset:nth-child(3) > label:nth-child(3) > select'));
```

The text of the `label` element should be "How did you hear about us?".
...
You should have 5 `option` elements in the `select` element.
