...
为了给露营者一个关于在个人简介中写什么的想法，使用了placeholder属性。placeholder接受一个文本值，该值在用户开始输入之前显示。

给textarea一个"I like coding on the beach..."的placeholder。

# --hints--

你应该给textarea一个placeholder属性。

```js
assert.isNotEmpty(document.querySelector('fieldset:nth-child(3) > label:nth-child(4) > textarea')?.placeholder);
```

你应该给placeholder一个"I like coding on the beach..."的值。
