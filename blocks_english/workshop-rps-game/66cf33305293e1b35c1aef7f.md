...
现在是时候获取这一轮的结果了。完成 `getRoundResults` 函数。

如果玩家赢得这一轮，将 `playerScore` 增加 `1` 并返回消息 `"Player wins! [player's choice] beats [computer's choice]"`。

如果计算机和玩家选择相同的选项，返回消息 `"It's a tie! Both chose [player's choice]"`。

如果计算机赢得这一轮，将 `computerScore` 增加 `1` 并返回消息 `"Computer wins! [computer's choice] beats [player's choice]"`。

`[computer's choice]` 应该替换为 `computerResult`，而 `[player's choice]` 应该替换为 `userOption`。
...
你的 `getRoundResults` 函数应该根据谁赢得这一轮返回正确的消息。如果没有人赢，消息应该说明是平局。
