...
创建一个名为 `showResults` 的函数，该函数有一个参数 `userOption`。

在你的 `showResults` 函数内部，`roundResultsMsg` 应该用这一轮的结果来更新。

然后，`playerScoreSpanElement` 和 `computerScoreSpanElement` 也应该被更新，以显示玩家和计算机的更新后分数。

记住，这里的顺序很重要。你需要先更新 `roundResultsMsg`，然后是 `playerScoreSpanElement`，最后是 `computerScoreSpanElement`，因为 `roundResultsMsg` 将被用来确定分数。
...
你的 `showResults` 函数应该有一个名为 `userOption` 的参数。

```js
assert.match(code, /showResults\s*(?:=\s*)?(?:\(\s*)?userOption(?:\s*\))?\s*/);
```

你的 `showResults` 函数应该用这一轮的结果来更新 `roundResultsMsg`。
...
你的 `showResults` 函数应该更新 `computerScoreSpanElement` 以显示计算机的更新后分数。
...
你的 `showResults` 函数应该更新 `playerScoreSpanElement` 以显示玩家的更新后分数。
