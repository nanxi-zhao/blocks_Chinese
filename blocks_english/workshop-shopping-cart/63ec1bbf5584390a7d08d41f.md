...
要获取的最后一个元素是 `#show-hide-cart` 元素�?将其存储在名�?`showHideCartSpan` 的变量中�?

然后，使�?`let` 声明一个名�?`isCartShowing` 的变量并将其设置�?`false`�?

# --hints--

你应该使�?`document.getElementById()` 来获�?`#show-hide-cart` 元素�?

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)show-hide-cart\1\s*\)/);
```

你应该将 `#show-hide-cart` 元素分配给名�?`showHideCartSpan` 的变量�?记住使用 `const` 来声明变量�?

```js
assert.match(code, /const\s+showHideCartSpan\s*=\s*document\.getElementById\(\s*('|"|`)show-hide-cart\1\s*\)/);
```

你应该使�?`let` 来声明一个名�?`isCartShowing` 的变量�?

```js
assert.match(code, /let\s+isCartShowing/);
```

你应该将 `isCartShowing` 变量设置�?`false`�?
