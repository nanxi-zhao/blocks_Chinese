...
在你�?`h2` 元素之后，创建两�?`p` 元素�?为第一个元素指定一�?`dessert-price` �?`class`，并将文本设置为美元符号 `"$"` 后跟 `price` 变量的值�?为第二个元素指定一�?`product-category` �?`class`，以及文�?`"Category: "`，后�?`category` 变量的值�?
