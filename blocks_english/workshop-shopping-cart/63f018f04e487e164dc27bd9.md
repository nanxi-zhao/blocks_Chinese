...
在你的第一�?`p` 元素中，添加一�?`span` 元素�?�?`span` 赋予 `product-count` 类和 `id` �?`product-count-for-id${id}`�?然后，在 span 之后，将 `name` 变量的文本赋�?`p` 元素�?

# --hints--

你的第一�?`p` 元素应该有一�?`span` 元素�?
...
你的 `span` 元素应该具有 `product-count` �?`class`�?
...
你的 `span` 元素应该具有 `product-count-for-id${id}` �?`id`�?
...
你的第一�?`p` 元素应该包含 `name` 变量的文本�?这应该在 span 之外�?
