...
你需要获取之前添加到 DOM 的所�?`Add to cart` 按钮�?声明一�?`addToCartBtns` 变量，并为其分配�?`document` 对象上调�?`getElementsByClassName()` 方法的值，并传入字符串 `"add-to-cart-btn"`�?

# --hints--

你应该使�?`const` 来声明你�?`addToCartBtns` 变量�?

```js
assert.match(code, /const\s+addToCartBtns\s*=/);
```

你应该在 `document` 对象上调�?`getElementsByClassName()` 方法�?

```js
assert.match(code, /document\s*\.\s*getElementsByClassName\s*\(/);
```

你应该将字符�?`"add-to-cart-btn"` 传递给 `getElementsByClassName()` 方法�?
