...
将你的回调函数添加到 `forEach` 方法�?它应该采�?`btn` 参数�?然后，在回调中，�?`btn` 添加一个事件监听器�?事件监听器应该监�?`click` 事件，并且应该采用带�?`event` 参数的另一个回调�?第二个回调应该是空的�?

# --hints--

你应该使用箭头语法向采用 `btn` 参数�?`forEach` 方法添加回调函数�?
...
你应该向事件监听器添加一个空的回调函数�?记得给它一�?`event` 参数�?
