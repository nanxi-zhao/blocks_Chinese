...
你需要一种方法来访问购物车中的商品总数�?最好的方法是向 `ShoppingCart` 类添加另一种方法，而不是直接访�?`items` 数组�?

�?`ShoppingCart` 类添�?`getCounts` 方法�?它应该返�?`items` 数组中的项目数�?

# --hints--

你的 `ShoppingCart` 类应该有一�?`getCounts` 方法�?

```js
assert.isFunction(cart.getCounts);
```

请记住使�?`this` 关键字来访问 `items` 数组�?
