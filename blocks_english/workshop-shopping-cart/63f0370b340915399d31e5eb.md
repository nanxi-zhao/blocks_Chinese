...
你也将在此方法中更新 HTML�?�?`cartSubTotal` �?`textContent` 设置�?`subTotal` 的值，固定为小数点�?2 位�?使用模板字面量语法将美元符号添加到值的开头�?

# --hints--

你应该访�?`cartSubTotal` 元素�?`textContent` 属性�?

```js
assert.match(cart.calculateTotal.toString(), /cartSubTotal\.textContent/);
```

你应该在 `subTotal` 变量上调�?`.toFixed()` 方法，并�?`2` 作为参数传递�?

```js
assert.match(cart.calculateTotal.toString(), /subTotal\.toFixed\(\s*2\s*\)/);
```

你应该使用模板字面量语法�?`.toFixed()` 调用之前添加美元符号�?
...
你应该将 `cartSubTotal` 元素�?`textContent` 设置为你的模板字符串�?
