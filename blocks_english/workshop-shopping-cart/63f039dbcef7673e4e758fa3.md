...
浏览器具有内置的 `confirm()` 函数，可向用户显示确认提示�?`confirm()` 接受一个字符串，这是向用户显示的消息�?如果用户确认则返�?`true`，如果用户取消则返回 `false`�?

声明一个变�?`isCartCleared`，并为其分配使用字符�?`"Are you sure you want to clear all items from your shopping cart?"` 调用 `confirm()` 的值�?

# --hints--

你应该使�?`const` 来声�?`isCartCleared` 变量�?
...
你应该将字符�?`Are you sure you want to clear all items from your shopping cart?` 传递给 `confirm()` 函数�?
