...
在尝试更新到邻居的距离之前，你需要验证邻居既可到达又未被访问。然后你将计算通过当前节点到达该邻居的总距离。

在[for]循环内，添加一个[if]语句，检查：

- [distance]不等于[INF]（意味着节点之间有边）
- 邻居[node_no]尚未被访问

在[if]块内，创建一个名为[new_distance]的变量，并将其赋值为[distances[current]]（到当前节点的最短距离）和[distance]（从当前节点到邻居的距离）的和。

# --hints--

你应该有一个[if]语句，检查[distance]是否不等于[INF]以及邻居节点是否未被访问。
