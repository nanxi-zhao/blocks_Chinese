...
现在你已经计算出了到邻居的新可能距离，检查它是否比当前存储在[distances]列表中的距离更好。如果更好，则更新距离。

在现有的[if]块内，添加一个[if]语句，检查[new_distance]是否小于[distances[node_no]]。

在这个新的条件块内，更新[distances[node_no]]以存储[new_distance]。

# --hints--

你应该有一个嵌套的[if]语句，检查[new_distance]是否小于[distances[node_no]]。
...
你应该更新[distances[node_no]]以存储[new_distance]。
