...
在这一点上，你只想显示不是起始节点且可以从起始节点到达的节点的结果。

添加一个条件语句，检查[node_no]是否等于[start_node]或者[distances[node_no]]是否等于[INF]。
...
你的[if]语句应该检查[node_no]是否等于[start_node]或者[distances[node_no]]是否等于[INF]。
