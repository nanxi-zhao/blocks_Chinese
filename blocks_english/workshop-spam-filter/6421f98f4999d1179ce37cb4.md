...
数组有一个 `some()` 方法。就像 `filter()` 方法一样，`some()` 接受一个回调函数，该函数应该将数组的一个元素作为参数。如果回调函数对数组中的至少一个元素返回 `true`，那么 `some()` 方法将返回 `true`。太厉害了！

下面是一个使用 `.some()` 方法的示例，用于检查数组中是否有任何元素是大写字母。
...
更新你的 `isSpam()` 函数以使用 `some()` 方法，该方法将检查针对 `denyList` 中的任何正则表达式测试 `msg` 是否返回 `true` 吧！
