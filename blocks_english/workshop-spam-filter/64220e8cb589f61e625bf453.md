...
<dfn>捕获组</dfn>是一种定义表达式中应该被捕获并保存以供后续引用的部分的方法。你可以通过将表达式的一部分用括号包装来定义捕获组。例如，`/h(i|ey) camper/` 将匹配 `hi camper` 或 `hey camper`，并在组中捕获 `i` 或 `ey`。太聪明了！

在你的数字和 `dollars` 文本中，你想要捕获位值。

将捕获组添加到你的 `dollarRegex` 中，并在其中使用 `|` 符号来允许在数字和 `dollars` 文本之间有 `hundred`、`thousand`、`million` 或 `billion`。
...
你的 `dollarRegex` 应该使用 `|` 符号。

```js
assert.match(dollarRegex.source, /\|/);
```

你的 `dollarRegex` 应该有三个 `|` 符号。
