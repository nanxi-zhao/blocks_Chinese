...
垃圾邮件经常使用数字而不是字母来绕过过滤器。你的正则表达式应该能捕获这些情况。太棒了！

将你的正则表达式中的 `e` 字符替换为匹配 `e` 和 `3` 的字符类吧！

# --hints--

你的 `freeRegex` 应该使用一个字符类。

```js
assert.match(freeRegex.source, /\[.*\]/);
```

你的 `freeRegex` 应该使用一个字符类来匹配 `e` 和 `3`。
