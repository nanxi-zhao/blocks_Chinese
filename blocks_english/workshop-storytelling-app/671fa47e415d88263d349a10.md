...
接下来要开始写一点点 JavaScript 魔法啦！第一步就是把 `script.js` 文件接进页面。

创建一个 `script` 元素，并把它的 `src` 属性指向 `script.js`。例如：

```html
<script src="path-to-javascript-file.js"></script>
```

连上脚本后，你的小应用就具备“灵魂入口”了，继续冲鸭～

# --hints--

需要创建一个 `script` 元素（让 JS 能被加载）。

```js
assert.lengthOf(document.querySelectorAll('script'), 1);
```

你的 `script` 元素要有 `src="script.js"`。（路径对了才会生效哦～）
