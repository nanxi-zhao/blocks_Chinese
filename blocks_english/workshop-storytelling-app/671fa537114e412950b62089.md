...
# --description--
在你的 JavaScript 文件中，选中类名为 `.story-container` 的那个元素，并把它存到一个变量 `storyContainer` 里。这样后面我们就可以很方便地修改它的样式或属性啦。轻松的一步，继续保持节奏～

# --hints--
需要创建一个变量 `storyContainer` 来保存选到的元素。

```js
assert.isDefined(storyContainer);
```

要使用 `querySelector` 方法选中 `.story-container`，并存入 `storyContainer`。（标准姿势～）
