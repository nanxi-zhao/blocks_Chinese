...
# --description--
选中 `#result` 这个段落元素，并存入变量 `resultParagraph`。稍后当我们点击按钮，就会把对应故事文字塞进去展示。它就是“小故事展示屏”啦～继续保持你的节奏，稳稳推进！

# --hints--
用 `getElementById` 选中 `#result`，并保存到变量 `resultParagraph`。（命名清晰，后面不迷路！）
