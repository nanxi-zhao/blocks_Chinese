...
# --description--
确认按钮事件触发没问题后，就可以清理测试代码啦。把 `displayStory` 函数里用于临时验证的 `console.log` 语句删除，让函数为真正展示故事做好准备。精简代码 = 更优雅～

# --hints--
需要移除 `displayStory` 里的那条 `console.log("You clicked the button")` 测试输出。
