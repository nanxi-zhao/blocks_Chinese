...
# --description--
在 `displayStory` 函数内部，检查 `storyObj` 是否包含与传入的 `genre` 相匹配的属性；如果有，就把对应的故事文本写到 `#result` 里。

同时，把 `storyContainer` 的边框颜色修改为该类型数据里的 `borderColor`。这样每种故事都有自己的“氛围色”～

测试方式：调用一次 `displayStory("scary")`，此时边框应为 `#ee4b2b`，故事文本应是：`In the dark woods, a group of friends stumbled upon an old, abandoned cabin. They enter the cabin and awaken something malevolent that had been dormant for centuries.`。
完成这一步，你的应用已经“会换内容又会换样式”啦，超棒！

# --hints--
需要调用一次 `displayStory("scary")` 进行测试。

```js
assert.match(code, /displayStory\(["']scary["']\)/);
```

调用 `displayStory("scary")` 后，外层容器边框颜色应变为 `#ee4b2b`。
...
调用 `displayStory("scary")` 后，文本区域应显示对应的恐怖故事那一长句内容。
