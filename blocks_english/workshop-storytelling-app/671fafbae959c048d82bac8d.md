...
- 修改 `scaryStoryBtn` 的事件监听，让它在点击时执行 `displayStory("scary")`
- 给 `funnyStoryBtn` 添加 `click` 监听，执行 `displayStory("funny")`
- 给 `adventureStoryBtn` 添加 `click` 监听，执行 `displayStory("adventure")`

这样用户点不同按钮就能看到不同故事，还能感受不同的边框颜色氛围～

完成这些，你就成功完成整个工作坊啦！抱抱自己，超棒的坚持与实现！

# --hints--
点击 `scaryStoryBtn` 时：`#result` 应显示那段恐怖故事文本，并且边框颜色应为 `#ee4b2b`。
...
点击 `funnyStoryBtn` 时：`#result` 应显示对应的搞笑故事，边框颜色应为 `#f1be32`。
...
点击 `adventureStoryBtn` 时：`#result` 应显示对应的冒险故事，边框颜色应为 `#acd157`。
