...
接下来要让这个 `h1` 更“高调”一点：给它添加这些类——`text-3xl`（放大字号）、`text-center`（居中）、`font-semibold`（更粗）、`text-gray-900`（更深颜色）、以及在中等及更大屏幕用的 `md:text-5xl`（进一步加大）。这样标题在不同尺寸设备上都精神满满～
