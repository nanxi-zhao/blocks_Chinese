...
每个列表项起始的勾勾 `span` 只是装饰。给它们加上 `aria-hidden` 让屏幕阅读器忽略。

同时给这些 `span` 添加 `text-green-700` 与 `mr-2`，颜色醒目又留出右侧间距～

# --hints--

前两个功能项中的 `span` 都要具备属性 `aria-hidden`。
...
前两个功能项中的 `span` 都要包含类 `text-green-700`。
