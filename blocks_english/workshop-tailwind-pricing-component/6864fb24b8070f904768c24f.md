...
接下来轮到 **Premium Plan** 高级方案。

为它的外层 `div` 添加：`bg-gray-950`（深色背景）、`text-white`（文字白）、`ring-2` + `ring-fuchsia-500`（亮色描边）、`p-8`（内边距）。高端质感立刻呈现～

# --hints--

Premium 外层 `div` 需要类 `bg-gray-950`。
...
Premium 外层 `div` 需要类 `ring-fuchsia-500`。
