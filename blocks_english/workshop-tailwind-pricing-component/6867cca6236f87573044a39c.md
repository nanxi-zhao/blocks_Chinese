...
# --description--
最后一步：完善 Family 方案的 CTA 按钮。

为它添加这些类：`block`、`rounded-md`、`bg-indigo-600`、`px-4`、`py-2`、`text-center`、`font-semibold`、`text-white`、`hover:bg-indigo-700`，让按钮在交互与视觉上都足够清晰醒目。

至此，你的 Tailwind 定价组件工作坊圆满完成，棒棒哒！

# --hints--
该按钮链接需要类 `bg-indigo-600`。
