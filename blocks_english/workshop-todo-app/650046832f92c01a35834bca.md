...
# --description--
当用户删除任务时，也要同步更新本地存储，否则刷新后它还会"复活"。这里不需要 `removeItem()` 或 `clear()` —— 因为你已经用 `splice()` 把那条任务从数组里删掉了。所以只需再次执行一次保存：`localStorage.setItem('data', JSON.stringify(taskData))`。

重点：记得仍要 `JSON.stringify` 哦～

# --hints--
删除后需要再次调用 `localStorage.setItem()`。
...
第二个参数是 `JSON.stringify(taskData)`。
