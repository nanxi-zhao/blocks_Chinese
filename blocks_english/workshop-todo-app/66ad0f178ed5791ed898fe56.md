...
# --description--
还剩两个 Bug 要处理。第一个：如果标题输入的只是空格，虽然是必填项，还是会生成一条“空白任务”。

解决它：在函数开头加入判断 `if (!titleInput.value.trim())`。当去掉空格后没有任何字符，就弹出 `alert("Please provide a title")`，然后立刻 `return`，阻止继续创建。可爱的小守卫让数据更干净～

# --hints--
需要在 `addOrUpdateTask` 中添加 `if (!titleInput.value.trim())`。

```js
assert.match(code, /if\s*\(\s*!titleInput\.value\.trim\(\s*\)\s*\)\s*/)
```

在该分支中调用 `alert("Please provide a title")`。
