...
3. Camperbot 在对 `Background and Interests` 子标题使?`h3` 元素，但有一个语法错误?找到问题并解决它?
...
你不应该从页面删?`<h1>Hello from Camperbot!</h1>` 元素?
...
你应该有一?`h2` 元素，其文本?`About` ?这是一个示例：`<h2>Text here</h2>`

```js
assert.match(code, /\<h2\s*\>\s*About\s*\<\/h2\s*\>/i);
```

你应该有一个段落元素，文本?`My name is Camperbot and I love learning new things.` 。这是一个示例：`<p>Text here</p>`

```js
assert.match(code, /\<p\s*\>\s*My\s*name\s*is\s*camperbot\s*and\s*I\s*love\s*learning\s*new\s*things\s*\.\s*\<\/p\s*\>/i);
```

你应该有一个段落元素，文本?`I enjoy solving puzzles.` 。这是一个示例：`<p>Text here</p>`
...
你应该有一?`h3` 元素，其文本?`Background and Interests` ?这是一个示例：`<h3>Text here</h3>`
