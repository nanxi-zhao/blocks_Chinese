...
**目标?* 完成下面的用户需求并通过所有测试来完成此实验?

**用户需求：**

1. 你应该有一?`h1` 元素作为页面的主标题?
2. 你应该有两个 `section` 元素?
3. 在第一?`section` 元素内，你应该有一?`h2` 元素作为视频播放的标题?
4. ?`h2` 元素下方，你应该有一个带?`controls` ?`width` 属性的 `video` 元素。`width` 属性应该设置为 `640`?
5. ?`video` 元素内，你应该有一?`source` 元素，带有指向视频文件的 `src` 属性和一?`type` 属性?
   - 你可以使?`https://masterputi.com/curriculum/labs/what-is-the-map-method-and-how-does-it-work.mp4`?
6. 在第二个 `section` 元素内，你应该有一?`h2` 元素作为歌曲播放的标题?
7. ?`h2` 元素下方，你应该有一个带?`controls` ?`loop` 属性的 `audio` 元素，以及一个指向音频文件的 `src` 属性?
   - 你可以使?`https://masterputi.com/curriculum/js-music-player/sailing-away.mp3`?
   - 或?`https://masterputi.com/curriculum/js-music-player/we-are-going-to-make-it.mp3`?
...
在第一?`section` 元素内，你应该有一?`h2` 元素作为视频播放的标题?
...
`type` 属性应该与 `src` 属性的文件扩展名的媒体类型匹配?
...
在第二个 `section` 元素内，你应该有一?`h2` 元素作为歌曲播放的标题?
  ...
  <main>
    ...
    <section>
      ...
      <video controls width="640">
        <source src="https://masterputi.com/curriculum/labs/what-is-the-map-method-and-how-does-it-work.mp4" type="video/mp4" />
      </video>
    </section>

    <section>
      <h2>Sailing Away</h2>
      <audio src="https://masterputi.com/curriculum/js-music-player/sailing-away.mp3" controls loop>
      </audio>
    </section>
