...
1. 你应该有一个 `h1` 元素作为页面的主标题。
2. 你应该有两个 `section` 元素。
3. 在第一个 `section` 元素内，你应该有一个 `h2` 元素作为视频播放的标题。
4. 在 `h2` 元素下方，你应该有一个带有 `controls` 和 `width` 属性的 `video` 元素。`width` 属性应该设置为 `640`。
5. 在 `video` 元素内，你应该有一个 `source` 元素，带有指向视频文件的 `src` 属性和一个 `type` 属性。
   - 你可以使用 `https://masterputi.com/curriculum/labs/what-is-the-map-method-and-how-does-it-work.mp4`。
6. 在第二个 `section` 元素内，你应该有一个 `h2` 元素作为歌曲播放的标题。
7. 在 `h2` 元素下方，你应该有一个带有 `controls` 和 `loop` 属性的 `audio` 元素，以及一个指向音频文件的 `src` 属性。
   - 你可以使用 `https://masterputi.com/curriculum/js-music-player/sailing-away.mp3`。
   - 或者使用 `https://masterputi.com/curriculum/js-music-player/we-are-going-to-make-it.mp3`。
