...
1. 你应该有一?`main` 元素?
2. ?`main` 元素内部，你应该有一?`h1` 元素，用于显示电影标题?
3. ?`h1` 元素下面，你应该有一?`img` 元素，用于显示电影封面?你的 `img` 元素应包含描述?`alt` 文本描述图像?你可以随意使用此图片：`https://cdn.MasterPuti.org/curriculum/labs/rise-beyond-2.png`?
4. 你应该有一?`p` 元素，其中包含简短的电影描述?
5. 你应该使用另一?`p` 元素来显示电影评分?在其中，你应该按照所列顺序排列这些项目：
   - 一?`strong` 元素，文本为 `Movie Rating:`?
   - 一?`span` 元素，其 `aria-hidden` 属性设?`true`，并且包含评分的可视化表示（例如星星 `⭐⭐⭐⭐⭐⭐⭐⭐⭐☆`）?
   - ?span 之后，显示一个括号括起来的数字评分值，（例?`9.2/10`）?
6. 你应该有一?`h2` 元素，文本为 `Cast Members` ?
7. 你应该有一?`ul` 元素?
8. ?`ul` 元素内部，应该包含多?`li` 元素，每个都应包含一?`strong` 元素表示演员姓名，之后是对应的角色名，并且以 `as` 连接?（例如，`James Holloway as Ethan Carter`）?
...
?`h1` 元素下面，你应该有一?`img` 元素，并且带有描述性的 `alt` 属性?
...
在第二个 `p` 元素中，你应该有一?`strong` 元素，文本为 `Movie Rating:`?
...
评分段落元素中的 `span` 元素，应当包?10 个星星，可以是实心（⭐）或空心（☆），并且在 `span` 后显示一个括号括起来的数字评分值?
...
评分段落元素中的 `span` 元素应该有一?`aria-hidden` 属性，设置?`true`?
...
每个 `li` 元素应包含一?`strong` 元素表示演员姓名，并在其后用 `as` 连接角色名?（例如，`James Holloway as Ethan Carter`）?
...
<html lang="en">
  ...
  <body>
    <main class="review">
      ...
      <img
        src="https://cdn.MasterPuti.org/curriculum/labs/rise-beyond-2.png"
