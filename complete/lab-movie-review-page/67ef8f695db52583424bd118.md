...
3. 在 `h1` 元素下面，你应该有一个 `img` 元素，用于显示电影封面。你的 `img` 元素应包含描述性的 `alt` 文本描述图像。你可以随意使用此图片：`https://cdn.MasterPuti.org/curriculum/labs/rise-beyond-2.png`。
...
评分段落元素中的 `span` 元素，应当包含10 个星星，可以是实心（⭐）或空心（☆），并且在 `span` 后显示一个括号括起来的数字评分值。
...
评分段落元素中的 `span` 元素应该有一个 `aria-hidden` 属性，设为 `true`。
