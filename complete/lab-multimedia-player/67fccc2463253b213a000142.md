...
In the prior lessons, you were introduced to working with `audio` and `video` elements. 在本实验中，你将构建一个多媒体播放器，它会展示一?`audio` 曲目和一个带有字幕的 `video`?

对于 `audio` 元素，你需要包含一?`source` 元素，用于指定所使用的媒体文件?
...
`source` 元素同样可以?`video` 元素中使用，如下所示：
...
1. 你应该有一?`h1` 元素，作为页面的主标题?
2. 你应该有三个 `section` 元素?
3. 在第一?`section` 元素中，应该包含一?`h2` 元素，作为正在播放歌曲的标题?
4. 在该 `h2` 元素下方，应该有一?`audio` 元素，带?`controls` 属性和 `aria-label` 属性?
5. ?`audio` 元素内部，应该有一?`source` 元素，带有指向音频文件的 `src` 属性和 `type` 属性?你可以使用此音频文件：`https://cdn.MasterPuti.org/curriculum/js-music-player/sailing-away.mp3`
6. 在第二个 `section` 元素中，应该包含一?`h2` 元素，作为正在播放视频的标题?
7. 在该 `h2` 元素下方，应该有一?`video` 元素，带?`controls`、`width` 属性和 `aria-label` 属性?
8. ?`video` 元素内部，应该有一?`source` 元素，带有指向视频文件的 `src` 属性和 `type` 属性?You are free to use this video URL if you like: `https://cdn.MasterPuti.org/curriculum/labs/what-is-the-map-method-and-how-does-it-work.mp4`
9. ?`source` 元素下方，应该有一?`track` 元素，带有指向字幕文件的 `src` 属性、`kind` 属性、`srclang` 属性和 `label` 属性?
10. 在第三个 `section` 元素中，应该包含一?`h2` 元素，作为该部分的标题，例如?"Transcript"?
11. 在该 `h2` 元素下方，应该有一?`p` 元素，用来写视频的文稿?
...
你的 `track` 元素应该有一个描述字幕语言?`srclang` 属性?
...
你的 `track` 元素应该有一个描述字幕语言标签?`label` 属性?
...
<body>
  ...
  <section>
    ...
    <audio controls aria-label="Audio Player for Sailing Away">
        <source src="https://cdn.MasterPuti.org/curriculum/js-music-player/sailing-away.mp3" type="audio/mpeg">
    </audio>
  </section>
  ...
  <section>
    ...
    <video controls width="600" aria-label="What is a map method and how does it work?">
      <source src="https://cdn.MasterPuti.org/curriculum/labs/what-is-the-map-method-and-how-does-it-work.mp4" type="video/mp4">
