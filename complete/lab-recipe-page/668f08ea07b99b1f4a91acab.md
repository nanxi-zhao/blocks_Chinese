...
1. 你应该有一个文本为 `Instructions` 的第二个 `h2` 元素，用于操作步骤部分。
1. 在第二个 `h2` 元素下面，你应该有一个至少有四个列表项的有序列表（`ol` 元素），按顺序列出食谱步骤。
1. 你应该有一个 `img` 元素，它有一个设置为有效图像的 `src` 属性，如果你愿意可以使用 `https://masterputi.com/curriculum/labs/recipe.jpg`，和一个描述图像的 `alt` 属性。
