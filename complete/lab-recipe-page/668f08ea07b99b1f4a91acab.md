...
1. 你应该有一?`!DOCTYPE html` 声明?
1. 你应该有一?`lang` 设置?`en` ?`html` 元素?
1. 你应该有一?`head` 元素，其中包含一个用于你的食谱名称的 `title` 元素和一?`charset` 属性设置为 `UTF-8` ?`meta` 元素?
1. 你应该有一?`body` 元素?
1. 你应该有一个包含你的食谱名称的 `h1` 元素?
1. ?`h1` 下面，你应该有一个介绍食谱的 `p` 元素?
1. 你应该有一个文本为 `Ingredients` ?`h2`元素，用于原料部分?
1. 在第一?`h2` 元素下面，你应该有一个至少有四个列表项（`li` 元素）的无序列表（`ul` 元素），列出你的原料?
1. 你应该一个有文本?`Instructions` 的第二个 `h2` 元素，用于操作步骤部分?
1. 在第二个 `h2` 元素下面，你应该有一个至少有四个列表项的有序列表（`ol` 元素），按顺序列出食谱步骤?
1. 你应该有一?`img` 元素，它有一个设置为有效图像?`src` 属性，如果你愿意可以使?`https://masterputi.com/curriculum/labs/recipe.jpg`，和一个描述图像的 `alt` 属性?

# --hints--

你的食谱页面该有一?`!DOCTYPE html` 声明?
...
你的 `head` 元素中应该有一?`meta` 元素?

```js
assert.match(code, /<\s*head\s*>[\s\S]*<\s*meta[\s\S]*>[\s\S]*<\/\s*head\s*>/i);
```

你的 `meta` 元素应该将它?`charset` 属性设置为 `UTF-8`?
...
<body>
  ...
  <img src="https://cdn.MasterPuti.org/curriculum/labs/recipe.jpg" alt="Ingredients for baking: three eggs, a bowl of flour, a glass of milk, and a whisk arranged on a wooden table.">
  <h2>Ingredients</h2>
  <ul>
