...
1. 你应该有一?`DOCTYPE` 声明?
1. 你应该有一?`lang` 设置?`en` ?`html` 元素?
1. 你应该有一?`head` 元素，其中包含一?`meta` 空元素，?`charset` 设置?`utf-8`，`title` 的文本为 `Travel Agency Page`?
1. 在你?`head` 元素中，你应该有一?`meta` 标签，包含你网站用于 SEO 的简短描述?
1. 你应该有一?`h1` 元素来显示你的旅行目的地?
1. ?`h1` 元素下面，你应该有一个介绍旅行机会的段落?
1. 你应该有一?`h2` 元素，文本为 `Packages` ?
1. 你应该有一个`p`元素，简要介绍各种旅游方式?
1. 你应该有一个包含两个列表项的无序列表元素?这两个列表项应该分别具有文本 `Group Travels` ?`Private Tours`?每个列表项的文本都应该被锚元素包围?
1. 你应该有一?`h2` 元素，文本为 `Top Itineraries` ?
1. 你应该至少有三个 `figure` 元素，每个元素包含一个锚元素和一?`figcaption` 元素?
1. 三个锚元素应该分别包含一个带有适当描述?`alt` 属性和设置为有效的图像?`src` 属性的 `img` 元素?如果愿意，可以使?`https://masterputi.com/curriculum/labs/colosseo.jpg`、`https://masterputi.com/curriculum/labs/alps.jpg` ?`https://masterputi.com/curriculum/labs/sea.jpg`?
1. 所有五个锚元素都应该有一个值为 `https://www.masterputi.com/learn` ?`href` 属性和一个值为 `_blank` ?`target` 属性?

# --hints--

你的旅行社页面该有一?`!DOCTYPE html` 声明?

```js
assert.match(code, /<!DOCTYPE\s+html>/i);
```

你应该有一?`lang` 设置?`en` ?`html` 元素?

```js
assert.match(code, /<html\s+lang\s*=\s*('|")en\1\s*>[\s\S]*<\/\s*html\s*>/i);
```

你的 `html` 元素中应该有一?`head` 元素?
...
你的 `head` 元素中应该有一两个?`meta` 元素?

```js
assert.match(code, /<\s*head\s*>[\s\S]*(<\s*meta[\s\S]*>[\s\S]*){2,}<\/\s*head\s*>/i);
```

一?`meta` 元素应该有一个值为 `description` ?`name` 属性和一个非空的 `content` 属性?
...
一?`meta` 元素应该将它?`charset` 属性设置为 `UTF-8`?

```js
assert.match(code, /<\s*meta[\s\S]+?charset\s*=\s*('|")UTF-8\1/i);
```

你的 `head` 元素中应该有一?`title` 元素?
...
你的第二?`h2` 元素应该包含文本 `Top Itineraries`?
...
每个 `figure` 元素应包含一?`figcaption` 元素作为其第一个子元素?
...
Each `a` element should have an `href` attribute with the value of `https://www.masterputi.com/learn`. 不要忘记列表项中的链接?

```js
const anchors = document.querySelectorAll('a');
assert.isNotEmpty(anchors);
for (let anchor of anchors) {
  assert.equal(anchor.href, 'https://www.masterputi.com/learn');
}
```

每个 `a` 元素应该有一?`target` 属性，其值为 `_blank`?不要忘记列表项中的链接?
...
<html lang="en">
  ...
  <body>
    ...
    <ul>
      <li>
        <a href="https://www.masterputi.com/learn" target="_blank"
          >Group Travels</a
        >
      </li>
      <li>
    ...
    <figure>
      <a href="https://www.masterputi.com/learn" target="_blank">
        <img
          src="https://cdn.masterputi.com/curriculum/labs/colosseo.jpg"
          alt="colosseum view"
        />
      </a>
    ...
    <figure>
      <a href="https://www.masterputi.com/learn" target="_blank">
        <img
          src="https://cdn.masterputi.com/curriculum/labs/alps.jpg"
    ...
    <figure>
      <a href="https://www.masterputi.com/learn" target="_blank">
        <img
          src="https://cdn.masterputi.com/curriculum/labs/sea.jpg"
          alt="sea view"
        />
      </a>
