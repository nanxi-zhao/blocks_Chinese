...
“引起注意”元?`b` 通常用于突出显示摘要中的关键字或评论中的产品名称?通常，浏览器会以粗体显示其文本?下面是一个使?`b` 元素来突出显示评论中产品名称的示例：
...
如果需要强调文本的重要性，则应使用 `strong` 元素，而不?`b` 元素?
...
两者在外观上非常相似，因为默认都是以粗体显示?但它们的含义却截然不同?“引起注意”元素只是引起人们对文本的注意，并不表示更高的重要程度，?`strong` 元素的作用却远不止于此?它传达了一种重要性或紧迫感?这就是它们的主要区别?
...
`b` 用于强调视觉效果，?`strong` 则用于强调重要性?

---

`strong` 用于样式设计，?`b` 则用于强调重要性?
