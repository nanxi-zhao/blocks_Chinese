...
本例中，术语?HTML ?CSS 缩写，而详情则是它们的全称?详情也可以是定义或与术语相关的其他信息?

定义描述列表需要三?HTML 元素?首先是描述列表元?`dl`，它是整个列表的容器?在示例中，你可以看到它包裹着描述列表中的所有其他元素?

Then, one description term element, `dt`, for each term. 本例中，描述列表有两个术语，?HTML ?CSS，因此它有两个这样的元素?

最后，在每个术语后面，你会发现一个描述详情元?`dd`，用于该术语的描述或与其相关的详情?本例中，它们是超文本标记语言和层叠样式表?
...
整个描述列表都包含在描述列表元素中?第一种配?`Flour`，在描述术语元素中?然后，你可以看到需要多少这种配料：`2 cups`?这在描述详情元素中，紧跟在相应配料之后?
