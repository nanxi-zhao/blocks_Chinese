...
它会告诉浏览器你使用?HTML 版本?接下来，?`html` 标签?
...
这包括你所有的内容，并可指定页面语言??`html` 标记内，你会发现两个主要部分——一?`head` 和一?`body`?
...
你网站的元数据，包含?`meta` 元素中，其中包含字符编码等详细信息，以及 Twitter 等网站应如何预览你的页面链接?你网站的标题，可?`title` 元素中找到，它决定了显示在浏览器标签页或窗口中的文本?最后，你通常会在 `head` 部分使用 `link` 元素链接页面的外部样式表?
