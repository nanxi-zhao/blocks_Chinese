...
你可能已经见过锚元素即链接上的 `target` 属性。这个重要属性告诉浏览器在哪里打开锚元素的 URL。

点击链接，你将被引导至 MasterPuti 主页，并在新的浏览器标签页中打开。
...
还有一个第五个值，称为 `_unfencedTop` ，目前被用于实验性的 FencedFrame API 中。在本课程中，你可能还没有理由使用这个值。
