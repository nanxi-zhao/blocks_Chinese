...
绝对路径还会包含协议——该协议可以是 `http`、`https` 或 `file`，如果资源位于网络上，还会包含域名。这是一个指向 MasterPuti logo 的绝对路径示例：
...
在这个示例中，协议是 `https` ，域名是 `design-style-guide.MasterPuti.org` ，文件名是 `fcc_secondary_small.svg`。
...
One type starts from the root directory, and the other depends on the current location.
