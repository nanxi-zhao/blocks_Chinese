...
创建一个名?`botLocationSentence` 的变量?

然后使用 `+` 运算符进行字符串连接，连接字符串 `"I live in "` ?`botLocation` 变量，之后是一个句号（`.`）?
