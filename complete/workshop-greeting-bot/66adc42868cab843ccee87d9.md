...
首先创建一个名?`nicknameIntroduction` 的新变量?

使用字符串连接将字符?`"My nickname is "` ?`bot` 变量连接，后跟一个句号（`.`）?

将结果字符串赋值给 `nicknameIntroduction` 变量?

然后，将 `nicknameIntroduction` 的值输出到控制台?

# --hints--

你应该有一个名?`nicknameIntroduction` 的变量?
...
你应该使用字符串连接将字符串 `"My nickname is "` ?`bot` 变量连接，后跟一个句号（`.`）。注意空格?
