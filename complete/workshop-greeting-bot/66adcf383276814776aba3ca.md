...
接下来，创建一个名?`favoriteSubjectSentence` 的变量?

使用字符串连接将字符?`"My favorite subject is "` ?`favoriteSubject` 变量连接，后跟一个句号?

将结果赋值给 `favoriteSubjectSentence` 变量?

然后，将 `favoriteSubjectSentence` 的值输出到 `console`?

# --hints--

你应该有一个名?`favoriteSubjectSentence` 的变量?
