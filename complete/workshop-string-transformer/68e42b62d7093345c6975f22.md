...
继续保持势头！在你的 `originalString` 变量下面，添加一?`console.log()`，输出字符串 `"Original string:"`，像是在为原始内容挂上一块可爱的标签?

然后再在它下面添加一?`console.log()`，输出变?`originalString`，这样我们就能在控制台里看到它的闪亮登场。一步一步来，你已经做得很棒?
...
你需要在控制台输?`"Original string:"` 这一行，给原始字符串一个暖心的开场白?

```js
assert.equal(getLogs()[0], "Original string:");
```

你还需要在控制台输?`originalString` 变量，让它正式亮相。继续加油！
