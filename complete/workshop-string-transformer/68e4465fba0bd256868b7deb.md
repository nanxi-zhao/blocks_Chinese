...
创建一个名?`learningSentence` 的变量，并赋值为 `"I love learning!"`?

然后依次输出 `"Original learning sentence:"` ?`learningSentence`。保持耐心，你的进步在一点点积累?
...
你需要将字符?`"I love learning!"` 赋值给 `learningSentence`?

```js
assert.equal(learningSentence, "I love learning!");
```

你需要输?`"Original learning sentence:"`?

```js
assert.equal(getLogs()[8], "Original learning sentence:");
```

你还需要输?`learningSentence` 的内容，让它被看到。继续保持笑容！
