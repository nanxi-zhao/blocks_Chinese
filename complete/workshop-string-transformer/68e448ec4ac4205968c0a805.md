...
如果句子中`"love"` 这个词能重复三次，是不是更充满热情呢？让我们来实现它。
...
现在创建一个名为`repeatedLove` 的变量，并赋值为 `"love ".repeat(3)`。再输出它看看成果，多出来的热情正在闪光。
...
你应该创建一个名为`repeatedLove` 的变量，承载满满的"love"。
...
你还需要输出`repeatedLove`，欣赏一下它的样子。太可爱啦！
