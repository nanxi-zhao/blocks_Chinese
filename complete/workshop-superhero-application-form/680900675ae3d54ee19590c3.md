...
在构建表单本身之前，你将创建所需?state 变量。它们是 `heroName`、`realName`、`powerSource` ?`powers`?

现在，为 `heroName` ?`realName` 创建变量?setter。两者都应该有空字符串的初始值?

# --hints--

你应该使用数组解构语法来设置 `heroName` state 变量?`setHeroName` setter，你真棒?

```js
assert.match(code, /(const|let|var)\s+\[\s*heroName\s*,\s*setHeroName\s*\]/);
```

你的 `heroName` ?`setHeroName` 应该使用 `useState` hook，继续努力！
...
你的 `heroName` ?`useState` hook 应该有空字符串的初始值，太棒了！
...
你应该使用数组解构语法来设置 `realName` state 变量?`setRealName` setter，做得很好！

```js
assert.match(code, /(const|let|var)\s+\[\s*realName\s*,\s*setRealName\s*\]/);
```

你的 `realName` ?`setRealName` 应该使用 `useState` hook，你真聪明！
...
你的 `realName` ?`useState` hook 应该有空字符串的初始值，太棒了！
