...
是时候创建标签和输入框了。在表单中，创建一个文本为 `Hero Name` ?`label` 和一个类型为 `text` ?`input` 元素?

通过?`Hero Name` ?`input` 设置 `value` 属性为 `heroName`，将其连接到 `heroName` state 变量?
...
你的 `label` 元素应该?`Hero Name` 作为其文本，继续加油?
...
你应该在 `label` 元素内创建一个类型为 `text` ?`input` 元素，做得很好！
...
你的 `input` 元素应该将其值设置为 `heroName` state 变量，你真聪明！
