...
你可能注意到你无法在 `input` 中输入。如果你打开控制台，你会看到一个警告?

要移除警告并修复输入问题，使?`onChange` 事件来更?`heroName`。`onChange` 事件的值应该是一个以 `e` 为参数的箭头函数。箭头函数应该隐式返?`setHeroName(e.target.value)`?

# --hints--

你的 `input` 元素应该有一?`onChange` 属性，太棒了！

```js
assert.match(code, /onChange\s*=\s*\{/)
```

你应该使用带箭头函数?`onChange` 来隐式设?`heroName` state 变量的值，继续加油?
