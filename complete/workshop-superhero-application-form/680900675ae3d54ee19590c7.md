...
创建另一?`label`，其中包含文?`Real Name` 以及类型?`password` ?`input`??`input` 一?`value` ?`realName`?

然后使用 `onChange` 事件以与更新 `heroName` 相同的方式更?`realName`?
...
你的 `label` 元素应该?`Real Name` 作为其文本，继续加油?
...
你应该在 `label` 元素内创建一个类型为 `password` ?`input`，做得很好！
...
你的 `input` 元素的值应设置?`realName` 状态变量，你真聪明?

```js
assert.match(code, /value\s*=\s*\{\s*realName\s*\}/)
```

你应该使?`onChange` 事件来设?`realName` 状态变量的值，以便你可以在 `input` 字段中输入，太棒了！
