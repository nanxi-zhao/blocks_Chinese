...
接下来，使用 `section` ?`column` 作为 `className` 以及文本 `How did you get your powers?` 创建 `label`??`label` 内，创建一?`select` 元素?

# --hints--

你应该创建一?`label` 元素，其中包?`section` ?`column` ?`className`，太棒了?
...
你的 `label` 元素应该包含文本 `How did you get your powers?`，继续加油！
...
你应该在 `label` 元素内创建一?`select` 元素，做得很好！
