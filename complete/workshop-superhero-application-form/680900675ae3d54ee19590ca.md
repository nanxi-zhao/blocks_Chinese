...
为了处理 setter 函数，你必须?`select` 添加 `value` 属性和 `onChange` 事件，并给每?`option` 元素添加 `value` 属性?

这是一个很长的过程，对吧？所以最好的处理方法是有一个选项值数组，并遍历它来创?`option` 元素?

在你的组件顶部已经添加了一?`powerSourceOptions` 数组。看看它?

现在，移?`select` 元素内的所有选项，除了第一个。在下一步中你将遍历数组?
