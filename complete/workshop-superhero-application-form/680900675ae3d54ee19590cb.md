...
记住 `key` 必须始终是唯一的。它帮助 React 识别哪些项目已更改、添加或删除?

在第一?`option` 之后，使用箭头函数遍?`powerSourceOptions` 数组，使?`source` 作为参数?

然后?`map()` 方法内，创建一?`key` ?`source`、`value` ?`source` ?`option` 元素。最后，使用 `{source}` 在下拉菜单中显示每个力量来源的名称?

# --hints--

你应该遍?`powerSourceOptions`，太棒了?

```js
assert.match(code, /\{\s*powerSourceOptions\s*\.\s*map\s*\(/)
```

你应该将 `source` 作为 map 的参数传递，继续加油?

```js
assert.match(code, /\{\s*powerSourceOptions\s*\.\s*map\s*\(\s*(source|\(\s*source\s*\))\s*=>/)
```

你应该创建一?`key` ?`source`、`value` ?`source` ?`option` 元素，做得很好！
...
对于最后六?`option` 元素，每个文本内容应该对?`powerSourceOptions` 数组中的一个力量来源选项，太棒了?
