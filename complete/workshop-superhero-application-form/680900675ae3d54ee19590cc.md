...
现在，使?`powerSource` 作为 `select` 元素的值。然后，使用箭头函数设置 `onChange` 处理程序，该函数?`e` 为参数并隐式返回 `setPowerSource(e.target.value)`?

# --hints--

你应该使?`{powerSource}` 作为 `select` 元素的值，太棒了！

```js
assert.match(code, /<\s*select\s+[\s\S]*value\s*=\s*\{\s*powerSource\s*\}/)
```

你应该使用带箭头函数?`onChange` 来设?`select` 元素的值，该函数隐式返回以使用 `setPowerSource` 更新 `powerSource` 状态，继续加油?
