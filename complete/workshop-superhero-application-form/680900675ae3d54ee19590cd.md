...
接下来要添加的输入是一组复选框，用于选择适用的超级英雄能力。

创建一个 `label`，`className` 为 `section column`，文本为 `List your powers (select all that apply):`。

# --hints--

你需要创建第二个带有 `section column` 类名的 `label`，太棒了！
...
该 `label` 文本应为 `List your powers (select all that apply):`，继续加油！
