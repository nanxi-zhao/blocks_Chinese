...
下一?`input` 是一个复选框，用于标记适用于用户的所有超级英雄力量?

创建一?`label` 元素，其?`className` ?`section` ?`column`，文本为 `List your powers (select all that apply):`?
...
你的 `label` 元素应该包含文本 `List your powers (select all that apply):`，继续加油！
