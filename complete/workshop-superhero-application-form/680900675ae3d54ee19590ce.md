...
将有六个复选框，所以最好为它们创建一个数组并遍历它们以显示为复选框。为此，已为你提供了 `powersOptions` 数组?

使用箭头函数遍历 `powersOptions` 数组，使?`power` 作为参数。在 map 内部，创建一?`key` ?`power` ?`label` 元素，文本为 `Hello`?

之后，你应该看到六个 `Hello` 文本。你将在下一步中填充数组的元素?

# --hints--

你应该遍?`powersOptions`，太棒了?

```js
assert.match(code, /\{\s*powersOptions\s*\.\s*map\s*\(/)
```

你应该使?`power` 作为 map 的参数，继续加油?

```js
assert.match(code, /{\s*powersOptions\s*\.\s*map\s*\(\s*(power|\(\s*power\s*\))\s*=>/)
```

你应该创建一?`key` ?`power` ?`label` 元素，做得很好！

```js
assert.match(code, /\{\s*powersOptions\s*\.\s*map\s*\([\s\S]*<\s*label\s+key\s*=\s*\{\s*power\s*\}\s*>[\s\S]*<\s*\/\s*label\s*>/)
```

你的 `label` 元素应该?`Hello` 文本，你真聪明！
