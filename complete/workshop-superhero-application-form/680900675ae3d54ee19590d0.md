...
要修复错误，创建一个带 `e` 参数?`handlePowersChange` 箭头函数?

在函数内部，?`e.target` 解构 `value` ?`checked` 来获取复选框的值以及是否被选中?

# --hints--

你应该创建一个带 `e` 参数?`handlePowersChange` 函数，太棒了?

```js
assert.match(code, /(const|let|var)\s+handlePowersChange\s*=\s*(e|\(\s*e\s*\))\s*=>\s*{/)
```

你应该从 `e.target` 解构 `value` ?`checked`，继续加油！
