...
接下来，告诉表单如何提交。给表单元素添加一个值为 `post` ?`method` 属性和一个值为 `https://superhero-application-form.MasterPuti.org` ?`action` 属性?

现在，提交表单将发送到这个 URL。你将在下一步中执行此操作?

# --hints--

你的 `form` 元素应该有一个设置为 `post` ?`method` 属性，太棒了！
...
你的 `form` 元素应该有一个设置为 `https://superhero-application-form.MasterPuti.org` ?`action` 属性，继续加油?

```js
const formEl = document?.querySelector("form")
assert.equal(formEl?.getAttribute("action"), "https://superhero-application-form.MasterPuti.org")
```
