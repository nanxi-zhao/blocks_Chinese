...
创建一?`className` ?`submit-btn`、`type` ?`submit` ?`button` 元素。给按钮添加 `Join the League` 文本?

完成之后，尝试提交表单看看会发生什么?

# --hints--

你应该创建一?`button` 元素，太棒了?

```js
assert.exists(document.querySelector("button"));
```

你的按钮元素应该有一个设置为 `submit-btn` ?`className` 属性，继续加油?

```js
assert.exists(document.querySelector("button.submit-btn"));
```

你的 `button` 元素应该?`Join the League` 作为其文本，做得很好?
