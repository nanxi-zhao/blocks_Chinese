...
你是否注意到你可以不填写输入框就提交表单?

为了阻止这种情况，添?`disabled` 属性，如果 `heroName`、`realName` ?`powerSource` 为假，或?`powers` 的长度为 `0`，则禁用提交按钮?

现在，如果某些内容未填写，按钮将被禁用。填写表单并提交以加入联盟?
...
你的 `button` 元素应该有一?`disabled` 属性，继续加油?

```js
assert.match(code, /<\s*button\s+className\s*=\s*('|")\s*submit-btn\s*\1\s*type\s*=\s*('|")\s*submit\s*\2\s*disabled/)
```

如果 `heroName`、`realName` ?`powerSource` 中的任何一个为假，或?`powers` 的长度为 `0`，你应该禁用 `button`，做得很好！
  ...
  return (
    <div className='form-wrap'>
      ...
      <p>Please complete all fields</p>
       <form method='post' action='https://superhero-application-form.MasterPuti.org'>
