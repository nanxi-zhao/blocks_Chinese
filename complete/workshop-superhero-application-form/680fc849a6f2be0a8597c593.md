...
现在，为 `powerSource` ?`powers` 创建 state 变量?setter。`powerSource` 应该有空字符串的初始值，`powers` 应该有空数组的初始值?

# --hints--

你应该使用数组解构来设置 `powerSource` state 变量?`setPowerSource` setter，太棒了?

```js
assert.match(code, /(const|let|var)\s+\[\s*powerSource\s*,\s*setPowerSource\s*\]/);
```

你的 `powerSource` ?`setPowerSource` 应该使用 `useState` hook，继续加油！
...
你的 `powerSource` ?`useState` hook 应该有空字符串的初始值，做得很好?
...
你应该使用数组解构来设置 `powers` state 变量?`setPowers` setter，你真聪明！

```js
assert.match(code, /(const|let|var)\s+\[\s*powers\s*,\s*setPowers\s*\]/);
```

你的 `powers` ?`setPowers` 应该使用 `useState` hook，太棒了?
...
你的 `powers` ?`useState` hook 应该有空数组的初始值，继续努力?
