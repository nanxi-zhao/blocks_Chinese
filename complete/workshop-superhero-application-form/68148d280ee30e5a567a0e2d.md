...
移除 `label` 内的 `Hello` 文本，然后创建一个 `type='checkbox'`、`value={power}` 的 `input`。

最后在关闭 `label` 前创建一个包含 `{power}` 的 `span`，用于在复选框旁显示能力名称。

# --hints--

你需要移除 `label` 中的 `Hello` 文本，太棒了！

```js
assert.notMatch(code, /hello/i)
```

你需要创建一个 `type='checkbox'`、`value={power}` 的 `input` 作为 `label` 的第一个子元素，继续加油！

```js
assert.match(code, /<\s*label\s+key\s*=\s*{\s*power\s*}\s*>\s*<\s*input\s+type\s*=\s*("|')checkbox\1\s*value\s*=\s*\{\s*power\s*\}\s*\/>[\s\S]*<\/\s*label\s*>/)
```

你需要创建一个文本内容为 `{power}` 的 `span` 作为 `label` 的最后一个子元素，做得很好！
