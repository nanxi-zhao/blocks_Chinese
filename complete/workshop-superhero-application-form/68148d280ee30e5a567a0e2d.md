...
移除 `label` 内的 `Hello` 文本，然后创建一个类型为 `checkbox`、值为 `power` ?`input`?

最后，通过在闭?`label` 之前创建一个文本内容为 `{power}` ?`span` 元素，来显示数组中的值在每个复选框旁边?

# --hints--

你应该从 `label` 元素中移?`Hello` 文本，太棒了?

```js
assert.notMatch(code, /hello/i)
```

你应该创建一?`type` 设置?`checkbox`、`value` 设置?`power` ?`input` 作为 label 的第一个子元素，继续加油！

```js
assert.match(code, /<\s*label\s+key\s*=\s*{\s*power\s*}\s*>\s*<\s*input\s+type\s*=\s*("|')checkbox\1\s*value\s*=\s*\{\s*power\s*\}\s*\/>[\s\S]*<\/\s*label\s*>/)
```

你应该创建一个文本内容为 `{power}` ?`span` 元素作为 label 的最后一个子元素，做得很好！
