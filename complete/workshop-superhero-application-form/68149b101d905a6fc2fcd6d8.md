...
在解构的 `value` ?`checked` 下面，更新已选中力量的列表?

调用 `setPowers` 函数，它接受一个数组参数。使用三元运算符检?`checked` 是否为真。如果是，将现有?`powers` 数组展开到一个新数组中并添加 `value`。如果不是，则使?`powers.filter(p => p !== value)` ?`powers` 中过滤掉 `value`?

# --hints--

你应该使用三元运算符来检?`checked` 是否为真。如果是，则将现有的 `powers` ?`value` 展开到数组中。如果不是，则从 `powers` 中过滤掉 `value`，太棒了?
