...
在解构出 `value` 和 `checked` 之后，需要更新已选能力的列表。

调用 `setPowers`，使用三元运算符检查 `checked`：若为真，将现有 `powers` 展开并追加 `value`；否则用 `powers.filter(p => p !== value)` 过滤掉该项。

# --hints--

你需要使用三元运算符：`checked ? [...powers, value] : powers.filter(p => p !== value)` 来更新数组，太棒了！
