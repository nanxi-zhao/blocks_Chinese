...
继续前进！这一小步我们再收集几位关键伙伴：`close-task-form-btn`、`add-or-update-task-btn` ?`cancel-btn`。它们分别负责关闭、提交或更新，以及取消操作?

请继续用 `getElementById()` 把它们收入囊中，并存到变?`closeTaskFormBtn`、`addOrUpdateTaskBtn`、`cancelBtn`。你正在让界面逐渐“活”起来，做得很棒?

# --hints--

先拿到关闭按?`close-task-form-btn`?

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)close\-task\-form\-btn\1\s*\)\s*;?/)
```

保存?`closeTaskFormBtn`，用 `const` 更安心?

```js
assert.match(code, /const\s+closeTaskFormBtn\s*=\s*document\.getElementById\(\s*('|"|`)close\-task\-form\-btn\1\s*\)\s*;?/)
```

再获取提?更新按钮 `add-or-update-task-btn`?

```js
assert.match(code, /document\.getElementById\(\s*('|"|`)add\-or\-update\-task\-btn\1\s*\)\s*;?/)
```

保存?`addOrUpdateTaskBtn`，继续稳步推进～
