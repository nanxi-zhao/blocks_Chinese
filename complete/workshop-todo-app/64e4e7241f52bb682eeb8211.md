...
继续加油，我们要收集最后一组表单帮手：`date-input` ?`description-input`！它们负责日期与描述，让任务更完整?
?`getElementById()` 获取它们，并分别存入变量 `dateInput`、`descriptionInput`。这一小步完成后，你的表单元素团队就齐了，给自己一个小赞✧?
# --hints--

你应该使?`getElementById()` 来获?`date-input` 元素?
```js
...
将它保存到变?`descriptionInput`，同样用 `const`。元素收集完成，太棒啦！
