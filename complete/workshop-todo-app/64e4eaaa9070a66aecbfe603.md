...
如果用户点击 `Cancel`，我们就温柔地关闭确认框，让他们继续编辑。`dialog` 元素?`close()` 方法可以用来关闭模态窗口：

```js
dialogElement.close();
```

?`cancelBtn` 添加一个监?`click` 的事件，回调里调?`confirmCloseDialog.close()` 即可?
这一步让交互更人性化，继续保持节奏，你很棒！

# --hints--

你应该在 `cancelBtn` 上调?`addEventListener()`?
```js
...
回调函数应调?`confirmCloseDialog.close()`?
```js
