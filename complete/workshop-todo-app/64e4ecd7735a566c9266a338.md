...
现在你已经可以熟练地打开和关闭表单了，下一步我们来抓取用户输入的内容，把它们存?`taskData` 数组，并稍后显示在页面上?
首先?`taskForm` 添加一个监?`submit` 的事件，使用箭头函数并传入参?`e`。在回调里调?`e.preventDefault()` 阻止表单提交后页面刷新?
这一步是数据处理的第一块基石，继续保持耐心与热情！

# --hints--

你应该在 `taskForm` 上调?`addEventListener()`?
```js
