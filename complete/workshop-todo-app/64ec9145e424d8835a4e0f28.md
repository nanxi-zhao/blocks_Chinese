...
任务已经被存入 `taskData`，接下来要把它们展示到页面上哦！

请在 `taskData` 使用 `forEach()`，并在回调的参数位置做对象解构，拿取 `id`, `title`, `date`, `description`。这一步是后面动态渲染的基础，先不用返回任何内容，只是准备遍历结构。

继续推进，你已进入"展示范"阶段，冲刺中！

# --hints--

需要在 `taskData` 上调用 `forEach()`。

```js
assert.match(code, /taskData\.forEach\(\s*/)
```

回调参数应解构为 `{id, title, date, description}`。
