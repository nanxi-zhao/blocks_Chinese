...
浠诲姟宸茬粡琚瓨鍏?`taskData`锛屾帴涓嬫潵瑕佹妸瀹冧滑灞曠ず鍒伴〉闈笂鍟︼?
璇峰?`taskData` 浣跨?`forEach()`锛屽苟鍦ㄥ洖璋冪殑鍙傛暟浣嶇疆鍋氬璞¤В鏋勶紝鎷垮?`id`, `title`, `date`, `description`銆傝繖涓€灏忔鏄悗闈㈠姩鎬佹覆鏌撶殑鍩虹锛屽厛涓嶇敤杩斿洖浠讳綍鍐呭锛屽彧鏄噯澶囬亶鍘嗙粨鏋勩?
缁х画鎺ㄨ繘锛屼綘宸茶繘鍏モ€滃睍绀哄眰鈥濋樁娈碉紝鍐查腑锛?
# --hints--

闇€瑕佸?`taskData` 涓婅皟鐢?`forEach()`?
```js
assert.match(code, /taskData\.forEach\(\s*/)
```

鍥炶皟鍙傛暟搴旇В鏋勫?`{id, title, date, description}`?
```js
