...
另外，在 `discardBtn` 的事件监听里也要移除切换 `taskForm` ?`hidden` 类的那行代码，改为调?`reset`。因为点?`Discard` 时，表单里填的东西都应该被清掉，状态回到初始。让放弃操作也变得干净利落?
# --hints--

你应该删除在 `discardBtn` 监听中切?`taskForm` ?`hidden` 类的代码?
```js
