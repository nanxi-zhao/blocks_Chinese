...
只有在输入框里真的有内容的时候，才应该弹出包?`Cancel` ?`Discard` 按钮的确认对话框?
先删除那?`confirmCloseDialog.showModal();`?
接着?`closeTaskFormBtn` 的事件监听里写逻辑：检?`titleInput`、`dateInput` ?`descriptionInput` 是否至少有一个存在值?
如果任意一个有值，就调?`confirmCloseDialog.showModal()` 显示对话框；否则（没有改动），调?`reset()` 清空输入并关闭表单。这样用户体验就超级友好啦～

# --hints--

如果任意一个输入框有值，就应该显示确认对话框；否则应该调?`reset` 函数?
```js
