...
请使用箭头函数语法创建一?`addOrUpdateTask` 函数，并?`dataArrIndex` 变量、`taskObj` 对象以及那个判断 `dataArrIndex === -1` ?`if` 语句都移动进去。结构清晰后后面扩展会轻松许多，继续加油呀?
# --hints--

你应该使?`const` 和箭头函数语法来创建一?`addOrUpdateTask` 函数?
```js
assert.match(code, /const\s+addOrUpdateTask\s*=\s*\(\s*\)\s*=>\s*\{\s*/)
```

你应该把 `dataArrIndex` 变量移动?`addOrUpdateTask` 函数内部?
```js
...
你应该把 `taskObj` 对象移动?`addOrUpdateTask` 函数内部?
```js
...
你应该把带条?`dataArrIndex === -1` 的那?`if` 语句移动?`addOrUpdateTask` 函数内部?
```js
