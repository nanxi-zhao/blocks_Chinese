...
创建一个名?`dataArrIndex` 的变量，用在 `taskData` 数组上调?`findIndex()` 来获取要删除的任务索引。回调函数用参数 `item`，并在函数体里比较：`item.id` 是否等于 `buttonEl.parentElement.id`。这样就能找到对应那条任务啦，准备开刀删除?
# --hints--
你不需要改?`deleteTask` 函数的声明结构?
```js
assert.match(code, /const\s+deleteTask\s*=\s*\(\s*buttonEl\s*\)\s*=>\s*\{\s*/)
```

你需要用 `const` 声明一?`dataArrIndex`，并把它设为 `taskData.findIndex()` 的结果?
```js
assert.match(code, /const\s+deleteTask\s*=\s*\(\s*buttonEl\s*\)\s*=>\s*\{\s*const\s+dataArrIndex\s*=\s*taskData\.findIndex\(/)
```

你的 `findIndex()` 箭头函数回调要传入参?`item`?
```js
