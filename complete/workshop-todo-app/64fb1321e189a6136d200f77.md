...
# --description--
要编辑的任务已经被存放到 `currentTask` 里啦。现在把它“预填”进表单?- `titleInput.value = currentTask.title`
- `dateInput.value = currentTask.date`
- `descriptionInput.value = currentTask.description`

这样用户就能看到原来的内容并进行修改，体验更友好?
# --hints--
你需要把 `titleInput.value` 设为 `currentTask.title`?
```js
assert.match(editTask.toString(), /titleInput\.value\s*=\s*currentTask\.title\s*;?/)
```

你需要把 `dateInput.value` 设为 `currentTask.date`?
```js
assert.match(editTask.toString(), /dateInput\.value\s*=\s*currentTask\.date\s*;?/)
```

你需要把 `descriptionInput.value` 设为 `currentTask.description`?
```js
