...
解决它：在函数开头加入判?`if (!titleInput.value.trim())`。当去掉空格后没有任何字符，就弹?`alert("Please provide a title")`，然后立?`return`，阻止继续创建。可爱的小守卫让数据更干净?
# --hints--
需要在 `addOrUpdateTask` 中添?`if (!titleInput.value.trim())`?
```js
assert.match(code, /if\s*\(\s*!titleInput\.value\.trim\(\s*\)\s*\)\s*/)
```

在该分支中调?`alert("Please provide a title")`?
```js
