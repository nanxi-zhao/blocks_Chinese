...
# --description--
来到最后一个问题啦！如果任务标题或描述里含有特殊字符（比如引号），渲染出来?`id` 会导致页面行为异常，看起来像任务“消失”了一样?
解决思路：写一个函?`removeSpecialChars`，接收一个字符串，移除其中的特殊字符，只保留字母、数字、短横线和空格，让生成的 `id` 更安全可控。冲刺收尾！

# --hints--
需要定义一个函?`removeSpecialChars`?
```js
