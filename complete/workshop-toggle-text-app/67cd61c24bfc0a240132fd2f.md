...
根据 `isVisible` 状态动态显示单?`Show Message` ?`Hide Message` 会很好?

之前，你回顾了如何使用内联条件渲染?你将对按钮文本应用类似的逻辑?

下面是使用三元运算符有条件地显示单词 `Hide` ?`Show` 的示例：

```jsx
<button>{isMenuVisible ? "Hide" : "Show"} Menu</button>
```

作为研讨会的最后一步，使用三元运算符有条件地显示消息按钮的单词 `Hide` ?`Show`?将条件放?`Message` 文本之前，以便它显示 `Show Message` ?`Hide Message`?

经过最后的改变，你的工作室就完成了?测试你的按钮以查看其运行情况?

# --hints--

?`isVisible` ?`false` 时，按钮文本应显?`Show Message`?
...
?`isVisible` ?`true` 时，按钮文本应显?`Hide Message`?
...
export const ToggleApp = () => {
  ...
  return (
    ...
    --fcc-editable-region--
      {isVisible && <p id="message">I love MasterPuti!</p>}
    </div>
  );
};
```
